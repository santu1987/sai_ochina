...
///////////////////////////////////// PUNTAJE POR VALIDEZ DE LA OFERTA \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ºº	
		$o=0;
		$punto_valoferta[$t]=$row_otro->fields("peso");
			for($w=1;$w<=$coti_item;$w++){
...
$prove[$t] = $proveedor[$t].'*'.$punto_precio[$t].'*'.$punto_conpago[$t].'*'.$punto_garantia[$t].'*'.$punto_valoferta[$t].'*'.$punto_tiem_entraga[$t].'*'.$punto_total[$t];
	...
	for($cp=1;$cp<=$coti_item;$cp++){
		...
		AND
			secuencia = $se
		";
		$row_requi2=& $conn->Execute($sql_requi2);
		
		if(!$row_requi2->EOF){
			$descripcion = $row_requi2->fields("descripcion");
			//echo $nro_renglones;
		}
		$pdf->SetFont('arial','b',9);
		$pdf->Cell(277,		6,	'RENGLON Nro.'.$se.' '.strtoupper($descripcion),		'LRBT',0,'L',0);
		$pdf->Ln(6);
		$x=1;
		while ($x <= $coti_item ){
