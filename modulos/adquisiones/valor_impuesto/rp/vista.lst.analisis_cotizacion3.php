...
{*/ 
	...
	class PDF extends FPDF
	{
		...
		{		
			...
			$this->Cell(146,10,'REFERENCIA '.$numero_requisicion.' '.$asunto,1,0,'L');
			$this->Cell(43,10, $proveedor1	,1,0,'C');
			$this->Cell(43,10, $proveedor2	,1,0,'C');
			$this->Cell(43,10, $proveedor3	,1,0,'C');
			$this->Ln(10);
			$this->SetFont('Arial','B',9);
			...
			$this->Cell(22,		6,		'Precio Total',		1,0,'C',1);
			$this->Cell(21,		6,		'Precio Unit.',		1,0,'C',1);
			$this->Cell(22,		6,		'Precio Total',		1,0,'C',1);
			$this->Cell(21,		6,		'Precio Unit.',		1,0,'C',1);
			$this->Cell(22,		6,		'Precio Total',		1,0,'C',1);
		...
		function Footer()
		{
			...
			$this->Cell(70,3,	'TN. HECTOR TORREALBA' ,					0,0,'C');
			$this->Cell(70,3,	'CC. JOAO FERREIRA PEREIRA' ,				0,0,'C');
			$this->Cell(70,3,	'CN. EDGAR BERNARDO PARRA DUQUE' ,			0,0,'C');
			$this->Ln();
	...
	while(!$row_requi->EOF)	{	
		...
		$coordenada= ceil($cadena / 60)*6;
			...
			$x = $x +99;
			$y = $y +0.1;
			$pdf->MultiCell(99,	6,	$row_requi->fields('descripcion'),		'LRB'	,'L');
			$pdf->SetXY($x,$y);
	$sql_analisis="SELECT 
	...
	WHERE
		(numero_requisicion = '$cotizacion')
	AND
		(descripcion = '".$row_requi->fields('descripcion')."')
	AND
		(nombre = '$proveedor1')
	ORDER BY 
		numero_requisicion";//$proveedor1
				$row_analisis=& $conn->Execute($sql_analisis);
				
				$pdf->Cell(21,		$coordenada,	number_format($row_analisis->fields('monto'),2,',','.'),	'LRB',0,'C',0);
				$pdf->Cell(22,		$coordenada,	number_format($row_analisis->fields('total'),2,',','.'),	'LRB',0,'C',0);
...
$sql_analisis="SELECT 
	...
	ORDER BY 
		numero_requisicion";//$proveedor1
				...
				$pdf->Cell(21,		$coordenada,	number_format($row_analisis2->fields('monto'),2,',','.'),	'LRB',0,'C',0);
				$pdf->Cell(22,		$coordenada,	number_format($row_analisis2->fields('total'),2,',','.'),	'LRB',0,'C',0);
...
$sql_analisis="SELECT 
	...
	ORDER BY 
		numero_requisicion";//$proveedor1
				...
				$pdf->Cell(21,		$coordenada,	number_format($row_analisis3->fields('monto'),2,',','.'),	'LRB',0,'C',0);
				$pdf->Cell(22,		$coordenada,	number_format($row_analisis3->fields('total'),2,',','.'),	'LRB',0,'C',0);
	...
	{
	$t = 60-$tamano;
			$pdf->Cell(146,		$t,	' ',		'LRT',0,'L',0);
			$pdf->Cell(43,		$t,	'',			'LTR',0,'R',0);
			$pdf->Cell(43,		$t,	'',			'LTR',0,'R',1);
			$pdf->Cell(43,		$t,	'',			'LTR',0,'R',1);
			$pdf->Ln($t);
	...
	$total2 = $iva2+$monto2;
	$total3 = $iva3+$monto3;
			$pdf->Cell(146,		6,	'SUB-TOTAL:',						'LRT',0,'L',0);
			$pdf->Cell(43,		6,	number_format($monto1,2,',','.'),	'LTR',0,'R',0);
			$pdf->Cell(43,		6,	number_format($monto2,2,',','.'),	'LTR',0,'R',1);
			$pdf->Cell(43,		6,	number_format($monto3,2,',','.'),	'LTR',0,'R',1);
			$pdf->Ln(5);
			$pdf->Cell(146,		6,	'IVA(12%)',		'LTR',0,'L',1);
			$pdf->Cell(43,		6,	number_format($iva1,2,',','.'),		'LTR',0,'R',1);
			$pdf->Cell(43,		6,	number_format($iva2,2,',','.'),		'LTR',0,'R',1);
			$pdf->Cell(43,		6,	number_format($iva3,2,',','.'),		'LTR',0,'R',1);
			$pdf->Ln(5);
			$pdf->Cell(146,		6,	'TOTAL:',							'LTR',0,'L',0);
			$pdf->Cell(43,		6,	number_format($total1,2,',','.'),	'LTR',0,'R',0);
			$pdf->Cell(43,		6,	number_format($total2,2,',','.'),	'LTR',0,'R',1);
			$pdf->Cell(43,		6,	number_format($total3,2,',','.'),	'LTR',0,'R',1);
			$pdf->Ln(5);
			$pdf->SetDrawColor(80);
			
			$pdf->SetFont('arial','',9);
			$pdf->Cell(146,		6,	'TIEMPO DE ENTREGA',				'LRT',0,'C',0);
			$pdf->Cell(43,		6,	$tiempo_entrega1,	'LTR',0,'R',0);
			$pdf->Cell(43,		6,	$tiempo_entrega2,	'LTR',0,'R',1);
			$pdf->Cell(43,		6,	$tiempo_entrega3,	'LTR',0,'R',1);
			$pdf->Ln(5);
			$pdf->Cell(146,		6,	'VALIDEZ DE LA OFERTA',				'LRT',0,'C',0);
			$pdf->Cell(43,		6,	$validez_oferta1,	'LTR',0,'R',0);
			$pdf->Cell(43,		6,	$validez_oferta2,	'LTR',0,'R',1);
			$pdf->Cell(43,		6,	$validez_oferta3,	'LTR',0,'R',1);
			$pdf->Ln(5);
			$pdf->Cell(146,		6,	'MARCAY/O NORMA DE CALIDA',			'LRT',0,'C',0);
			$pdf->Cell(43,		6,	'	',	'LTR',0,'R',0);
			$pdf->Cell(43,		6,	'	',	'LTR',0,'R',1);
			$pdf->Cell(43,		6,	'	',	'LTR',0,'R',1);
			$pdf->Ln(5);
			$pdf->Cell(146,		6,	'CONDICIONES DE PAGO',				'LRT',0,'C',0);
			$pdf->Cell(43,		6,	$condiciones_pago1,	'LTR',0,'R',0);
			$pdf->Cell(43,		6,	$condiciones_pago2,	'LTR',0,'R',1);
			$pdf->Cell(43,		6,	$condiciones_pago3,	'LTR',0,'R',1);
			$pdf->Ln(5);
			$pdf->Cell(146,		6,	'GARANTIA',							'LRT',0,'C',0);
			$pdf->Cell(43,		6,	'	',	'LTR',0,'R',0);
			$pdf->Cell(43,		6,	'	',	'LTR',0,'R',1);
			$pdf->Cell(43,		6,	'	',	'LTR',0,'R',1);
			$pdf->Ln(5);
