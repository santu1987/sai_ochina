...
var nd=new Date().getTime();
	...
	(
		{
			url:"modulos/adquisiones/impuesto/db/grid_cuenta_contable.php",
			data:"id_grid=list_grid_"+nd+"&id_pager=pager_grid_"+nd,
			type:'POST', 
			cache: false,
			success: 
			function(data)
			{
				...
				dialog_reload=function gridReload()
				{ 
					var busq_cuenta= $("#consulta-cuenta-contable-busqueda-nombre_ad").val(); 
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/adquisiones/impuesto/db/sql_grid_cuenta_suma.php?busq_cuenta="+busq_cuenta,page:1}).trigger("reloadGrid"); 
					url="modulos/adquisiones/impuesto/db/sql_grid_cuenta_suma.php?busq_cuenta="+busq_cuenta;
					alert(url);
				}					
				crear_grid();
				var timeoutHnd; 
				var flAuto = true;
				//
				$("#consulta-cuenta-contable-busqueda2_ad").keypress(
					function(key)
					{
				...
				{
					var busq_nom= $("#consulta-cuenta-contable-busqueda2_ad").val();
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/adquisiones/impuesto/db/sql_grid_cuenta_suma.php?busq_nom="+busq_nom,page:1}).trigger("reloadGrid");					
					url="modulos/adquisiones/impuesto/db/sql_grid_cuenta_suma.php?busq_nom="+busq_nom;	
					//alert(url);
				}
				//
				$("#consulta-cuenta-contable-busqueda-nombre_ad").keypress(
					function(key)
					{
...
/////////////////////////////////////////////////////
function consulta_automatica_cuentas_contables_ad()
{
	$.ajax({
			url:"modulos/adquisiones/impuesto/db/sql_grid_cuentas_cont.php",
            data:dataForm('form_adquisiciones_db_impuesto'), 
			type:'POST',
			cache: false,
			 success:function(html)
			 {
				...
				getObj('adquisiciones_impuesto_db_cuenta_contable').value = recordset[1];
				getObj('valor_impuesto_db_id_cuenta').value=recordset[0];
				}
				else
				{
				getObj('adquisiciones_impuesto_db_cuenta_contable').value = "";
				getObj('valor_impuesto_db_id_cuenta').value="";
    ...
    <tr>
		<th>Cuenta Contable:</th>
		 ...
		 <li>
				...
				onchange="consulta_automatica_cuentas_contables()" onblur="consulta_automatica_cuentas_contables_ad()" 
				/>
