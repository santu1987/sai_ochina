...
$unidad_id = $_POST['requisiones_pr_unidad_id'];
...
/*if (!$roww->EOF)
{
	...
	$sqlNRequisicionN = "SELECT count(id_requisicion_encabezado) FROM requisicion_encabezado 
				WHERE 
					(id_organismo = '".$_SESSION['id_organismo']."')
	...
	$rowNN=& $conn->Execute($sqlNRequisicionN);
	//echo $sqlNRequisicionN;
}
...
if( $roww->EOF ){
	if($row->EOF && $numero_requision !=""){
					...
					INSERT INTO requisicion_detalle(
						...
						'".utf8_decode($_POST['requisiones_pr_producto'])."',
						".$_SESSION['id_usuario'].",
						'".$fecha."',
						'".$partida."',
						'".$generica."',
						...
						UPDATE 
							requisicion_encabezado
						SET 
						   asunto='".utf8_decode($_POST['requisiones_pr_asunto'])."', 
						   observacion='".utf8_decode($_POST['requisiones_pr_obesrvacion'])."', 
	...
	if($reglon == "")
		...
		UPDATE 
			requisicion_detalle
		SET 
			descripcion= '".utf8_decode($_POST['requisiones_pr_producto'])."',
