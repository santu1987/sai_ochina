...
$Sql="
			SELECT 
					...
					WHERE
						\"orden_compra_servicioD\".secuencia = \"solicitud_cotizacionD\".secuencia
					AND
						\"orden_compra_servicioE\".numero_requisicion = \"solicitud_cotizacionD\".numero_requisicion 
				...
				WHERE 
					(\"solicitud_cotizacionD\".id_organismo= 1 )
				AND
					(\"solicitud_cotizacionD\".numero_requisicion = '$nro_requision')
				AND
					(\"solicitud_cotizacionD\".numero_cotizacion = '$cotizacion')
				ORDER BY 
					\"solicitud_cotizacionD\".secuencia
...
";
$roww=& $conn->Execute($Sql);
$count = 0;
/*
if (!$roww->EOF)
{*/
	while(!$roww->EOF){
		$can = $roww->fields("cantidad") - $roww->fields("can_orden");
		//echo($can.'<br>');
		if($can <> 0)
			$count++;
...
$Sql = "	
				SELECT 
					DISTINCT  ON (\"solicitud_cotizacionD\".secuencia)
					\"solicitud_cotizacionD\".id_solicitud_cotizacion,
					\"solicitud_cotizacionD\".secuencia,
					\"solicitud_cotizacionD\".cantidad,
					(SELECT 
...
while (!$row->EOF) 
{
	...
	$cantidad = $row->fields("cantidad") - $row->fields("can_orden");
	//echo($cantidad."<br>");
	if($cantidad   <> ""){
