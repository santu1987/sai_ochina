<script>
var dialog;
$("#anular_orden_pr_btn_guardar").click(function() {
		setBarraEstado(mensaje[esperando_respuesta]);
		
anular_orden_numero = getObj("anular_orden_numero").value;
anular_orden_unidad = getObj("anular_orden_unidad").value;
	if(confirm("¿Desea anular la orden nº "+anular_orden_numero+" de la "+anular_orden_unidad+"?, Presione aceptar para anular")) 
	{	
		if(confirm("¿Desea transformar en la orden anulada en preorden?, Presione aceptar convertirla en preorden")) 
		{
			$.ajax (
			{
			url: "modulos/adquisiones/orden/pr/sql.convertir_pre_orden.php?orden="+anular_orden_numero,
				data:dataForm('form_anular_orden'),
				type:'POST',
				cache: false,
			...
			$.ajax (
			{
			url: "modulos/adquisiones/orden/pr/sql.anular_orden.php?orden="+anular_orden_numero,
				data:dataForm('form_anular_orden'),
				type:'POST',
				cache: false,
				success: function(html)
...
$("#anular_orden_pr_btn_consulta_nro_orden").click(function() {
if(getObj("anular_orden_unidad_id").value != ""){
	...
	$.post("modulos/adquisiones/orden/pr/grid_orden.php", { },
						...
						function crear_grid()
						{
							...
							({
								...
								url:'modulos/adquisiones/orden/pr/cmb.sql.anular_numero_orden.php?nd='+nd+'&unidad='+getObj("anular_orden_unidad_id").value,
								datatype: "json",
								colNames:['Orden','idproveedor','Proveedor'],
								colModel:[
								...
								onSelectRow: function(id){
									...
									getObj("anular_orden_numero").value=ret.orden;
...
//------------------------------------------------------------------------------------------------------------------------
//***********************************************************************************************************************
$("#anular_orden_pr_btn_cancelar").click(function() {
		getObj('anular_orden_unidad_codigo').value='';
		getObj('anular_orden_unidad').value='';
		getObj('anular_orden_numero').value='';
	});
//------------------------------------------------------------------------------------------------------------------------
//***********************************************************************************************************************
$("#anular_orden_pr_btn_consulta_unidad").click(function() {
	var nd=new Date().getTime();
	setBarraEstado("<img align='absmiddle' src='imagenes/loading.gif' /> Esperando Respuesta del Servidor...");
	$.post("modulos/adquisiones/orden/pr/grid_orden.php", { },
                        function(data)
						...
						function crear_grid()
						{
							...
							({
								...
								onSelectRow: function(id){
								var ret = jQuery("#list_grid_"+nd).getRowData(id);
									getObj('anular_orden_unidad_id').value = ret.id;
									getObj('anular_orden_unidad_codigo').value = ret.codigo;
									getObj('anular_orden_unidad').value = ret.nombre;
									dialog.hideAndUnload();
					 			},
								loadComplete:function (id){
									setBarraEstado("");
	...
	$.ajax({
			...
			cache: false,
			 success:function(html)
			 {
				...
				getObj('anular_orden_unidad_id').value = recordset[0];
				getObj('anular_orden_unidad').value=recordset[1];
				//setBarraEstado(html);
				}
				else
			 	{  
			   	getObj('anular_orden_unidad_id').value = "";
				getObj('anular_orden_unidad').value="";		
				}
			 }
		});	 	 
}
$('#anular_orden_unidad_codigo').change(consulta_automatica_unidad_ejecutora);
</script>

<div id="botonera">
	<img id="anular_orden_pr_btn_cancelar" class="btn_cancelar"src="imagenes/null.gif" />
	<!--<img id="anular_orden_pr_btn_imprimir" class="btn_imprimir"src="imagenes/null.gif" style="display:none"  />		
	<img id="anular_orden_pr_btn_consultar" class="btn_consultar" src="imagenes/null.gif"/>-->
	<img id="anular_orden_pr_btn_guardar" class="btn_guardar" src="imagenes/null.gif"  />
</div>
<form name="form_anular_orden" id="form_anular_orden">
	<table class="cuerpo_formulario" >
		<tr>
			<th class="titulo_frame" colspan="2"><img src="imagenes/iconos/desktop24x24.png" style="padding-right:5px;" align="absmiddle" />Anular Orden de Compra/Servicio</th>
		</tr>
		<tr>
			...
			<td>
				<table class="clear">
					<tr>
						<td >
							<input type="text" name="anular_orden_unidad_codigo" id="anular_orden_unidad_codigo" maxlength="5" size="5" 
							...
							<input type="text" name="anular_orden_unidad" id="anular_orden_unidad" maxlength="100" readonly size="60" >						
							<img id="anular_orden_pr_btn_consulta_unidad" class="btn_consulta_emergente" src="imagenes/null.gif"/>
							<input type="hidden" name="anular_orden_unidad_id" id="anular_orden_unidad_id" readonly>							
						</td>
					</tr>
				</table>
			</td>
		...
		<tr>
			...
			<td>
				<table class="clear" style="width:50">
					<tr>
						<td><input type="text" name="anular_orden_numero" id="anular_orden_numero" maxlength="10" ></td>
						<td><img id="anular_orden_pr_btn_consulta_nro_orden" class="btn_consulta_emergente" src="imagenes/null.gif"/></td>
					</tr>
				</table>
			</td>
		</tr>
