...
class PDF extends PDF_Code128
	{
		...
		{	
			...
			$this->Cell(0,10,'RELACIÓN CHEQUES HISTÓRICO BANCO/CUENTAS',0,0,'C');
			$this->Ln();
			...
			$this->Cell(120,6,		     'PROVEEDOR',			0,0,'L',1);
	...
	{	 if($cont==0)
			{
				...
				//	$banco=$row->fields("id_banco");
					$cuenta=$row->fields("cuenta_banco");
						...
						$pdf->Cell(120,6,substr(strtoupper($proveedor),0,80),	0,0,'L',1);
						...
						$usuario_ant=$nombre_usuario;
							...
							else
							{
								...
								$n_cheque=$n_cheque_sig;
								$total_cuenta=$total_cuenta+$monto_cheques2;
								$pdf->Cell(235,6,"Total Cuenta:".number_format($total_cuenta,2,',','.'),0,1,'R',1);
								$total_cuenta=0;
							}
							if($coordenada>=16)
								{
