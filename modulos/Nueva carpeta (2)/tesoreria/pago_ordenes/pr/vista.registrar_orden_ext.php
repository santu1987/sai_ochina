		...
		$.ajax (
		{
			...
			success: function(html)
			{
				if (html=="Actualizado")
				{
					...
					jQuery("#list_orden_pago_ext").setGridParam({url:"modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php"}).trigger("reloadGrid");
				}
...
{	
	...
	{//alert("entro");
		...
		$.ajax (
		{
			url: "modulos/tesoreria/pago_ordenes/pr/sql.guardar_pagado.php",
			data:dataForm('form_tesoreria_db_pagado'),
			type:'POST',
			cache: false,
			success: function(html)
			{//alert(html);
				if (html=="Registrado")
				{
					...
					clearForm('form_tesoreria_db_pagado');
					jQuery("#list_orden_pago_ext").setGridParam({url:"modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php"}).trigger("reloadGrid");
						...
						$.ajax (
									{
										...
										
										{
											...
											{	recordset=html;
												...
												url="pdf.php?p=modulos/tesoreria/cheques/rp/vista.lst.cheque.phpøid_banco="+getObj('tesoreria_pago_db_banco_id_banco').value+"@ncheque="+recordset[0]+"@proveedor="+getObj('tesoreria_pago_pr_proveedor_id').value+"@ordenes="+getObj('tesoreria_pago_db_ordenes_pago').value+"@caducidad="+getObj('tesoreria_pago_pr_caducidad').value+"@endosable="+getObj('tesoreria_pago_db_endosable_oculto').value+"@secuencia="+recordset[1]+"@proyecto="+recordset[3]+"@ejecutora="+recordset[2]+"@partida="+recordset[4]; 
												modificar_inactivo();
												openTab("cheques",url);
										/*	
...
//alert(urls);
	...
	$.post("modulos/tesoreria/cheques/pr/grid_cheques.php", { },
						...
						//jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_pago_db_n_cuenta').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value,page:1}).trigger("reloadGrid");
									jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value+'&rif='+getObj('tesoreria_pago_pr_proveedor_rif').value,page:1}).trigger("reloadGrid");
						...
						function consulta_cheque_banc_dosearch()
					{
						...
						function crear_grid()
						{
							...
							({
								...
								dialog.hideAndUnload();
								jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value+'&islr='+'&rif='+getObj('tesoreria_pago_pr_proveedor_rif').value,page:1}).trigger("reloadGrid");
				 			},
								loadComplete:function (id){
									setBarraEstado("");
									dialog.center();
						...
						function crear_grid()
						{
							...
							({
								...
								//	url='modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_pago_db_n_cuenta').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value+'&islr='+getObj('tesoreria_pago_pr_ret_islr').value;
									//setBarraEstado(url);
									jQuery("#list_orden_pago_ext").setGridParam({url:'modulos/tesoreria/pago_ordenes/pr/cmb.sql.orden_pago_ext.php?nd='+nd+'&proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value,page:1}).trigger("reloadGrid");
					 			url='modulos/tesoreria/pago_ordenes/pr/cmb.sql.orden_pago_ext.php?nd='+nd+'&proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value;
							//	alert(url);
								},
								loadComplete:function (id){
									setBarraEstado("");
		...
		$.post("modulos/tesoreria/cheques/pr/grid_cheques.php", { },
						...
						function crear_grid()
						{
							...
							({
								...
								datatype: "json",
								colNames:['Id','Id Banco','Banco','N Cuenta','id_proveedor','codigo_proveedor','nombre_proveedor2','nombre_proveedor','Base Imp','Concepto','porcentaje','ordenes','islr','sustraendo'],
								colModel:[
									{name:'id',index:'id', width:50,sortable:false,resizable:false,hidden:true},
								...
								*/
								url='modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value;
...
$("#list_orden_pago_ext").jqGrid({
	height: 115,
	width: 650,
	recordtext:"Consulta(s)",
	loadtext: "Recuperando InformaciÛn del Servidor",

	url:'modulos/tesoreria/pago_orden/pr/cmb.sql.orden_pago_ext.php?nd='+new Date().getTime()+'&rif='+getObj('tesoreria_pago_pr_proveedor_rif').value,
//	+'&islr='+getObj('tesoreria_pago_pr_ret_islr').value
								...
								vector2=vector.split(",");
									...
									{										
											...
											while((i<vector2.length))
											{
													...
													jQuery("#list_orden_pago_ext").setSelection(vector2[i]);
													i=i+1;		
											}
									}			
	...
	onSelectRow: function(id){
        var ret = jQuery("#list_orden_pago_ext").getRowData(id);
		html1="";
	   	s = jQuery("#list_orden_pago_ext").getGridParam('selarrrow');
		...
		$.ajax({
			url:"modulos/tesoreria/cheques/pr/sql.consulta_selec.php?id="+idd+"&vector="+s+'&rif='+getObj('tesoreria_pago_pr_proveedor_rif').value,
		    data:dataForm('form_tesoreria_db_pagado'), 
			type:'GET',
			cache: false,
			 success:function(html)
...
onSelectAll:function(id){
		...
		if(id && id!==lastsel){
			getObj('tesoreria_pago_db_ordenes_pago').value=s;
		$.ajax({
			url:"modulos/tesoreria/pago_ordenes/pr/sql.consulta_selec_ext.php?id="+idd+"&vector="+s+'&rif='+getObj('tesoreria_pago_pr_proveedor_rif').value,
		    data:dataForm('form_tesoreria_db_pagado'), 
			type:'GET',
			cache: false,
			 success:function(html)
...
//--------------------------------------------------------------------------------------------------------------------------------------
$("#tesoreria_orden_pagado_db_btn_consultar_beneficiario").click(function() {
	var nd=new Date().getTime();
	setBarraEstado("<img align='absmiddle' src='imagenes/loading.gif' /> Esperando Respuesta del Servidor...");
		$.ajax ({
		    url:"modulos/tesoreria/pago_ordenes/pr/grid_beneficiario.php",
			data:"id_grid=list_grid_"+nd+"&id_pager=pager_grid_"+nd,
			type:'POST', 
			cache: false,
			success: function(data)
			{
				dialog=new Boxy(data,{ title: 'Consulta Emergente De Empleados ', modal: true,center:false,x:0,y:0,show:false});
				dialog_reload=function gridReload(){ 
					var busq_proveedor= jQuery("#tesoreria_pr_proveedor_consulta_cheques_m").val(); 
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/pago_ordenes/pr/cmb.sql.beneficiario.php?busq_proveedor="+busq_proveedor,page:1}).trigger("reloadGrid"); 
			}	
				crear_grid();
				var timeoutHnd; 
				var flAuto = true;
				$("#tesoreria_pr_proveedor_consulta_pagado").keypress(function(key)
				{
						if(key.keyCode==27){dialog.hideAndUnload();}
...
/////////////////////
$("#tesoreria_pr_proveedor_codigo_consulta_pagado").keypress(function(key)
				{
						if(key.keyCode==27){dialog.hideAndUnload();}
...
////////////////////////////////////////										
						...
						{
							var busq_proveedor= jQuery("#tesoreria_pr_proveedor_consulta_pagado").val(); 
							jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/pago_ordenes/pr/cmb.sql.beneficiario.php?busq_proveedor="+busq_proveedor,page:1}).trigger("reloadGrid"); 
							url="modulos/tesoreria/pago_ordenes/pr/cmb.sql.beneficiario.php?busq_proveedor="+busq_proveedor;
						}
///////////////////////////////////////////
////////////////////////////////////////										
						function consulta_doc2_gridReload2()
						{
							var busq_codigo= jQuery("#tesoreria_pr_proveedor_codigo_consulta_pagado").val(); 
							jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/pago_ordenes/pr/cmb.sql.beneficiario.php?busq_codigo="+busq_codigo,page:1}).trigger("reloadGrid"); 
							url="modulos/tesoreria/pago_ordenes/pr/cmb.sql.beneficiario.php?busq_codigo="+busq_codigo;
							//alert(url);
						}						
						...
						function crear_grid()
						{
							...
							({
								...
								recordtext:"Registro(s)",
								loadtext: "Recuperando InformaciÛn del Servidor",		
								url:'modulos/tesoreria/pago_ordenes/pr/cmb.sql.beneficiario.php?nd='+nd,
								datatype: "json",
								...
								onSelectRow: function(id){
								var ret = jQuery("#list_grid_"+nd).getRowData(id);
									getObj('tesoreria_pago_orden_pr_empleado_codigo').value = ret.rif;
									getObj('tesoreria_pagado_orden_pr_empleado_nombre').value = ret.beneficiario;
	...
	$.ajax({
			...
			cache: false,
			 success:function(html)
			 {
	            ...
	            jQuery("#list_orden_pago_ext").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+recordset[0]+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value+'&rif='+getObj('tesoreria_pago_pr_proveedor_rif').value,page:1}).trigger("reloadGrid");
...
//url='modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_pago_db_n_cuenta').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value+'&precheque='+getObj('tesoreria_pago_db_n_precheque').value+'&islr='+getObj('tesoreria_pago_pr_ret_islr').value+'&ordenes='+getObj('tesoreria_pago_db_ordenes_pago').value+'&rif='+getObj('tesoreria_pago_pr_proveedor_rif').value
	jQuery("#list_orden_pago_ext").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+getObj('tesoreria_pago_pr_proveedor_id').value+'&banco='+getObj('tesoreria_pago_db_banco_id_banco').value+'&islr='+getObj('tesoreria_pago_pr_ret_islr').value+'&ordenes='+getObj('tesoreria_pago_db_ordenes_pago').value+'&rif='+getObj('tesoreria_pago_pr_proveedor_rif').value,page:1}).trigger("reloadGrid");	
...
<form method="post" id="form_tesoreria_db_pagado" name="form_tesoreria_db_pagado" onload="cargar_iframe()">
<input type="hidden"  id="tesoreria_vista_pagado" name="tesoreria_vista_pagado"/>
  ...
  <table   class="cuerpo_formulario">
   ...
   <tr>
	...
	<tr>
	 <th>Beneficiario</th>
	  <td><label>
	  <input name="tesoreria_pago_orden_pr_radio" type="radio" id="tesoreria_pago_orden_pr_radio1" onclick="getObj('tr_benef_pagado').style.display='none'; getObj('tr_porve_pagado').style.display='';" value="1" checked="CHECKED"/>
	  </label>
	 
	    Prooveedor</label>
	    &nbsp;&nbsp;
	    <label>
          <input name="tesoreria_pago_orden_pr_radio" type="radio" id="tesoreria_pago_orden_pr_radio2"  onclick="getObj('tr_benef_pagado').style.display=''; getObj('tr_porve_pagado').style.display='none';" value="0" />
   		 </label>
	
	  Empleado</label></br>
      <input type="hidden" name="tesoreria_pago_pr_op_oculto_orden" id="tesoreria_pago_pr_op_oculto_orden" value="1" /></td>
   </tr>
    <tr id="tr_porve_pagado">
		<th>Proveedor:</th>
		  <td>
				<ul class="input_con_emergente">
				<li>
				...
				<input type="text" name="tesoreria_pago_pr_proveedor_nombre" id="tesoreria_pago_pr_proveedor_nombre" size="45"
	...
	<tr  id="tr_benef_pagado" style="display:none">
			<th>Empleado</th>
      <td >		<ul class="input_con_emergente">
	  <li><input name="tesoreria_pago_orden_pr_empleado_codigo" type="text" id="tesoreria_pago_orden_pr_empleado_codigo"
				onblur="consulta_automatica_benef_manual_orden"   size="5"  maxlength="4" 
				message="Introduzca un Codigo para el Empleado."
				/>
	    <input name="tesoreria_pagado_orden_pr_empleado_nombre" type="text" id="tesoreria_pagado_orden_pr_empleado_nombre" size="45" maxlength="45"
	      ...
	      <input type="hidden" name="textprue3" id="textprue3" />
	  		...
	  		<li id="tesoreria_orden_pagado_db_btn_consultar_beneficiario" class="btn_consulta_emergente"></li>
		</ul>      </td>
	</tr>
...
<input  name="tesoreria_pago_db_ordenes_pago" type="text" id="tesoreria_pago_db_ordenes_pago" size="16" maxlength="16" />		
<input type="hidden"  id="cuentas_por_pagar_db_ordenes" name="cuentas_por_pagar_db_ordenes"/>	
<input type="hidden"  id="cuentas_por_pagar_db_factura" name="cuentas_por_pagar_db_factura"/>
		</td>
	</tr>
	<tr>
		<td class="celda_consulta" colspan="2">
				<table id="list_orden_pago_ext" class="scroll" cellpadding="0" cellspacing="0"></table> 
