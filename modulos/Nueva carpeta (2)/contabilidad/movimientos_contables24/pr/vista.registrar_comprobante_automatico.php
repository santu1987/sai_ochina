	...
	(
		{
			...
			function(data)
			{
				...
				{ 
					...
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/contabilidad/movimientos_contables/pr/cmb.sql_movimientos_contables.php?busq_cuenta="+busq_cuenta,page:1}).trigger("reloadGrid"); 
				}					
				crear_grid();
				var timeoutHnd; 
				var flAuto = true;
				...
				function gridReload()
				{
					var busq_cuenta= $("#consulta_comprobante").val();
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/contabilidad/movimientos_contables/pr/cmb.sql_movimientos_contables.php?busq_cuenta="+busq_cuenta,page:1}).trigger("reloadGrid");					
					url="modulos/contabilidad/movimientos_contables/pr/cmb.sql_movimientos_contables.php?busq_cuenta="+busq_cuenta;
                 //  alert(url);				
				}
			}
		}
						...
						function crear_grid()
						{
							jQuery("#list_grid_"+nd).jqGrid
	 						({
								...
								datatype: "json",
								colNames:['Id','Organismo','A&ntilde;o','Mes','tipo','Comprobante','Secuencia','Comentarios','Cuenta Contable','Desc','REF','Debito','Credito','Fecha Comprobante','CodAux','CodUbic','Codcost','CodUFondos','codigo_tipo_comp','id_cc','ejecutora','utf','auxiliar','codigo_proyecto','id_acc','cod_acc','r_aux','r_ejec','r_proy','r_uft','monto_dif'],
								colModel:[
				...
				getObj('contabilidad_comprobante_pr_numero_comprobante').value =ret.numero_comprobante;
		...
		/*	getObj('contabilidad_comprobante_pr_tipo').value=ret.codigo_tipo_comp;
			getObj('contabilidad_comprobante_pr_tipo_id').value=ret.id_tipo_comprobante;*/
				...
				num_comp=getObj('contabilidad_comprobante_pr_tipo').value+getObj('contabilidad_comprobante_pr_numero_comprobante').value;
 ...
 <input type="hidden" id="contabilidad_comprobante_pr_activo3" name="contabilidad_comprobante_pr_activo3" value="1"/>
  ...
  <table   class="cuerpo_formulario">
	...
	<th>N&uacute;mero Comprobante:</th>
		<td>
				<input type="text" id="contabilidad_comprobante_pr_numero_comprobante" name="contabilidad_comprobante_pr_numero_comprobante"   size='12' maxlength="12"  message="Introduzca nº comprobante" onchange="consulta_numero_comprobante_auto()"  onblur="consulta_numero_comprobante_auto()" >		
