...
if (isset($_GET['cuenta_contable']))
{
	if($_GET['cuenta_contable']!="")
	{	
		$cuenta_contable = strtolower($_GET['cuenta_contable']);
		$where.="and saldo_auxiliares.cuenta_contable='$cuenta_contable'";
	}
}
//die($cuenta_contable);
$limit = 15;
...
$Sql="
			...
			".$where."
			group by auxiliares.id_auxiliares, auxiliares.cuenta_contable , auxiliares.cuenta_auxiliar, auxiliares.id_organismo, auxiliares.nombre, auxiliares.comentarios, cuenta_contable_contabilidad.cuenta_contable 
			order by
			cuenta_auxiliar	
			";
			//die($Sql);
...
while (!$row->EOF) 
{
	...
	$responce->rows[$i]['cell']=array(	
															...
															substr($row->fields("comentarios"),0,20),
															$row->fields("nombre2")														);
	$i++;
	$cuenta_contable=0;
	$row->MoveNext();
