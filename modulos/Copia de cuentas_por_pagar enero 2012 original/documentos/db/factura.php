...
SELECT 
		...
		compromiso,
		((documentos_cxp.porcentaje_retencion_iva*(documentos_cxp.porcentaje_iva*documentos_cxp.monto_base_imponible/100)/100)+(documentos_cxp.retencion_iva2*(documentos_cxp.porcentaje_iva2*documentos_cxp.monto_base_imponible2/100)/100)+((porcentaje_retencion_islr*documentos_cxp.monto_bruto/100)-documentos_cxp.sustraendo)+(documentos_cxp.retencion_ex1+documentos_cxp.retencion_ex2)) as retenciones,
...
while(!$recordset->EOF)
	{
		...
		$numero_cheque=$recordset->fields("cheque");
		...
		if(!$row_cheques->EOF)
		{
				$monto_cheques=$row_cheques->fields("monto_cheque");
				$numero_cheques=$row_cheques->fields("numero_cheque");
		}
		$total_renglon2=$recordset->fields("total_renglon");
		$total_ren_iva=$recordset->fields("total_renglon")*$recordset->fields("porcentaje_iva")/100;
		$total_renglon2=$total_renglon2+$total_ren_iva;
...
?>		 
	  <tr>
            	...
            	<td><?php echo(number_format($total_renglon2,2,',','.'));?></td>
				<td><?php echo(number_format($recordset->fields("retenciones"),2,',','.'));?></td>
				<td><?php echo(number_format($recordset->fields("total_retenciones"),2,',','.'));?></td>
				<td><?php echo($numero_cheques);?></td>
				<td><?php echo(number_format($monto_cheques,2,',','.'));?></td>
