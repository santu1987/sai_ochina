...
{ 
	...
	class PDF extends FPDF
	{
		...
		{		
			...
			$this->Cell(127,	6,		'  Descripcion',	0,0,'L',1);
			$this->Cell(18,		6,		'Monto',			0,0,'C',1);
			$this->Ln(6);
		}
		//Pie de página
		function Footer()
	...
	$i=0;
		...
		$SqlProyectoAccion="SELECT 
								descripcion, cantidad, nombre, monto   
							FROM 
	...
	$i++;
		...
		$pdf->Cell(12,		6,	$i,											'L',0,'C',1);
		$pdf->Cell(18,		6,	$rowProyectoAccion->fields('cantidad'),		'L',0,'C',1);
		$pdf->Cell(15,		6,	$rowProyectoAccion->fields('nombre'),		'L',0,'C',1);
		$y=$pdf->GetY();
		$pdf->MultiCell(127,6,	$rowProyectoAccion->fields('descripcion'),	'L','L');
		$pdf->SetXY(182,$y);
		$pdf->Cell(18,		6,	number_format($rowProyectoAccion->fields('monto'),2,',','.'),		'LR',0,'C',1);
		$pdf->Ln(6);
