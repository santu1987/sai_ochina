...
if ($clsificacion == '403')
	$tipo_ordn = 'PREORDEN DE SERVICIO';
else
	$tipo_ordn = 'PREORDEN DE COMPRA';
...
//	$pdf=new PDF();
	...
	$sql_saldo ="
	SELECT 
		(monto_presupuesto[".date('n')."] + monto_traspasado[".date('n')."] + monto_modificado[".date('n')."] - monto_comprometido [".date('n')."]) AS monto
	FROM 
