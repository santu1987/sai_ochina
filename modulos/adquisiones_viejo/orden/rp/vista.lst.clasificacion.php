...
$sql = "
	...
	WHERE
	\"orden_compra_servicioE\".numero_orden_compra_servicio = '".$cotizacion."'
";
...
if (!$row_en->EOF)
{
	...
	class PDF extends FPDF	{
		...
		{		
			...
			$this->Cell(0,10,'DISPONIBILIDAD PRESUPUESTARIA',0,0,'C');
			$this->Ln(10);
			...
			$this->Ln(6);
			$y=$this->GetY();
			$this->MultiCell(37,5,'FECHA PRECOMPROMISO :',0,0,'C');
			$this->SetXY(47,$y);
			...
			$y=$this->GetY();
			$this->MultiCell(28,5,		'PRESUPUESTO APROBADO',	1,'C',1);
			$this->SetXY(138,$y);
...
while (!$row_detalle->EOF) 
	...
	SELECT  
		...
		(monto_presupuesto[1]+monto_presupuesto[2]+monto_presupuesto[3]+
		 ...
		 monto_presupuesto[7]) AS presupuesto_hoy,
		(monto_comprometido[1]+monto_comprometido[2]+monto_comprometido[3]+
		 ...
		 monto_comprometido[7]+monto_comprometido[8]) AS presupuesto_comprometido
	FROM 
	...
	$disponible = 	$row_pre->fields('presupuesto_hoy') - $row_pre->fields('presupuesto_comprometido') + $monto_total;
		...
		$pdf->Cell(29,	6,	number_format($row_pre->fields('presupuesto_comprometido')- $monto_total,2,',','.'),			'L',0,'C',1);
