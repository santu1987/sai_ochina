...
{
	protected $campos;
	 public function __construct($id_ramo, $id_organismo, $nombre, $comentarios, $fecha_modificacion, $ultimo_usuario)
	{
	  $this->campos[0]=$id_ramo;
	  $this->campos[1]=$id_organismo;
	  $this->campos[2]=$nombre;
	  $this->campos[3]=$comentarios;
	  $this->campos[4]=$fecha_modificacion;
	...
	public function buscarSelect(){  
	  ...
	  if ($resultSet = $conexion->consulta($sql))
	  {
		...
		{
			while(list($id_ramo, $id_organismo, $nombre, $comentarios, $fecha_modificacion, $ultimo_usuario)=pg_fetch_row($resultSet))  
			{
            $tmp[]=new buscar(
				         $id_ramo, 
						 $id_organismo, 
	...
	public function buscarDonde($busca){  
	  $conexion = new dbdatos();
	  $sql="SELECT * FROM ramos WHERE (".$busca.");";
	  if ($resultSet = $conexion->consulta($sql))
	  {
	  	if(pg_num_rows($resultSet))
		{
	...
	public function paginas(){  
	  $conexion = new dbdatos();
	  $sql="SELECT * FROM ramos ORDER BY $sortname $sortorder OFFSET  $start LIMIT $rp;";
	  if ($resultSet = $conexion->consulta($sql))
	  {
	  	if(pg_num_rows($resultSet))
		{
	...
	public function countRec() {
		$conexion = new dbdatos();
		$sql = "SELECT count(id_ramo) FROM ramos;";
		if ($resultSet = $conexion->consulta($sql))
		{
