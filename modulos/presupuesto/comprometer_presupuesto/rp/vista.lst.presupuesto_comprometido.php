...
//*************************************************************************************************************
	...
	while (!$row->EOF)
	{
		...
		$unidad_anetrior = $id_unidad;
			...
			$pdf->Cell(255,	6,	$codigo_unidad ." ".utf8_decode($unidad_ejecutora),								'TB',0,'L',1);
			$pdf->Ln(6);
			$pdf->SetFont('arial','',10);	
		}
		if($proyectos == 1){
			if ($proyecto_anetrior <> $id_proyecto){
				...
				$pdf->SetFont('Arial','B',10);
				//echo ("<br>".$codigo_unidad."&nbsp;".$unidad_ejecutora."<br>");
				$pdf->Cell(255,	6,	"Proyecto: ".$proyecto_accion_codigo ." ".utf8_decode($proyecto_accion_nombre),								'TB',0,'L',1);
				$pdf->Ln(7);	
				$pdf->SetFont('Arial','B',10);
			}
		}elseif($acciones == 1){
			if ($accion_central_anetrior <> $id_accion_central){
				...
				$pdf->Cell(255,	6,	"Accion Centralizada: ".$proyecto_accion_codigo ." ".utf8_decode($proyecto_accion_nombre),								'TB',0,'L',1);
				$pdf->Ln(7);	
				$pdf->SetFont('Arial','B',10);
			}
		}/*else{
			if ($accion_central_anetrior <> $id_accion_central ) {
				...
				$pdf->SetFont('Arial','B',10);
				//echo ("<br>".$codigo_unidad."&nbsp;".$unidad_ejecutora."<br>"); $acciones
				$pdf->Cell(255,	6,	"Proyecto /A.C.: ".$accion_central_anetrior ." ".utf8_decode($proyecto_accion_nombre),								'TB',0,'L',1);
				$pdf->Ln(7);	
				$pdf->SetFont('Arial','B',10);
			}elseif  ($accion_central_anetrior <> $id_proyecto){
						$accion_central_anetrior = $id_proyecto;
		...
		$pdf->Cell(23,	5,	number_format($calculo_dos,2,',','.'),								'L',0,'R',1);
		...
		$pdf->Cell(23,	5,	number_format($suma_calculo_dos,2,',','.'),					'LT',0,'R',1);
