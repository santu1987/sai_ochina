...
if($proyectos == 1){
	$where = $where ." AND 	proyecto.id_proyecto <> 0 ";
}
if($acciones == 1){
	$where = $where ." AND 	accion_centralizada.id_accion_central <> 0 ";
}
if($acciones_espe == 1){
	$where = $where ." AND 	accion_especifica.id_accion_especifica <> 0 ";
}
//************************************************************************ @desde=9@hasta=9
...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		{		
			...
			$this->Cell(0,10,'RESUMEN DE EJECUCION DE PRESUPUESTO',0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',9);
			...
			$this->SetTextColor(0);
			$this->Cell(30,				12,		'CUENTA ',						0,0,'L',1);
			$this->Cell(100,			12,		'DESCRIPCION',					0,0,'L',1);
			$this->SetFont('Arial','B',8);
			$this->Cell(30,				12,		'CORREGIDO',					'RL',0,'C',1);
			$y=$this->GetY();
			$this->SetFont('Arial','B',7);
			$this->MultiCell(30,		12,		'COMPROMETIDO',					0,'C',1);
			$this->SetXY(200,$y);
			$this->MultiCell(30,		4,		'DISPONIBLE SEGUN COMPROMETIDO',0,'C',1);
			$this->SetXY(230,$y);
			$this->SetFont('Arial','B',8);
			$this->MultiCell(30,		12,		'CAUSADO',						0,'C',1);
			$this->SetXY(260,$y);
			$this->MultiCell(30,		4,		'DISPONIBLE SEGUN CAUSADO',		0,'C',1);
			//$this->Ln(6);
		}
		//Pie de página
		function Footer()
...
//*************************************************************************************************************
	...
	while (!$row->EOF)
	{
		...
		$unidad_anetrior = $id_unidad;
			...
			$pdf->Cell(279,	6,	$codigo_unidad ." ".utf8_decode($unidad_ejecutora),								'TB',0,'L',1);
			$pdf->Ln(6);
			$pdf->SetFont('arial','',10);	
		}
		if($proyectos == 1){
			if ($proyecto_anetrior <> $id_proyecto){
				$proyecto_anetrior = $id_proyecto;
				$pdf->SetFont('Arial','B',10);
				//echo ("<br>".$codigo_unidad."&nbsp;".$unidad_ejecutora."<br>");
				$pdf->Cell(280,	6,	"Proyecto: ".$proyecto_accion_codigo ." ".utf8_decode($proyecto_accion_nombre),								'TB',0,'L',1);
				$pdf->Ln(7);	
				$pdf->SetFont('Arial','B',10);
			}
		}
		if($acciones == 1){
			if ($accion_central_anetrior <> $id_accion_central){
				$accion_central_anetrior = $id_accion_central;
				$pdf->SetFont('Arial','B',10);
				//echo ("<br>".$codigo_unidad."&nbsp;".$unidad_ejecutora."<br>"); $acciones
				$pdf->Cell(280,	6,	"Accion Centralizada: ".$proyecto_accion_codigo ." ".utf8_decode($proyecto_accion_nombre),								'TB',0,'L',1);
				$pdf->Ln(7);	
				$pdf->SetFont('Arial','B',10);
			}
		}
		...
		$pdf->Cell(30,	5,	$partidas ,															0,0,'L',1);
		$pdf->Cell(100,	5,	utf8_decode($row->fields("clasificador_presupuestario")),						0,0,'L',1);
		$pdf->Cell(30,	5,	number_format($presupuesto_corregido,2,',','.'),					'RL',0,'R',1);
		$pdf->Cell(30,	5,	number_format($presupuesto_comprometido,2,',','.'),					'L',0,'R',1);
		$pdf->Cell(30,	5,	number_format($calculo_uno,2,',','.'),								'L',0,'R',1);
		$pdf->Cell(30,	5,	number_format($presupuesto_causado,2,',','.'),						'L',0,'R',1);
		$pdf->Cell(30,	5,	number_format($calculo_dos,2,',','.'),								'L',0,'R',1);
		$pdf->Ln(5);

		//echo ("&nbsp;&nbsp;&nbsp;".$proyecto_accion_codigo."&nbsp;".$proyecto_accion_nombre."<br>");
		$row->MoveNext();
	}
		$pdf->Cell(130,	5,	'TOTAL' ,													'T',0,'L',1);
		$pdf->Cell(30,	5,	number_format($suma_coreegido,2,',','.'),					'RTL',0,'R',1);
		$pdf->Cell(30,	5,	number_format($suma_comprometido,2,',','.'),				'LT',0,'R',1);
		$pdf->Cell(30,	5,	number_format($suma_calculo_uno,2,',','.'),					'LT',0,'R',1);
		$pdf->Cell(30,	5,	number_format($suma_causado,2,',','.'),						'LT',0,'R',1);
		$pdf->Cell(30,	5,	number_format($suma_calculo_dos,2,',','.'),					'LT',0,'R',1);
		$pdf->Ln(5);
