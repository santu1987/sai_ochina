...
SELECT 
	...
	(monto_presupuesto[1]+monto_presupuesto[2]+monto_presupuesto[3])AS primer_trimestre, 
	(monto_presupuesto[4]+monto_presupuesto[5]+monto_presupuesto[6])AS segundo_trimestre,
	(monto_presupuesto[7]+monto_presupuesto[8]+monto_presupuesto[9])AS tercer_trimestre,
	(monto_presupuesto[10]+monto_presupuesto[11]+monto_presupuesto[12])AS cuarto_trimestre
FROM 
...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		{		
			...
			$this->Cell(77,				10,		'PROYECTO Y/O ACCION CENTRALIZADA 1','B',0,'C',1);
...
//*************************************************************************************************************
	...
	$total = $row->fields("primer_trimestre")+$row->fields("segundo_trimestre")+$row->fields("tercer_trimestre")+$row->fields("cuarto_trimestre");
		if ($ac <> $row->fields("id_unidad_ejecutora")){
			$pdf->SetFont('Arial','B',10);
			$pdf->Cell(270,	5,	$row->fields("codigo_unidad_ejecutora")." ".utf8_decode($row->fields("unidad_ejecutora")),								1,0,'L',1);
			$pdf->SetFont('Arial','',9);
			$ac = $row->fields("id_unidad_ejecutora");
			$pdf->Ln(5.3);
		}/*else{
			$pdf->Cell(45,	5,	'',								'RLB',0,'L',1);
		}
		*/
		...
		if($row->fields("id_proyecto") !=0){
			if ($id_proyecto == $row->fields("id_proyecto") )
			{
				...
				$proyecto = $row->fields("codigo_proyecto").' '.utf8_decode(substr($row->fields("nombre"),0,60));
			}
			$pdf->Cell(77,	5,	$proyecto,								'LRB',0,'L',1);
		}else{
			if ($id_accion_centralizada == $row->fields("id_accion_centralizada") )
			{
				...
				$acion = $row->fields("codigo_accion_central").' '.substr(utf8_decode($row->fields("denominacion")),0,60);
			}
			$pdf->Cell(77,	5,	$acion,					'LRB',0,'L',1);
		}
		if ($id_accion_especifica == $row->fields("id_accion_especifica")){
			$pdf->Cell(65,	5,	'',					'LRB',0,'L',1);	
		}else{
			$id_accion_especifica = $row->fields("id_accion_especifica");
			$pdf->Cell(65,	5,	$row->fields("codigo_accion_especifica").' '.utf8_decode(substr($row->fields("accion_especifica"),0,60)),					'LRB',0,'L',1);	
		}
