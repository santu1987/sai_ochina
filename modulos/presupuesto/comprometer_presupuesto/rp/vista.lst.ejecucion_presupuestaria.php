...
SELECT
	...
	where
		\"presupuesto_ejecutadoR\".partida = traspaso_entre_partidas.partida_receptora
	AND
		\"presupuesto_ejecutadoR\".generica = traspaso_entre_partidas.generica_receptora
	AND
		\"presupuesto_ejecutadoR\".especifica = traspaso_entre_partidas.especifica_receptora
	AND
		\"presupuesto_ejecutadoR\".sub_especifica = traspaso_entre_partidas.subespecifica_receptora
	)AS monto_traspasado1,
	 (
	SELECT
		SUM(monto_cedente)
	FROM
		traspaso_entre_partidas
	where
		\"presupuesto_ejecutadoR\".partida = traspaso_entre_partidas.partida_cedente
	AND
		\"presupuesto_ejecutadoR\".generica = traspaso_entre_partidas.generica_cedente
	AND
		\"presupuesto_ejecutadoR\".especifica = traspaso_entre_partidas.especifica_cedente
	AND
		\"presupuesto_ejecutadoR\".sub_especifica = traspaso_entre_partidas.subespecifica_cedente
	)AS monto_traspasado2
FROM
...
$dispo_presuxx = $correjido_monto - $comprometido_todo; 
$dispo_totalxx = $correjido_monto -( $rowprecomprometido_todo->fields('numero_precompromiso') + $comprometido_todo);
...
$rowcomprometido_par=& $conn->Execute($sqlcomprometido_par);	
	...
	$monto_traspasado_par = $rowpartid->fields("monto_traspasado1") - $rowpartid->fields("monto_traspasado2");

$correjido_monto =  $rowpartid->fields("presupuesto") + $monto_traspasado_par  +$rowpartid->fields("monto_modificado");
$dispo_presuxx = $correjido_monto - $comprometido_par ; 
$dispo_totalxx = $correjido_monto -( $rowprecomprometido_par->fields('numero_precompromiso') + $comprometido_par);
				$pdf->SetFont('arial','B',10);
				...
				$pdf->Cell(28,	5,			number_format($monto_traspasado_par ,2,',','.'),		'TRL',0,'C',1);
...
$monto_traspasado_gene = $rowgene->fields("monto_traspasado1") - $rowgene->fields("monto_traspasado2");
$correjido_montoxl =  $rowgene->fields("presupuesto") + $monto_traspasado_gene +$rowgene->fields("monto_modificado");
$dispo_presuxxly = $correjido_montoxl - $comprometido_gen; 
$dispo_presuxxl = $correjido_montoxl -( $rowprecomprometido_gene->fields('numero_precompromiso') + $comprometido_gen);
				...
				$pdf->Cell(28,	5,			number_format($monto_traspasado_gene,2,',','.'),					'TRL',0,'C',1);
			...
			$dispo_presu = $presu_correj - $comprometido;
			$dispo_total = $presu_correj - ($rowprecomprometido->fields('numero_precompromiso') + $comprometido);
			
			$monto_traspasado = $row->fields('monto_traspasado1') - $row->fields('monto_traspasado2');
		$suma_total = 0;
			$pdf->Cell(23,	5,			$row->fields('partida').".".$row->fields('generica').".".$row->fields('especifica').".".$row->fields('sub_especifica'),	'TRL',0,'C',1);
			$pdf->Cell(28,	5,			number_format($row->fields('presupuesto'),2,',','.'),								'TRL',0,'R',1);
			$pdf->Cell(28,	5,			number_format($monto_traspasado,2,',','.'),						'TRL',0,'R',1);
