...
SELECT 
	...
	accion_centralizada.denominacion AS accion_central,
...
//$unidad_ejecutora= $row->fields('unidad');
if($unidad  != "")
	$unidad_ejecutora= $row->fields('unidad');
else
	$unidad_ejecutora= "";
$accion_central= $row->fields('accion_central');
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		function Header()
		{		
			global $unidad_ejecutora,  $ano,$accion_central, $codigo_unidad,  $texto;
			...
			//$this->Cell(100,7,'UNIDAD '.utf8_decode($unidad_ejecutora),0,0,'L');
			if ($unidad_ejecutora != "")
				$this->Cell(135,7,'UNIDAD '.utf8_decode($unidad_ejecutora),0,0,'L');
			else
				$this->Cell(135,7,' ',0,0,'L');
			$this->Cell(135,7,'ACCION CENTRAL '.utf8_decode($accion_central),0,0,'R');
			$this->Ln();
			$this->SetFont('Arial','B',9);
...
$fecha = $dia.'-'.$mes.'-'.$ano;
		...
		$y=$pdf->GetY();
		//$y= $y+6;
		$pdf->MultiCell(60,	5,		utf8_decode($row->fields('nombre')),			'T','L',1);
		$pdf->SetXY(110,$y);
