...
$unidad_anetrior =0;
$accion_central = $row->fields('codigo_accion_central')." ".$row->fields('denominacion');
if (!$row->EOF){
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		function Header()
		{		
			global $unidad_ejecutora,  $ano, $accion_central, $partida,  $texto;
			...
			$this->Cell(0,10,'RESUMEN DE EJECUCIÓN DE PRESUPUESTO POR PARTIDA',0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',9);
			$this->Cell(0,10, $texto,0,0,'C');
			$this->Ln();
			$this->SetFont('Arial','B',13);
			$this->Cell(140,10,'Año: '.$ano,0,0,'L');
			$this->Cell(150,10,'Accion Centralizada: '.$accion_central,0,0,'L');
			/*$this->Ln(5);
			$this->SetFont('Arial','B',10);
			$this->Cell(90,10,date("d/m/Y"),0,0,'C');	*/
			$this->Ln(8);
			$this->SetFont('Arial','B',9);
...
//*************************************************************************************************************
	...
	while (!$row->EOF)
	{
		...
		$unidad_anetrior = $id_unidad;
			...
			$pdf->Cell(280,	6,	$codigo_unidad ." ".$unidad_ejecutora,								'TB',0,'L',1);
			$pdf->Ln(7);	
			$pdf->SetFont('Arial','',10);
		}
		...
		$pdf->Cell(100,	5,	$row->fields("clasificador_presupuestario"),						0,0,'L',1);
