...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		{		
			...
			$this->Cell(0,10,'RESUMEN DE MODIFICACION DE PRESUPUESTO',0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',9);
			...
			$this->Cell(136,			10,		'PROYECTO O ACCIÓN CENTRAL',	'B',0,'L',1);
...
//*************************************************************************************************************
	...
	while (!$row->EOF)
	{
		...
		}elseif($row->fields('id_accion_centralizada') <> 0){
		...
		$sql_pa = "
		SELECT 
			COUNT(id_modificacion_ley) AS contar_par, 
			SUM(monto)  AS monto_par
			FROM 
			modificacion_ley
		WHERE
			partida = '".$row->fields('partida')."'
		AND
			(modificacion_ley.id_proyecto = ".$row->fields('id_proyecto').")
		AND
			(modificacion_ley.id_accion_centralizada = ".$row->fields('id_accion_centralizada').")
		";
		...
		if($proyectos == 1){
			...
			$pdf->Cell(136,	5,	$proyecto_accion_codigo ." ".$proyecto_accion_nombre,			'RL',0,'L',1);
		}
		if($acciones == 1){
		...
		$pdf->Cell(25,	5,	strtoupper($row->fields("mes_modificado")),			'RLB',0,'R',1);
		$pdf->Cell(22,	5,	number_format(	$negativo,2,',','.'),				'LB',0,'R',1);
		$pdf->Cell(22,	5,	number_format(	$positivo,2,',','.'),				'LRB',0,'R',1);
		$pdf->Ln(5);
		if ($parrr == $row_pa->fields('contar_par')){
		
			$pdf->SetFont('Arial','B',10);
			//echo ("<br>".$codigo_unidad."&nbsp;".$unidad_ejecutora."<br>");
			$pdf->Cell(211,	5,	'Total Partida '.$row->fields('partida'),			'RLT',0,'R',1);
			$pdf->Cell(44,	5,	$row_pa->fields('monto_par'),						'RLT',0,'C',1);
			$pdf->Ln(5);
			$pdf->SetFont('arial','',10);	
			$parrr=0;
		}
