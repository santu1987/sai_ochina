...
$where = " WHERE 1=1 ";
if ($busq_cedula!='')
$where.= " AND lower(jefe_proyecto.cedula_jefe_proyecto) like '%$busq_cedula%' ";
if ($busq_nombre!='')
...
$Sql="
			SELECT 
				...
				jefe_proyecto.cargo_jefe_proyecto, 
				jefe_proyecto.grado_jefe_proyecto ,
				jefe_proyecto.estatus 
			FROM 
				jefe_proyecto ".$where."
			AND
				(jefe_proyecto.id_organismo =".$_SESSION['id_organismo'].")
			ORDER BY 
				jefe_proyecto.nombre_jefe_proyecto,$sidx $sord 
			LIMIT 
				$limit 
			OFFSET 
				$start ;";
...
while (!$row->EOF) 
{
	...
	$responce->rows[$i]['cell']=array(	
															...
															$row->fields("cedula_jefe_proyecto"),
															$row->fields("nombre_jefe_proyecto"),
															$row->fields("cargo_jefe_proyecto"),
															$row->fields("grado_jefe_proyecto"),
															$row->fields("estatus")
														);
	$i++;
	$row->MoveNext();
