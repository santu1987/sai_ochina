...
if(!$row_cierre->EOF){
	...
	//if($rowww->fields('estatus') == 0){
		...
		$sqlBus = "SELECT  * FROM presupuesto_ley WHERE (id_organismo = ".$_SESSION['id_organismo'].") AND ((id_accion_central = ".$accion_central.") OR (id_proyecto = ".$proyecto.")) AND (id_unidad_ejecutora = ".$_POST[presupuesto_aprobado_pr_id_unidad].") AND (id_accion_especifica = ".$_POST[presupuesto_aprobado_pr_id_accion_e].")  AND (anio = '".$_POST[presupuesto_aprobado_pr_ano]."')  AND (partida = '".$partida[0]."') AND (generica = '".$partida[1]."') AND (especifica = '".$partida[2]."') AND (sub_especifica = '".$partida[3]."')";
