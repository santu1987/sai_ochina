...
{ 
		...
		if(!$rowAccionC->EOF){
			...
			$AccionE = $rowAccionC->fields('denominacion');
		}
		...
		class PDF extends FPDF
		{
			...
			{		
				...
				$this->Ln(8);
				$this->Cell(170,10,'Proyecto: '.$proyecton,0,0,'L');
				$this->Ln(8);
				$this->Cell(170,10,'Accion Especifica: '.$AccionE,0,0,'L');
				...
				$this->Cell(60,	6,			'Partida',	0,0,'C',1);
	...
	$total=0;
		...
		WHERE
			...
			$pdf->Cell(60,		6,	$partidas,			0,0,'C',1);
			...
			$monto_partida = $monto_partida + $monto_presupuesto;
				...
				if 	($e == $rowpar->fields('sum_unidad')){
					...
					$pdf->Cell(130,		6,	'Total de la Partidad '.$partida,	0,0,'R',1);
					$pdf->Cell(40,		6,	$monto_partida,						0,0,'R',1);
					$pdf->Ln(6);
					$pdf->SetFont('arial','',10);
					$e=0;
					$monto_partida =0;
	   ...
	   FROM presupuesto_ley where  (id_proyecto= $proyecto) AND (id_accion_especifica = $especifica)";
			...
			$pdf->SetFont('arial','B',10);
			$pdf->Cell(130,		6,'Total',							0,0,'R',1);
			$pdf->Cell(40,		6,number_format($rowtotal->fields("sum"),2,',','.'),			'T',0,'R',1);
			$pdf->Ln(6);
		$pdf->Output();
}else{
	class PDF extends FPDF
