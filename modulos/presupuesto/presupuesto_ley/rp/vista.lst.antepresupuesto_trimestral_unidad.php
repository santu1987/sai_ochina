...
WHERE
	anteproyecto_presupuesto.anio = '$anio' and
	anteproyecto_presupuesto.id_unidad_ejecutora = $unidades
	$where
ORDER BY 
	unidad_ejecutora.codigo_unidad_ejecutora,
	accion_centralizada.codigo_accion_central,
...
$unidad_ejecutora =utf8_decode($row->fields("unidad"));
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		function Header()
		{		
		global $codigo_unidad, $unidad_ejecutora;
			...
			$this->Ln(5);	
			$this->SetFont('Arial','B',9);
			$this->Cell(0,10, $codigo_unidad." ".$unidad_ejecutora,0,0,'L');
			$this->Ln();
			...
			$this->Cell(110,			10,		'PARTIDA',	'B',0,'C',1);
...
//*************************************************************************************************************
	...
	$total = $row->fields("total");
		...
		$pdf->Cell(110,	5,	$row->fields("partida").".".$row->fields("generica").".".$row->fields("especifica").".".$row->fields("subespecifica")." ".$row->fields("denominacion"),		'RLB',0,'L',1);
