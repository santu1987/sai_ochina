...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		{		
			...
			$this->Cell(0,10,'RESUMEN DE PRESUPUESTO DE LEY',0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',9);
			...
			$this->Cell(130,				10,		'PROYECTO Y/O ACCION CENTRALIZADA ','B',0,'C',1);
			//$this->Cell(65,				10,		'ACCION ESPECIFICA ',				'B',0,'C',1);
			$this->Cell(30,				10,		'PARTIDA',	'B',0,'C',1);
			$this->Cell(30,				10,		'APROBADO',	'B',0,'C',1);
			$this->Cell(30,				10,		'MODIFICADO',	'B',0,'C',1);
			$this->Cell(30,				10,		'TRASPASADO',	'B',0,'C',1);
			$this->Cell(30,				10,		'TOTAL',	'B',0,'C',1);
...
//*************************************************************************************************************
	...
	$total = $row->fields("monto_presupuesto")+$row->fields("monto_traspasado")+$row->fields("monto_modificado");
		if($id_unidad_ejecutora != $row->fields("id_unidad_ejecutora")){
			$pdf->Cell(280,	5,	$row->fields("codigo_unidad_ejecutora")." ".utf8_decode($row->fields("unidad_ejecutora")),								1,0,'L',1);
			$pdf->Ln(5.1);
			$id_unidad_ejecutora = $row->fields("id_unidad_ejecutora");
		}
		if($row->fields("id_proyecto") !=0){
			if ($id_proyecto != $row->fields("id_proyecto")){
				$pdf->Cell(130,	5,	$row->fields("codigo_proyecto").' '.utf8_decode(substr($row->fields("nombre"),0,60)),								'LRB',0,'L',1);
				$id_proyecto = $row->fields("id_proyecto");
			}else{
				$pdf->Cell(130,	5,	'',								'LRB',0,'L',1);
			}
		}else{
			if ($id_accion_centralizada != $row->fields("id_accion_centralizada")){
				$pdf->Cell(130,	5,	$row->fields("codigo_accion_central").' '.utf8_decode(substr($row->fields("denominacion"),0,60)),					'LRB',0,'L',1);
				$id_accion_centralizada = $row->fields("id_accion_centralizada");
			}else{
				$pdf->Cell(130,	5,	'',								'LRB',0,'L',1);
			}
		}//$pdf->Cell(90,	5,	$row->fields("codigo_accion_especifica").' '.substr($row->fields("accion_especifica"),0,60),					'LRB',0,'L',1);	
		$pdf->Cell(30,	5,	$row->fields("partida").".".$row->fields("generica").".".$row->fields("especifica").".".$row->fields("sub_especifica"),		'RLB',0,'L',1);
		$pdf->Cell(30,	5,	number_format($row->fields("monto_presupuesto"),2,',','.'),																			'LB',0,'R',1);
		$pdf->Cell(30,	5,	number_format($row->fields("monto_modificado"),2,',','.'),																			'LRB',0,'R',1);
		$pdf->Cell(30,	5,	number_format($row->fields("monto_traspasado"),2,',','.'),																			'LB',0,'R',1);
		$pdf->Cell(30,	5,	number_format($total,2,',','.'),		'LRB',0,'R',1);
		$pdf->Ln(5);
	...
	$pdf->SetFont('Arial','B',9);
		...
		$pdf->Cell(30,	5,	number_format($monto_presupuesto_t,2,',','.'),		1,0,'R',1);
		$pdf->Cell(30,	5,	number_format($monto_modificado_t,2,',','.'),		1,0,'R',1);
		$pdf->Cell(30,	5,	number_format($monto_traspasado_t,2,',','.'),		1,0,'R',1);
		$pdf->Cell(30,	5,	number_format($total_t,2,',','.'),		1,0,'R',1);
