...
$Sql="
	...
	WHERE (id_accion_central= $central) AND (id_accion_especifica = $especifica) AND (anio ='$anio')
...
{ 
		$SqlAccionC="SELECT 
					...
					WHERE 
						(accion_centralizada.id_accion_central = $central) 
					AND
		...
		if(!$rowAccionC->EOF){
			$codigoC = $rowAccionC->fields('codigo_accion_central');
			$codigoE = $rowAccionC->fields('codigo_accion_especifica');
			$AccionC = $rowAccionC->fields('accion_centralizada');
		...
		class PDF extends FPDF
		{
			...
			{		
				...
				$this->Ln(8);
				$this->Cell(175,10,'Acción Centralizada: '.$AccionC,0,0,'L');
				$this->Ln(8);
	...
	$total=0;
	   ...
	   FROM presupuesto_ley where  (id_accion_central= $central) AND (id_accion_especifica = $especifica)";
