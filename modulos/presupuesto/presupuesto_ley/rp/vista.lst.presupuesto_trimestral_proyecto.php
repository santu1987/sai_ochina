...
//************************************************************************
//if($id_proyecto != ""){
	$where = " AND presupuesto_ley.id_proyecto = $id_proyecto";
/*}else{
...
//************************************************************************
//$where = " AND \"presupuesto_ejecutadoR\".id_unidad_ejecutora = $id_proyecto ";
//************************************************************************
...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		{		
			...
			$this->Cell(0,10,'RESUMEN DE PRESUPUESTO DE LEY POR TRIMESTRE POR PROYECTO',0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',9);
...
//*************************************************************************************************************
	...
	$total = $row->fields("primer_trimestre")+$row->fields("segundo_trimestre")+$row->fields("tercer_trimestre")+$row->fields("cuarto_trimestre");
		...
		if($row->fields("id_proyecto") !=0)
			$pdf->Cell(77,	5,	$row->fields("codigo_proyecto").' '.utf8_decode(substr($row->fields("nombre"),0,60)),								'LRB',0,'L',1);
		else
			$pdf->Cell(77,	5,	$row->fields("codigo_accion_central").' '.utf8_decode(substr($row->fields("denominacion"),0,60)),					'LRB',0,'L',1);
