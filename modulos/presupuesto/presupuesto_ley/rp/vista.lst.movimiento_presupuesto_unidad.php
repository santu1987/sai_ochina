...
ON
	(clasificador_presupuestario.partida = \"presupuesto_ejecutadoR\".partida AND
	clasificador_presupuestario.generica = \"presupuesto_ejecutadoR\".generica AND 
	clasificador_presupuestario.especifica = \"presupuesto_ejecutadoR\".especifica AND 
...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		function Header()
		{	
		global   $anio,$codigo_unidad, $unidad_ejecutora, $texto;	
			...
			$this->Cell(0,10,'RESUMEN DE PRESUPUESTO DE LEY POR UNIDAD EJECUTORA',0,0,'C');
			$this->Ln(7);	
			$this->SetFont('Arial','B',8);
			$this->Cell(0,5,$texto,0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',12);
			$this->Cell(160,10,'Año: '.$anio,0,0,'L');
			$this->SetFont('Arial','B',12);
			$this->Cell(95,10,$codigo_unidad." ".utf8_decode($unidad_ejecutora),0,0,'C');	
			$this->Ln(8);
			$this->SetFont('Arial','B',9);
			...
			$this->Cell(45,				10,		'ACCION ESPECIFICA ',				'B',0,'C',1);
			$this->Cell(53,				10,		'PARTIDA',	'B',0,'C',1);
...
//*************************************************************************************************************
	...
	$total = $row->fields("monto_presupuesto")+$row->fields("monto_traspasado")+$row->fields("monto_modificado");
		...
		$pdf->Cell(45,	5,	$row->fields("codigo_accion_especifica")/*.' '.substr($row->fields("accion_especifica"),0,60)*/,					'LRB',0,'L',1);	
		$pdf->Cell(53,	5,	$row->fields("partida").".".$row->fields("generica").".".$row->fields("especifica").".".$row->fields("sub_especifica")/*." ".utf8_decode($row->fields("clasificador_presupuestario"))*/,		'RLB',0,'C',1);
