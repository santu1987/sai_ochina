...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		function Header()
		{	
		global   $anio,$codigo_accion_especifica, $accion_especifica, $texto;	
			...
			$this->Cell(0,10,'RESUMEN DE PRESUPUESTO DE LEY POR ACCION ESPECIFICA',0,0,'C');
			$this->Ln(7);	
			$this->SetFont('Arial','B',8);
			$this->Cell(0,5,$texto,0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',12);
			$this->Cell(110,10,'Año: '.$anio,0,0,'L');
			$this->SetFont('Arial','B',12);
			$this->Cell(155,10,$codigo_accion_especifica." ".$accion_especifica,0,0,'R');	
			$this->Ln(8);
			$this->SetFont('Arial','B',9);
			...
			$this->Cell(90,				10,		'PROYECTO / ACCION CENTRALIZADA ','B',0,'C',1);
			//$this->Cell(90,				10,		'ACCION ESPECIFICA ',				'B',0,'C',1);
...
//*************************************************************************************************************
	...
	$total = $row->fields("monto_presupuesto")+$row->fields("monto_traspasado")+$row->fields("monto_modificado");
		...
		if($row->fields("id_proyecto") !=0)
			$pdf->Cell(90,	5,	$row->fields("codigo_proyecto").' '.substr($row->fields("nombre"),0,60),								'LRB',0,'L',1);
		else
			$pdf->Cell(90,	5,	$row->fields("codigo_accion_central").' '.substr($row->fields("denominacion"),0,60),					'LRB',0,'L',1);
