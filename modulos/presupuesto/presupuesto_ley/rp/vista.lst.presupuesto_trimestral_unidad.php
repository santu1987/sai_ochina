...
$unidad = $row->fields("codigo_unidad_ejecutora")." ".utf8_decode($row->fields("unidad_ejecutora"));
//************************************************************************
if (!$row->EOF)
{
//die($sql);
...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		global $unidad;
			...
			$this->Cell(0,10,'RESUMEN DE PRESUPUESTO DE LEY POR TRIMESTRE POR UNIDAD',0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',9);
			...
			$this->Cell(150,10,'Año: '.date('Y'),0,0,'L');
			$this->SetFont('Arial','B',10);
			$this->Cell(140,10,$unidad,0,0,'C');	
			$this->Ln(8);
	...
	$total = $row->fields("primer_trimestre")+$row->fields("segundo_trimestre")+$row->fields("tercer_trimestre")+$row->fields("cuarto_trimestre");
		...
		if($row->fields("id_proyecto") !=0){
			...
			if ($id_accion_centralizada == $row->fields("id_accion_centralizada") )
			{
				...
				$acion = $row->fields("codigo_accion_central").' '.utf8_decode(substr($row->fields("denominacion"),0,60));
			}
