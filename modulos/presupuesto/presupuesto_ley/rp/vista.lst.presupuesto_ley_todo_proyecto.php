...
$Sql="
		...
		ON
			(
			presupuesto_ley.partida = clasificador_presupuestario.partida
			AND
			presupuesto_ley.generica = clasificador_presupuestario.generica
			AND
			presupuesto_ley.especifica = clasificador_presupuestario.especifica
			AND
			presupuesto_ley.sub_especifica = clasificador_presupuestario.subespecifica
			)
		LEFT JOIN
			accion_centralizada
		ON
		...
		AND 
			(presupuesto_ley.id_accion_central=0) 
		ORDER BY 
...
{ 
	...
	class PDF extends PDF_Code128 
		{
			...
			{		
				...
				$this->Cell(0,10,'PRESUPUESTO LEY 2011',0,0,'C');
				$this->Ln(11);	
	...
	$total=0;
	   ...
	   FROM presupuesto_ley where  
			...
			$pdf->Cell(150,		6,'Presupuesto Ley 2011',							0,0,'L',1);
