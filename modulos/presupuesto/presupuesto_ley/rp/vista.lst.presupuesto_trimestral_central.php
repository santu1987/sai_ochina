...
$unidad = $_GET['unidad'];
$centralizada = $_GET['centralizada'];
//************************************************************************
if (($desde == 01) || ($desde ==1))
	$desde_mes ='Enero';
if (($desde == 02) || ($desde ==2))
...
//************************************************************************
//if($id_proyecto != ""){
/*	$where = " AND \"presupuesto_ejecutadoR\".id_proyecto = $id_proyecto";
}else{*/
	$where = " AND presupuesto_ley.id_accion_central = $centralizada";
//}
//************************************************************************
...
//*************************************************************************************************************
	class PDF extends FPDF
	{
		...
		{		
			...
			$this->Cell(0,10,'RESUMEN DE PRESUPUESTO DE LEY POR TRIMESTRE POR ACCION CENTRALIZADA',0,0,'C');
			$this->Ln(5);	
			$this->SetFont('Arial','B',9);
...
//*************************************************************************************************************
	...
	$total = $row->fields("primer_trimestre")+$row->fields("segundo_trimestre")+$row->fields("tercer_trimestre")+$row->fields("cuarto_trimestre");
		if ($ac <> $row->fields("id_unidad_ejecutora")){
			...
			$pdf->Cell(270,	5,	$row->fields("codigo_unidad_ejecutora")." ".$row->fields("unidad_ejecutora"),								1,0,'L',1);
			$pdf->SetFont('Arial','',9);
		...
		$pdf->Cell(65,	5,	$row->fields("codigo_accion_especifica").' '.substr($row->fields("accion_especifica"),0,60),					'LRB',0,'L',1);	
