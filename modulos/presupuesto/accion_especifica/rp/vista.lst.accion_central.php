...
$Sql="SELECT * FROM accion_centralizada WHERE ano = '".date('Y')."' ORDER BY id_accion_central";
...
{ 
	...
	class PDF extends PDF_Code128 
	{
		...
		{		
			...
			$this->Cell(0,10,'LISTADO DE ACCIONES CENTRALIZADAS',0,0,'C');
			$this->Ln(10);
			$this->SetFont('Times','B',6);
			$this->SetLineWidth(0.3);
			$this->SetFillColor(0) ;
			$this->SetTextColor(255);
			$this->Cell(87,		6,			'ACCIÓN CENTRALIZADA',		1,0,'L',1);
	...
	while (!$row->EOF) 
	{
		...
		$pdf->Cell(10,		6,		$row->fields("codigo_accion_central"),		1,0,'C',1);
		if($salto2 ==0){
			$y=$pdf->GetY();
			$pdf->MultiCell(77,		$coordena1,		utf8_decode($row->fields("denominacion"))/*$contar_letra.'  '.$salto1*/,				1,'L',1);
			$pdf->SetXY(97,$y);
