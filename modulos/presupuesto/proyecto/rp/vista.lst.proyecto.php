...
$Sql="SELECT * FROM proyecto WHERE ano = '".date('Y')."' ORDER BY id_proyecto ";
...
{ 
	...
	class PDF extends PDF_Code128 
	{
		...
		{		
			...
			$this->Cell(0,10,'LISTADO DE PROYECTOS',0,0,'C');
			$this->Ln(10);
			$this->SetFont('Times','B',6);
			$this->SetLineWidth(0.3);
			$this->SetFillColor(0) ;
			$this->SetTextColor(255);
			$this->Cell(87,		6,			'PROYECTO',		1,0,'L',1);
	...
	while (!$row->EOF) 
	{
		...
		$pdf->Cell(10,		$coordena,		$row->fields("codigo_proyecto"),		1,0,'C',1);
		if($salto2 ==0){
			$y=$pdf->GetY();
			$pdf->MultiCell(77,		$coordena1,		utf8_decode($row->fields("nombre"))/*$contar_letra.'  '.$salto1*/,				1,'L',1);
			$pdf->SetXY(97,$y);
			$pdf->MultiCell(104,	$coordena,		utf8_decode($row->fields("comentario"))/*$contar_comentario.'  '.$salto2*/,						1,'L',1);
		}else{
			$y=$pdf->GetY();
			
			$pdf->MultiCell(77,		6,		utf8_decode($row->fields("nombre"))/*$contar_letra.'  '.$salto1*/,				'TRL','L',1);
			$pdf->SetXY(97,$y);
			$y=$pdf->GetY();
