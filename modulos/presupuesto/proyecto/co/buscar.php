...
{
	protected $campos;
	 public function __construct($id_proyecto, $id_organismo, $nombre, $comentarios, $ultimo_usuario, $fecha_modificacion)
	{
	  $this->campos[0]=$id_proyecto;
	...
	public function buscarSelect(){  
	  ...
	  if ($resultSet = $conexion->consulta($sql))
	  {
		...
		{
			while(list($id_proyecto, $id_organismo, $nombre, $comentarios, $ultimo_usuario, $fecha_modificacion)=pg_fetch_row($resultSet))  
			{
            $tmp[]=new buscar(
						 $id_proyecto, 
						 $id_organismo, 
	...
	public function buscarDonde($busca){  
	  $conexion = new dbdatos();
	  $sql="SELECT * FROM proyecto WHERE (".$busca.");";
	  if ($resultSet = $conexion->consulta($sql))
	  {
	  	if(pg_num_rows($resultSet))
		{
	...
	public function paginas(){  
	  $conexion = new dbdatos();
	  $sql="SELECT * FROM proyecto ORDER BY $sortname $sortorder OFFSET  $start LIMIT $rp;";
	  if ($resultSet = $conexion->consulta($sql))
	  {
	  	if(pg_num_rows($resultSet))
		{
	...
	public function countRec() {
		$conexion = new dbdatos();
		$sql = "SELECT count(id_proyecto) FROM proyecto;";
		if ($resultSet = $conexion->consulta($sql))
		{
