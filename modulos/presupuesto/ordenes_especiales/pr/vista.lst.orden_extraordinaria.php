...
$clsificacion = substr ($cotizacion, 0, 3);
	...
	$tipo_ordn = 'ORDEN DE EXTRAORDINARIA';
...
$row_firma=& $conn->Execute($sql_firma);
	...
	$direccion=utf8_decode($row->fields("direccion"));
	$telefono=$row->fields("telefono");
	$titulo=utf8_decode($row->fields("concepto"));
	...
	class PDF extends FPDF
	{
		...
		{		
			...
			$y=$this->GetY();
			if (strlen($direccion) <= 64)
				$this->MultiCell(130,16,$direccion.'. Telf. '.$telefono,'LBR','L');
			else
				$this->MultiCell(130,8,$direccion.'. Telf. '.$telefono,'LBR','L');
			$this->SetXY(140,$y);
...
//	$pdf=new PDF();
	...
	$i=0;
		...
		$SqlProyectoAccion=
			...
			WHERE
				(\"orden_compra_servicioD\".numero_pre_orden = '".substr($cotizacion,3)."')
			ORDER BY
				secuencia
		";
	...
	while (!$rowProyectoAccion->EOF) 
	{
		...
		$pdf->SetXY(148,$y);
		$pdf->Cell(31,			$coordenada,	$rowProyectoAccion->fields('partida').".".$rowProyectoAccion->fields('generica').".".$rowProyectoAccion->fields('especifica').".".$rowProyectoAccion->fields('subespecifica'),			'L',0,'R');
