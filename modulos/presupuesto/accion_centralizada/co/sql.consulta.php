...
$sidx = $_GET['sidx'];
$sord = $_GET['sord'];
$busq_nombre = strtolower($_GET['accion_centralizada_busqueda_nombre']);
$busq_anio = $_GET['accion_centralizada_busqueda_anio'];
...
if($busq_nombre!='')
	$where.= " AND  ((lower(accion_centralizada.denominacion) like '%$busq_nombre%') OR (lower(accion_centralizada.denominacion) like '$busq_nombre%'))";
	
if($busq_codigo!='')
	$where.= " AND (accion_centralizada.id_accion_central = $busq_codigo)";
	
if($busq_anio!='')
	$where.= " AND (accion_centralizada.ano = $busq_anio)";	
else
	$where.= " AND (accion_centralizada.ano = 2010)";	
$Sql="
			SELECT 
