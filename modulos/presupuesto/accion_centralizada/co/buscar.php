...
{
	protected $campos;
	 public function __construct($id_accion_central, $id_organismo, $denominacion, $comentarios, $ultimo_usuario, $fecha_modificacion)
	{
	  $this->campos[0]=$id_accion_central;
	  $this->campos[1]=$id_organismo;
	  $this->campos[2]=$denominacion;
	  $this->campos[3]=$comentarios;
	  $this->campos[4]=$ultimo_usuario;
	  $this->campos[5]=$fecha_modificacion;
    }
	...
	public function buscarSelect(){  
	  ...
	  $sql="SELECT * FROM acciones_centarlizadas;";
	  if ($resultSet = $conexion->consulta($sql))
	  {
	  	if(pg_num_rows($resultSet))
		{
			while(list($id_accion_central, $id_organismo, $denominacion, $comentarios, $ultimo_usuario, $fecha_modificacion)=pg_fetch_row($resultSet))  
			{
            $tmp[]=new buscar(
				         $id_accion_central, 
						 $id_organismo, 
	...
	public function buscarDonde($busca){  
	  $conexion = new dbdatos();
	  $sql="SELECT * FROM acciones_centarlizadas WHERE (".$busca.");";
	  if ($resultSet = $conexion->consulta($sql))
	  {
	  	if(pg_num_rows($resultSet))
		{
	...
	public function paginas(){  
	  $conexion = new dbdatos();
	  $sql="SELECT * FROM acciones_centarlizadas ORDER BY $sortname $sortorder OFFSET  $start LIMIT $rp;";
	  if ($resultSet = $conexion->consulta($sql))
	  {
	  	if(pg_num_rows($resultSet))
		{
	...
	public function countRec() {
		$conexion = new dbdatos();
		$sql = "SELECT count(id_accion_central) FROM acciones_centarlizadas;";
		if ($resultSet = $conexion->consulta($sql))
		{
