...
$("#contabilidad_cuentas_contables_db_btn_consultar").click(function() {
	...
	{		
		...
		({
			...
			datatype: "json",
			colNames:['','Cuenta','Nombre','Tipo','tipo','id_cuenta_suma','cuenta_suma','id_cuenta_presupuesto','cuenta_presupuesto','naturaleza_cuenta','requiere_auxiliar','requiere_proyecto','requiere_unidad_ejecutora','requiere_utf','cta_suma_nombre','columna'],
			colModel:[
				...
				{name:'cta_suma_nombre',index:'cta_suma_nombre', width:100,sortable:false,resizable:false,hidden:true},
				{name:'columna',index:'columna', width:100,sortable:false,resizable:false,hidden:true}
			],
			...
			onSelectRow: function(id){
				...
				$("#contabilidad_cuentas_contables_db_cuentasuma_desc").val(ret.cta_suma_nombre);
				$("#contabilidad_cuentas_contables_db_id_cuenta_presupuesto").val(ret.id_cuenta_presupuesto);
				$("#contabilidad_cuentas_contables_db_columna").val(ret.columna);
							...
							jQuery("#list_grid_"+nd).jqGrid
	 						({
								...
								var ret = jQuery("#list_grid_"+nd).getRowData(id);
									...
									$('#contabilidad_cuentas_contables_db_cuentasuma_desc').val(ret.nombre);
									dialog.hideAndUnload();
					 			},
								loadComplete:function (id){
									setBarraEstado("");
...
function consulta_automatica_cuentas_contables()
{
	$.ajax({
			...
			/*	getObj("contabilidad_cuentas_contables_db_id_cuenta_suma").value=recordset[5];
				...
				getObj('contabilidad_cuentas_contables_db_cuentasuma_desc').value=recordset[13];
				getObj('contabilidad_cuentas_contables_db_columna').value=recordset[14];
				$('#contabilidad_cuentas_contables_db_btn_eliminar').show()
				getObj('contabilidad_cuentas_contables_db_btn_cancelar').style.display='';
				getObj('contabilidad_cuentas_contables_db_btn_actualizar').style.display='';
				getObj('contabilidad_cuentas_contables_db_btn_guardar').style.display='none';
...
function consulta_cuenta_cont()
{
	$.ajax({
			url:"modulos/contabilidad/cuentas_contables/db/sql_grid_cuentas_cont_suma.php",
            data:dataForm('form_contabilidad_db_cuentas_contables'), 
			type:'POST',
			cache: false,
			 success:function(html)
			...
			//	alert(html);
				...
				getObj('contabilidad_cuentas_contables_db_cuentasuma_desc').value=recordset[2];
				getObj('contabilidad_cuentas_contables_db_id_cuenta_suma').value=recordset[0];
				}
				else
				{
				getObj('contabilidad_cuentas_contables_db_cuentasuma_desc').value = "";
				getObj('contabilidad_cuentas_contables_db_id_cuenta_suma').value="";
...
<input   type="hidden" name="contabilidad_cuentas_contables_db_id"  id="contabilidad_cuentas_contables_db_id" />
  <table   class="cuerpo_formulario">
	...
	<tr id="contabilidad_cuentas_contables_db_fila_cuenta_suma" style="display:none">
	    ...
	    <td>
            <table class="clear">
                <tr>
                    <td width="86%">	
                        <input disabled="disabled" name="contabilidad_cuentas_contables_db_cuenta_suma" type="text" id="contabilidad_cuentas_contables_db_cuenta_suma"   style="width:18ex;" maxlength="12"  onblur="consulta_cuenta_cont();" onchange="consulta_cuenta_cont();"
                                        message="Introduzca la Cuenta Suma. Ejem: '40100000' " 
                                       />
						<input type="text" id="contabilidad_cuentas_contables_db_cuentasuma_desc" name="contabilidad_cuentas_contables_db_cuentasuma_desc"	 size="35" />		   
				  <input 		type="hidden" name="contabilidad_cuentas_contables_db_id_cuenta_suma"  id="contabilidad_cuentas_contables_db_id_cuenta_suma" />					</td>
                    <td width="14%">
                        <img  style="margin-left:auto" class="btn_consulta_emergente" id="contabilidad_cuentas_contables_db_btn_consultar_cuenta_suma" src="imagenes/null.gif" />                    </td>
                </tr>                    
            </table>        </td>
   </tr>
   ...
   <tr>
   		<th>Columna:</th>
		<td><input type="text" name="contabilidad_cuentas_contables_db_columna" id="contabilidad_cuentas_contables_db_columna"  size="12"	message="Introduzca la columna que corresponde a la cuenta en los estados financieros Ej:'1' para cuentas detalle" 
					jval="{valid:/^[0-9]{1,12}$/, message:'Numero de Columna Invalido', styleType:'cover'}"
