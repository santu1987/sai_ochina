...
$where = "WHERE 1=1";
$cuentas_contables=$_POST[contabilidad_cuentas_contables_db_cuenta_contable];
...
if(!$row->EOF) 
{
	...
	if($id_cuenta_suma!="")
	{	
		$sql_csuma="select cuenta_contable_contabilidad.nombre from cuenta_contable_contabilidad  where cuenta_contable_contabilidad.id='$id_cuenta_suma'";
		//die($sql_csuma);
		$row_suma=& $conn->Execute($sql_csuma);
		if(!$row_suma->EOF)
		{
			$cta_suma_nom=$row_suma->fields("nombre");
		}
	}else
	$cta_suma_nom="";
	//
	...
	$responce=$row->fields("id")."*".$row->fields("cuenta_contable")."*".strtoupper($row->fields("nombre"))."*".$row->fields("tipo_cuenta")."*".$row->fields("tipo")."*".$row->fields("id_cuenta_suma")."*".$row->fields("cuenta_suma")."*".$row->fields("id_cuenta_presupuesto")."*".$row->fields("cuenta_presupuesto")."*".$row->fields("id_naturaleza_cuenta")."*".$row->fields("requiere_auxiliar")."*".$row->fields("requiere_unidad_ejecutora")."*".$row->fields("requiere_proyecto")."*".$cta_suma_nom."*".$row->fields("columna");
}
else
	$responce="vacio";
