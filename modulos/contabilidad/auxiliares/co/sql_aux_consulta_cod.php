...
$sord = $_GET['sord'];
//-
$saldo_auxi_desde=$_POST['saldo_auxi_desde'];
$saldo_auxi_hasta=$_POST['saldo_auxi_hasta'];
$cuenta_contable=$_POST['saldo_ant_nombre_cuenta'];
...
if($cuenta_contable!='')
{
	$sql_cuenta="select * from cuenta_contable_contabilidad where cuenta_contable='$cuenta_contable'";
	$row_cuenta_c=& $conn->Execute($sql_cuenta);
	if(!$row_cuenta_c->EOF)
	{
		$id_cc=$row_cuenta_c->fields("id");
	}
	else
		die("vacio");
	$where.="AND saldo_auxiliares.cuenta_contable=$id_cc ";

}
$cod_aux=$_POST['saldo_aux_cuenta_aux'];
if($cod_aux!='')
{

	$sql_auxiliar="select id_auxiliares from auxiliares where cuenta_auxiliar='$cod_aux' ";
	$row_cuenta_a=& $conn->Execute($sql_auxiliar);
	if(!$row_cuenta_a->EOF)
		$id_aux=$row_cuenta_a->fields("id_auxiliares");
	else
		die("vacio");
$where.= " AND saldo_auxiliares.cuenta_auxiliar='$id_aux'";
//die($where);	
}
$limit = 15;
if(!$sidx) $sidx =1;
$Sql="
			...
			INNER JOIN
				naturaleza_cuenta
			ON
				cuenta_contable_contabilidad.id_naturaleza_cuenta=naturaleza_cuenta.id	".$where;//die($Sql);
...
$responce=$row->fields("id_auxiliares")."*".$row->fields("cuenta_auxiliar")."*".$row->fields("nombre")."*".$row->fields("cc")."*".$row->fields("desc_cuenta")."*".number_format($debe_total2,2,',','.')."*".number_format($haber_total2,2,',','.')."*".number_format($saldo_movimientos,2,',','.')."*".number_format($saldo_actual,2,',','.')."*".$desde_fecha."*".$hasta_fecha."*".number_format($total_cuenta_debe_haber,2,',','.');
}else
$responce='vacio';
