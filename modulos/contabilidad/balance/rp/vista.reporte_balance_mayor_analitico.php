	...
	(
		{
			...
			function(data)
			{
				...
				{ 
					...
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/contabilidad/auxiliares/db/sql_grid_cuenta_suma.php?busq_cuenta="+busq_cuenta+"busq_nom="+busq_nom,page:1}).trigger("reloadGrid"); 
				}					
				crear_grid();
				var timeoutHnd; 
				var flAuto = true;
...
//									$('#contabilidad_mayor_rp_id_cuenta').val(ret.id);
	...
	(
		{
			...
			function(data)
			{
				...
				function gridReload()
				{
	    			var busq_nom= $("#consulta-auxiliar-contable-busqueda2").val();
					var busq_cuenta= $("#consulta-auxiliar-contable-busqueda-nombre").val();
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/contabilidad/movimientos_contables/pr/cmb.sql_auxiliares.php?busq_cuenta="+busq_cuenta+"&busq_nombre="+busq_nom+"&cuenta="+getObj('contabilidad_mayor_rp_id_cuenta').value,page:1}).trigger("reloadGrid");					
					url="modulos/contabilidad/movimientos_contables/pr/cmb.sql_auxiliares.php?busq_cuenta="+busq_cuenta+"&busq_nombre="+busq_nom+"&cuenta="+getObj('contabilidad_mayor_rp_id_cuenta').value;
                // alert(url);				
				}
			}
		}
