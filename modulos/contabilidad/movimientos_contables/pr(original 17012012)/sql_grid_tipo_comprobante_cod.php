...
//
				$valor_medida=strlen($numero_comprobante);													//echo($numero_comprobantex3);
												...
												$codigo_tipo=$row->fields("codigo_tipo_comprobante");
												$med=strlen($codigo_tipo);
										if($med==1)
										{
											$codigo_tipo="0".$codigo_tipo;
											}else
											//$codigo_tipo=$_POST[contabilidad_comp_pr_tipo];
//$comprobante=$codigo_tipo_comprobante.$_POST[contabilidad_comp_pr_numero_comprobante];

	$responce->rows[$i]['id']=$row->fields("id");
	$responce=$row->fields("id")."*".$codigo_tipo."*".$row->fields("nombre")."*".$row->fields("comentario")."*".$numero_comprobante;	
