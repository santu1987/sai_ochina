...
die($dia2."-".$mes2."-".$ano2."comp");
	...
	die*/		
			...
			if($debe==$haber)
			{
									...
									///////////////////////////////////////////asiganndo numero de comprobante contable
										    $secuencia=1;
		$sql_num="SELECT  
	...
	if(!$rs_comprobante->EOF)
	{
		...
		//		die($rs_comprobante->fields("maximo"));
...
//$comprobante=$ano.$mes.$dia.$comprobante;
	...
	$mes=$mes;
	$comprobante=$tipo_comprobante.$mes2.'000';
...
$sql_act_comp="UPDATE
					...
					where
						...
						id='$_POST[contabilidad_comprobante_pr_tipo_id]'
						";
//echo($comprobante."-".$sig_comp);die("!");
	...
	//die($comprobante);
									...
									*/
									$sql_datos="SELECT 
													...
													integracion_contable.modulo_origen,
...
$id_sumas=$id_cc;
		...
		{			
				...
				//*** NOTA: date("n") LO CAMBIE POR 4MES EN TODAS LAS OPCIONES ESTO PUEDE VOLVER A CAMBIAR..COMO TODO 			
						if($suma_cuenta!="")
						{
								$sql_mov_suma="SELECT  
										   (debe[".$mes."])as debe,
...
//echo($mes);										
														...
														//	echo($sql);														
																				...
																				if (!$conn->Execute($sql)) 
																				{
																					$responce='Error al Actualizar: '.$sql.$conn->ErrorMsg();
																					$responce=$responce."*".$debe."*".$haber;
																					die($responce);
																				}
																				else
																				{	
																					$responce="cerrado"."*".$sig_comp;
																					
																				}
											$unidad_ejecutora=$row_comp->fields("modulo_origen");
											$row_comp->MoveNext();
											$alet++;
											}
						/////////////////////////////////////- verifico deacuerdo a cual sea la uejecutora y modifico el valor del numero de comprobante en el documento que lo origino.. ejm si el documento q genero el comprobante es un cheque debo cambiar los campos de numero_comprobante y numero_comprobante_integracion a tra ves d ela siguiente rutina
						if($unidad_ejecutora=='4')
						{
								$sql_cheques="  SELECT 
														...
														INNER JOIN 
																integracion_contable
														ON
															cheques.numero_comprobante_integracion=integracion_contable.numero_comprobante		 
														where
															 integracion_contable.numero_comprobante='$comporbante_int'
														AND
															integracion_contable.ano_comprobante='$ano'	 
										...
										}else
										{
											...
											$sql_cheques="UPDATE
															...
															where
																	numero_comprobante_integracion='$comporbante_int'				
																AND numero_cheque='$numero_cheque';
															delete 
																	from
																		 integracion_contable
															where		
																	numero_comprobante='$comporbante_int'
															AND
																	integracion_contable.ano_comprobante='$ano'				
																...
																{
																	$responce='Error al Actualizar el cheque: '.$sql_cheques.$conn->ErrorMsg();
																	$responce=$responce."*".$debe."*".$haber;
																	die($responce);
																}
						...
						}//fin de si uejecutora==4//TESORERIA
				/////////////////////////////////////////
