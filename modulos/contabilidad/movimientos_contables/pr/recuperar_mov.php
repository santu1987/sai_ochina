...
{
	clearForm('form_recuve_mov');busq_cuenta="";ano="";
	jQuery("#list_comprobante_recuve").setGridParam({url:"modulos/contabilidad/movimientos_contables/pr/cmb.sql_movimientos_contables_recuve.php?busq_cuenta="+busq_cuenta+"&ano="+ano,page:1}).trigger("reloadGrid");
											//alert(url);
}
$("#contabilidad_recuve_pr_btn_consultar_n_comprobante").click(function() {
var nd=new Date().getTime();
anos=getObj('contabilidad_recuve_pr_ayo').value;
	setBarraEstado("<img align='absmiddle' src='imagenes/loading.gif' /> Esperando Respuesta del Servidor...");
	$.ajax 
	(
		{
			...
			function(data)
			{
				...
				{ 
					...
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/contabilidad/movimientos_contables/pr/sql_grid_numero_comprobante.php?busq_cuenta="+busq_cuenta+"&ano="+ano,page:1}).trigger("reloadGrid");					
	...
	function crear_grid()
	{
		...
		({
			...
			recordtext:"Registro(s)",
			loadtext: "Recuperando InformaciÛn del Servidor",		
			url:'modulos/contabilidad/movimientos_contables/pr/sql_grid_numero_comprobante.php?nd='+nd+"&ano="+anos,
			datatype: "json",
			...
			onSelectRow: function(id){
				...
				$('#contabilidad_recuve_pr_numero_comprobante2').val(ret.numero_comprobante2);
	
					jQuery("#list_comprobante_recuve").setGridParam({url:'modulos/contabilidad/movimientos_contables/pr/cmb.sql_movimientos_contables_recuve.php?nd='+new Date().getTime()+"&numero_comprobante="+getObj('contabilidad_recuve_pr_numero_comprobante2').value,page:1}).trigger("reloadGrid");
				url='modulos/contabilidad/movimientos_contables/pr/cmb.sql_movimientos_contables_recuve.php?nd='+new Date().getTime()+"&numero_comprobante="+getObj('contabilidad_recuve_pr_numero_comprobante2').value,
//alert(url);
				dialog.hideAndUnload();
			},
								...
								$.ajax (
									{
										...
										//alert(html);
											...
											}else if (recordset=="abrir_comprobante")
											{
													setBarraEstado("<div id='mensaje'><p><img align='absmiddle' src='imagenes/iconos/folder_important.png />NO SE REALIZ” LA OPERACI”N DEBIDO A QUE EL COMPROBANTE SE ENCUENTRA CERRADO, APERTURE EL COMPROBANTE PARA PODER RECUPERAR EL REGISTRO</p></div>",true,true);
											}
			...
			cache: false,
			 success:function(html)
			 {
				...
				//alert(recordset);
					if(html!="vacio")
					{
						$('#contabilidad_recuve_pr_numero_comprobante2').val(html);
						ncomp2=html;
						url='modulos/contabilidad/movimientos_contables/pr/cmb.sql_movimientos_contables_recuve.php?nd='+new Date().getTime()+"&numero_comprobante="+ncomp2;
						//alert(url);
				
						jQuery("#list_comprobante_recuve").setGridParam({url:'modulos/contabilidad/movimientos_contables/pr/cmb.sql_movimientos_contables_recuve.php?nd='+new Date().getTime()+"&numero_comprobante="+ncomp2,page:1}).trigger("reloadGrid");
...
numero_comprobante=getObj('contabilidad_recuve_pr_numero_comprobante2').value;
...
<input type="hidden" name="orden_pago_pr_cot_select" id="orden_pago_pr_cot_select"  />
  <table   class="cuerpo_formulario">
	<tr>
	<th class="titulo_frame" colspan="4"><img src="imagenes/iconos/desktop24x24.png" style=" padding-right:5px;" align="absmiddle" />Recuperar  Movimientos Contables </th>
	</tr>
	<tr>
			<th>A&ntilde;o:</th>
			<td>
				<select  name="contabilidad_recuve_pr_ayo" id="contabilidad_recuve_pr_ayo">
					<?
					$anio_inicio=date("Y");
		...
		<ul class="input_con_emergente">
		  <li>
			<input type="text" id="contabilidad_recuve_pr_numero_comprobante" name="contabilidad_recuve_pr_numero_comprobante"    message="Introduzca n comprobante" size="12" maxlength="12" onblur="consulta_manual_mov_contables();"  
				...
				jvalkey="{valid:/[0-9]/, cFunc:'|', cArgs:['Codigo: '+$(this).val()]}"/>
			<input type="hidden" id="contabilidad_recuve_pr_numero_comprobante2" name="contabilidad_recuve_pr_numero_comprobante2"    message="Introduzca n comprobante" size="12" maxlength="50" onblur="consulta_manual_mov_contables();"  />	
