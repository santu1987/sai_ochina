...
$baseimp = str_replace(".","",$_POST[tesoreria_cheque_manual_db_baseimp]);
$islr = str_replace(".","",$_POST[tesoreria_cheque_manual_db_islr]);
...
if(($cerrado!="ano")||($cerrado!="mes"))
	{
			...
			//------- VERIFICANDO SI LAS ORDENES DE PAGO NO FUERON CANCELADAS POR OTROS CHEQUES -------- // 
						if($_POST['tesoreria_cheque_manual_db_ordenes_pago']!="")
						{
							//die($_POST['tesoreria_cheques_db_ordenes_pago']);
							$vector = split( ",", $_POST['tesoreria_cheque_manual_db_ordenes_pago'] );
							...
							while($i < $contador)
							{
									...
									if(($row_orden->fields("numero_cheque")!=$_POST['tesoreria_cheque_manual_db_n_precheque']) AND ($row_orden->fields("numero_cheque")!=0))	
												die ('Error-orden');
			...
			//-----------------------------------------------------------------------------------------------------------------------------------------------
							...
							$sql_prueba="	SELECT 
											...
											WHERE cheques.numero_cheque='$_POST[tesoreria_cheque_manual_db_n_precheque]'
											AND
			...
			$id_proveedor=$_POST[tesoreria_cheque_manual_pr_proveedor_id];		
										...
										$sql="		UPDATE cheques
													SET
														concepto='$_POST[tesoreria_cheque_manual_db_concepto]',
														benef_nom='$_POST[tesoreria_manual_nombre]',
														id_proveedor='$id_proveedor',
														porcentaje_itf='$porcentaje',
														monto_cheque='".str_replace(",",".",$monto)."',
														base_imponible='".str_replace(",",".",$baseimp)."',
														porcentaje_islr='".str_replace(",",".",$islr)."',
														fecha_ultima_modificacion='".date("Y-m-d H:i:s")."',
														ultimo_usuario=".$_SESSION['id_usuario']."
													WHERE
														 cheques.numero_cheque='$_POST[tesoreria_cheque_manual_db_n_precheque]'
													AND
														cheques.id_organismo=$_SESSION[id_organismo]
													AND
														cheques.tipo_cheque='2'	
