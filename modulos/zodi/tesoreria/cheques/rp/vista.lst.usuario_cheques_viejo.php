...
$Sql="
			...
			ORDER BY
				banco.nombre,cheques.cuenta_banco,cheques.secuencia,id_proveedor,cedula_rif_beneficiario
";
			
$row=& $conn->Execute($Sql);
/*$cheque=$row->fields("numero_cheque");$where
$sql_orden="		SELECT 
						*
					FROM
						 \"orden_pagoE\"
...
{ 
	...
	class PDF extends FPDF
	{
		...
		{	global $nombre_usuario;	
			...
			$this->Cell(0,10,'RELACIÓN PREPARADOR CHEQUES REGISTRADOS',0,0,'C');
            $this->ln();
	...
	//$n_cheque=$row->fields("numero_cheque");
			...
			$pdf->Cell(45,6,substr($row->fields("concepto"),0,15),	0,0,'L',1);
			...
			if($row->fields("tipo_cheque")=='1')
			{
					$sql_orden="SELECT DISTINCT
							...
							FROM
								 \"orden_pagoE\"
							WHERE
									(\"orden_pagoE\".numero_cheque='$n_cheque')
							AND
							...
							AND
								...
								";	
				$row_orden=& $conn->Execute($sql_orden);
				$tamano=25;		//$row_orden->fields("numero_orden_pago")
				while(!$row_orden->EOF)
					{
						$pdf->SetFont('Arial','',10);
						$pdf->Cell($tamano,6,$row_orden->fields("numero_orden_pago"),0,0,'R',1);
							//$pdf->Cell($tamano,6,"gianni",0,0,'R',1);
