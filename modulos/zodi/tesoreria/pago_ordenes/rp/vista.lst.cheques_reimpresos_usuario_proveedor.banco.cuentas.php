...
$nombre_banco=strtoupper($row->fields("banco"));
$cuenta_banco=$row->fields("cuenta_banco");
...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{	global $nombre_banco;
			global $fecha;	
			global $desde;	global $hasta;global $proveedor;global $nombre_usuario;global $cuenta_banco;
			...
			$this->Cell(0,10,'Reimpresion de Cheques Proveedor',0,0,'C');
			$this->ln(5);
			$this->Cell(0,10,"USUARIO:".$nombre_usuario,0,0,'C');
			$this->ln(5);
			$this->Cell(0,10,"PROVEEDOR:".strtoupper($proveedor).":",0,0,'C');
			$this->ln(5);
			$this->Cell(0,10,"BANCO: ".$nombre_banco.$cuenta_banco,0,0,'C');
			$this->ln(5);
			...
			$this->Cell(40,6,		     'CHEQUE REIMPRESO ',	0,0,'L',1);
			$this->Cell(40,6,		     'BANCO ',	0,0,'L',1);
			$this->Cell(40,6,		     'CUENTA ',	0,0,'L',1);
			$this->Cell(50,6,		     'CHEQUE EMITIDO',0,0,'L',1);
	...
	//
		...
		$total_general=$total_general+$row->fields("monto_cheque");	
			...
			$pdf->Cell(40,6,		     $n_cheque_reimpreso,	0,0,'L',0);
			$pdf->Cell(40,6,		     substr($banco_reimpreso,0,18),	0,0,'L',0);
			$pdf->Cell(40,6,		     $row->fields("cuenta_reimpreso"),	0,0,'L',0);
			$pdf->Cell(50,6,		     $n_cheque,	0,0,'L',0);
			$pdf->Cell(30,6,		     number_format($monto,2,',','.'),	0,0,'L',0);
		...
		//	$proveedor_ant=strtoupper($row->fields("proveedor"));
			...
			else
			{
				...
				$total_cuenta=$total_cuenta+$monto;
				$pdf->Cell(253,6,"Total Cuenta:".number_format($total_cuenta,2,',','.'),0,1,'R',1);
				//$pdf->Ln();	
