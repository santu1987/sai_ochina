...
//$nombre=$row->fields("nombre");
		...
		$banco=strtoupper($row->fields("banco"));
...
{ 		$id_banco_reimpreso=$row->fields("id_banco_reimpreso");
	...
	class PDF extends FPDF
	{
		...
		{	global $nombre_banco;
			global $fecha;	
			global $desde;	global $hasta;	global $nombre_usuario;global $banco_reimpreso;
			...
			$this->Cell(0,10,'Reimpresión de Cheques Usuarios/Bancos',0,0,'C');
            $this->ln(5);
			$this->Cell(0,10,$nombre_usuario." ".$banco_reimpreso,0,0,'C');
            $this->ln(5);
			...
			$this->SetTextColor(0);
			$this->Cell(40,6,			 'BENEFICIARIO',		0,0,'L',1);
			$this->Cell(45,6,		     'CHEQUE EMITIDO',	0,0,'L',1);
			$this->Cell(40,6,		     'BANCO ',	0,0,'L',1);
			$this->Cell(40,6,		     'CUENTA ',	0,0,'L',1);
			$this->Cell(45,6,		     'CHEQUE REIMPRESO ',	0,0,'L',1);
			$this->Cell(40,6,		     'MONTO ',	0,0,'L',1);
	...
	while (!$row->EOF) 
	{
		...
		$total_general=$total_general+$row->fields("monto_cheque");	
			...
			$pdf->Cell(40,6,			 $proveedor,		0,0,'L',0);
			$pdf->Cell(45,6,		     $n_cheque_reimpreso,	0,0,'L',0);
			$pdf->Cell(40,6,		     strtoupper(substr($row->fields("banco"),0,18)),	0,0,'L',0);
			$pdf->Cell(40,6,		     $row->fields("cuenta_banco"),	0,0,'L',0);
			$pdf->Cell(45,6,		     $n_cheque,	0,0,'L',0);
			$pdf->Cell(40,6,		     number_format($monto,2,',','.'),	0,0,'R',0);
