...
$sord = $_GET['sord'];
//************************************************************************
$busq_utf=$_POST[contabilidad_ut_fondos_db_cuenta_contable];
$where="WHERE cuenta_utilizacion_fondos='$busq_utf'";
$limit = 15;
if(!$sidx) $sidx =1;
$Sql="
			SELECT 
...
if (!$row->EOF) 
{
	$responce->rows[$i]['id']=$row->fields("id_utilizacion_fondos");
	$responce =$row->fields("id_utilizacion_fondos")."*".$row->fields("cuenta_utilizacion_fondos")."*".$row->fields("nombre")."*".$row->fields("tipo")."*".$row->fields("comentarios");	
}else
{
	$responce="";
}
