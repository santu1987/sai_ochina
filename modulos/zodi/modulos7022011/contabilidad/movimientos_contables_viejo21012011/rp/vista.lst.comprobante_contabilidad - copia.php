				...
				{	
						...
						$codigo_tipo_comprobante=$row->fields("codigo_tipo_comprobante");
								if($contar==0)
								{
									...
									$pdf->ln(4);
									$pdf->Cell(0,10,'COMPROBANTE '." ".substr($numero_comprobante,2)." "."del"." ".$fecha_comprobante,0,0,'C');
									$pdf->ln(4);
										//$this->Cell(0,10,"AÑO:" .date("Y"),0,0,'C');
									$pdf->Ln(6);
...
//$pdf->Cell(40,6,"aux"." ".$row->fields("codigo_auxiliar")."  ".$auxiliar,1,0,'L',1);
							...
							if($desc2<150)
							{
								...
								/////////////////////////	
											...
											$x = $pdf->GetX();
											$y = $pdf->GetY(); 
											$pdf->MultiCell(15,3,$row->fields("cuenta_contable").$valor_concat2."  ".auxiliar,1,'LBR','L');
											$pdf->SetXY($x+15,$y);
											
											//2da
											$x = $pdf->GetX();
											$y = $pdf->GetY();
											$pdf->MultiCell(40,3,substr(strtoupper($desc),0,24).$valor_concat."".$cuenta_auxiliar." ".substr(strtoupper($auxiliar),0,20),1,'LBR','L');
											$pdf->SetXY($x+40,$y);
											//
							}
							/*else
							...
							$pdf->Cell(60,6,substr(strtoupper($row->fields("descripcion")),0,55),1,0,'L',1);
								if(($row->fields("id_utilizacion_fondos")!="")&&($row->fields("id_utilizacion_fondos")!=0))	
								{
										//1era//
											$x = $pdf->GetX();
											//$y = $pdf->GetY();
											$pdf->MultiCell(20,3,$row->fields("referencia")."    "."UF".$cuenta_utf,1,'LBR','L');
											$pdf->SetXY($x+20,$y);
								}else
								{
