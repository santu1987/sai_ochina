...
//		set_locale(LC_ALL,"es_ES@euro","es_ES","esp");
	    $pdf->Ln(15);
		$pdf->Cell(185,4,'**********'.number_format($row->fields("monto_cheque"),2,',','.').'***',0,0,'R');
		$pdf->Ln(15);
		$pdf->Cell(100,4,"********".strtoupper($row->fields("proveedor"))."********",0,0,'R');
		$pdf->Ln(25);
		$pdf->Cell(40,4,"La Guaira $dia2 de $mes de $ano",0,0,'L');
		$pdf->Ln(20);
	    ...
	    $pdf->Cell(40,4,substr($monto_escrito,0,65),0,0,'L');
		$pdf->Ln();
		$pdf->Cell(40,4,substr($monto_escrito,65,50),0,0,'L');
		$pdf->Ln();
		$pdf->Cell(40,4,substr($monto_escrito,115,50),0,0,'L');
		
		//$pdf->Cell(100,4,$monto_escrito,0,0,'L');
		//$pdf->Ln();
		$pdf->SetFont('arial','B',11);
		$pdf->Cell(115,4,number_format($row->fields("monto_cheque"),2,',','.'),0,0,'R');
		$pdf->Ln(4);
		$pdf->Cell(100,4,substr($row->fields("concepto"),0,90),0,0,'L');
		$pdf->Ln();
		$pdf->Cell(100,4,substr($row->fields("concepto"),90),0,0,'L');
		$pdf->Ln(36);
		$pdf->Cell(60,4,substr($row->fields("nombre"),0,80),0,0,'C');
    	$pdf->Cell(140,4,substr($row->fields("cuenta_banco"),0,80),0,0,'R');
		$pdf->Ln(30);
		$pdf->SetFont('arial','B',11);
		$pdf->Cell(40,4,$nombre_jefe,0,0,'L');
    	$pdf->Cell(100,4,$nombre_director,0,0,'C');
		$pdf->Ln(26);
		$pdf->Cell(40,4,$nombre_preparado,0,0,'L');
		$pdf->Cell(100,4,$nombre_administrador,0,0,'C');
		$pdf->Ln(34);
		$pdf->Cell(100,4,"      Tesoreria   ",0,0,'R');
		$pdf->Cell(50,4,number_format($row->fields("monto_cheque"),2,',','.'),0,0,'R');
