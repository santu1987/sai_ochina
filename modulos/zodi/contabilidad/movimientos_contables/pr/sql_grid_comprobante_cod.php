...
/*** '".$_SESSION['id_usuario']."'";**/
$med=strlen($_POST[contabilidad_comp_pr_tipo]);
										if($med==1)
										{
											$codigo_tipo_comprobante="0".$_POST[contabilidad_comp_pr_tipo];
											}else
											$codigo_tipo_comprobante=$_POST[contabilidad_comp_pr_tipo];
$comprobante=$codigo_tipo_comprobante.$_POST[contabilidad_comp_pr_numero_comprobante];
//die($_POST[contabilidad_comp_pr_tipo].$_POST[contabilidad_comp_pr_numero_comprobante]);
...
if(!$sidx) $sidx =1;
		$sql2="
				SELECT
						count(movimientos_contables.numero_comprobante)
				FROM
...
$Sql_int="
	...
	if (!$row_int->EOF) 
	{
		...
		//////////////////////////////////////////////////////////////////	 
						$responce->rows[$i]['id']=$row3->fields("id_movimientos_contables");
						$responce =$row3->fields("id_movimientos_contables")."*".$row3->fields("id_organismo")."*".$row3->fields("ano_comprobante")."*".$row3->fields("mes_comprobante")."*".$row3->fields("id_tipo_comprobante")."*".$row_int->fields("numero_comprobante")."*".$row3->fields("secuencia")."*".$row3->fields("comentarios")."*".$row3->fields("cuenta_contable")."*".$row3->fields("descripcion")."*".$row3->fields("referencia")."*".$debe."*".$haber."*".$row3->fields("fecha_comprobante")."*".$id_auxiliar."*".$id_uejecutora."*".$id_proyecto."*".$row3->fields("id_utilizacion_fondos")."*".$row3->fields("codigo_tipo")."*".$row3->fields("id_cc")."*".$cuenta_utf."*".$codigo_uejecutora."*".$cuenta_auxiliar."*".$cod_proyecto."*".$cod_acc."*".$row3->fields("estatus")."*".$id_acc."*".$row3->fields("requiere_auxiliar")."*".$row3->fields("requiere_unidad_ejecutora")."*".$row3->fields("requiere_proyecto")."*".$row3->fields("requiere_utilizacion_fondos")."*".$resta."*".number_format($row3->fields("monto_debito"),2,',','.')."*".number_format($row3->fields("monto_credito"),2,',','.');
