...
{
	$id=$_GET['id'];
	$wehere="and integracion_contable.id='$id'"; 
}
else
$where=" and 1=1";
//************************************************************************
...
if(!$sidx) $sidx =1;
	...
	if (!$row_int->EOF) 
	{
		...
		$responce =$row_int->fields("id")."*".$row_int->fields("codigo_organismo")."*".$row_int->fields("ano_comprobante")."*".$row_int->fields("mes_comprobante")."*".$row_int->fields("codigo_tipo_comprobante")."*".$row_int->fields("numero_comprobante")."*".$row_int->fields("secuencia")."*".$row_int->fields("comentarios")."*".$row_int->fields("cuenta_contable")."*".$row_int->fields("descripcion_cuenta")."*".$row_int->fields("referencia")."*".number_format($row_int->fields("monto_debito"),2,',','.')."*".number_format($row_int->fields("monto_credito"),2,',','.')."*".$row_int->fields("codigo_auxiliar")."*".$row_int->fields("codigo_unidad_ejecutora")."*".$row_int->fields("codigo_proyecto")."*".$row_int->fields("codigo_utilizacion_fondos");
		echo ($responce);
