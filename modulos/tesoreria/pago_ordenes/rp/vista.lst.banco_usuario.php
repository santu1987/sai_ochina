...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{	global $banco;	
			...
			$this->Cell(0,10,'RELACIÓN USUARIOS/BANCOS CHEQUES EMITIDOS',0,0,'C');
            $this->ln(5);
			$this->Cell(0,10,strtoupper($nombre_emisor)." ".strtoupper($banco),0,0,'C');
			$this->Ln(5);
		...
		/*	if($row->fields("tipo_cheque")=='1')
					{
						...
						$sql_orden="SELECT DISTINCT
								...
								while(!$row_orden->EOF)
									{
										$pdf->SetFont('Arial','',10);
										$pdf->Cell($tamano,6,$row_orden->fields("orden_pago"),0,0,'C',1);
										$tamano=$tamano+480;
										$coordenada++;
										$row_orden->MoveNext();
										$pdf->Ln();
