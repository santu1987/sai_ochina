...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				$("#tesoreria_banco_cuenta_ano").keypress(function(key)
				{	banco_dosearch();
					});
					...
					{
							var busq_banco= jQuery("#tesoreria_banco_cuenta_banco-busqueda_bancos").val();
							var busq_ano= getObj("tesoreria_banco_cuenta_ano").value;
							
							
							jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/banco_cuentas/db/sql_grid_banco_cuentas.php?busq_banco="+busq_banco+"&busq_ano="+busq_ano,page:1}).trigger("reloadGrid");			
						url="modulos/tesoreria/banco_cuentas/db/sql_grid_banco_cuentas.php?busq_banco="+busq_banco+"&busq_ano="+busq_ano;
						//alert(url);
						}
