...
//die($_GET[busq_ano]);//segun el parametro del año
if(isset($_GET[busq_ano]))
{
	$ano=$_GET[busq_ano];
	if($ano!="")
	{
		$where.=" AND banco_cuentas.ayo =$ano";
	}else
		$where.=" AND banco_cuentas.ayo =$ayo";
}else
	$where.=" AND banco_cuentas.ayo =$ayo";
//segun filtro de centa
if(isset($_GET[busq_cuenta]))
{
	$cuenta=$_GET[busq_cuenta];
	if($cuenta!="")
	{
		$where.=" AND banco_cuentas.cuenta_banco like '%$cuenta%'";
	}
}
