...
/*if(getObj('tesoreria_banco_chequeras_rp_id_banco').value!="")
{
	*/
	/*var nd=new Date().getTime();
	setBarraEstado("<img align='absmiddle' src='imagenes/loading.gif' /> Esperando Respuesta del Servidor...");
	$.post("modulos/tesoreria/chequeras/rp/sql_grid_cuentas_chequeras.php", { },
                        function(data)
                        {								
		...
		$.ajax ({
			...
			success: function(data)
			{
				...
				dialog_reload=function gridReload(){ 
					...
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/chequeras/rp/sql_grid_cuentas.php?nd="+nd+'&banco='+getObj('tesoreria_banco_chequeras_rp_id_banco').value+'&fecha='+getObj('tesoreria_banco_chequeras_rp_ayo').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid"); 
				}	
				...
				{	ano_dosearch();
					...
					function ano_gridReload()
					{
							var busq_cuenta= jQuery("#tesoreria_banco_cuentas_rp_cuenta").val(); 
jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/chequeras/rp/sql_grid_cuentas.php?nd="+nd+'&banco='+getObj('tesoreria_banco_chequeras_rp_id_banco').value+'&fecha='+getObj('tesoreria_banco_chequeras_rp_ayo').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid");
						    url="modulos/tesoreria/chequeras/rp/sql_grid_cuentas.php?nd="+nd+'&banco='+getObj('tesoreria_banco_chequeras_rp_id_banco').value+'&fecha='+getObj('tesoreria_banco_chequeras_rp_ayo').value+'&busq_cuenta='+busq_cuenta;
						  // alert(url);
						}
				}
		});			
