...
//	$pdf=new PDF('P','mm','Legal');
	...
	//	$pdf->Ln(15);
		...
		$pdf->Ln(27);
		$pdf->SetFont('arial','B',8);
		$pdf->Cell(70,4,$nombre_administrador,0,0,'C');
    	$pdf->Cell(60,4,$nombre_director,0,0,'C');
		$pdf->Ln(18);
		$pdf->Cell(70,4,$nombre_jefe,0,0,'L');
		$pdf->Cell(60,4,$nombre_preparado,0,0,'L');
		$pdf->Ln(25);
		...
		$pdf->MultiCell(180,4,$row->fields("concepto").$con,0,'LBR','L');
		if($act==1)$pdf->Ln($ln);
		$pdf->MultiCell(140,4,$row->fields("concepto").$con,0,'LBR','L');
		if($act==1)$pdf->Ln($ln);
		//$pdf->Ln(16);
		$pdf->Cell(85,4,substr($bancos[0],0,80).,0,0,'C');
    	$pdf->Cell(93,4,substr($row->fields("cuenta_banco"),0,80),0,0,'R');
		...
		$pdf->Ln(22);
		$pdf->Cell(100,4,4,$ejecutora,0,0,'L');
		$pdf->Cell(40,4,number_format($row->fields("monto_cheque"),2,',','.'),0,0,'R');
		$pdf->Ln(6);
		$pdf->Cell(60,4,$proyecto,0,0,'C');
		$pdf->Cell(30,4,$partida,0,'C');*/
