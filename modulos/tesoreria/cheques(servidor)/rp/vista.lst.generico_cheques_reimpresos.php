 ...
 if((isset($_GET['desde']))&&(isset($_GET['hasta'])))
{
	...
	$where=" WHERE
		 ...
		 ";
} //AND chequeras.estatus='1'  AND cheques.estatus!=5	AND cheques.fecha_reimpresion between'$desde' AND '$hasta'
...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{	global $nombre_banco;
			...
			$this->Cell(0,10,'Reimpresión de Cheques',0,0,'C');
            $this->ln(5);
			//$this->Cell(0,10,$nombre_usuario,0,0,'C');
            $this->ln(5);
	...
	while (!$row->EOF) 
	{
		...
		$total_general=$total_general+$row->fields("monto_cheque");	
			...
			$pdf->Cell(80,6,			 substr($proveedor,0,100),		0,0,'L',0);
			$pdf->Cell(30,6,		     $n_cheque_reimpreso,	0,0,'L',0);
			$pdf->Cell(25,6,	         strtoupper(substr($banco_reimpreso,0,15)),0,0,'L',0);
			$pdf->Cell(30,6,		     $row->fields("cuenta_reimpreso"),	0,0,'L',0);
			$pdf->Cell(30,6,		     $n_cheque,	0,0,'L',0);
			$pdf->Cell(30,6,		     strtoupper(substr($row->fields("banco"),0,18)),	0,0,'L',0);
			$pdf->Cell(30,6,		     $row->fields("cuenta_banco"),	0,0,'L',0);
			$pdf->Cell(20,6,		     number_format($monto,2,',','.'),	0,0,'C',0);
