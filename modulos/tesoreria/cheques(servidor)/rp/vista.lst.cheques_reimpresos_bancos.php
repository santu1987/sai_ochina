...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{	global $nombre_banco;
			global $fecha;	
			global $desde;	global $hasta;global $banco;
			...
			$this->Cell(0,10,'Reimpresion de Cheques/Bancos',0,0,'C');
			$this->ln(5);
			...
			$this->SetTextColor(0);
			/*
			$pdf->Cell(80,6,			 substr($proveedor,0,80),		0,0,'L',0);
			$pdf->Cell(35,6,		     $n_cheque_reimpreso,	0,0,'L',0);
			$pdf->Cell(30,6,		     substr($banco_reimpreso,0,18),	0,0,'L',0);
			$pdf->Cell(35,6,		     $row->fields("cuenta_reimpreso"),	0,0,'L',0);
			$pdf->Cell(30,6,		     $n_cheque,	0,0,'L',0);
			$pdf->Cell(25,6,		     $row->fields("cuenta_banco"),	0,0,'L',0);
			$pdf->Cell(30,6,		     number_format($monto,2,',','.'),	0,0,'R',0);
		
			*/
			$this->Cell(80,6,			 'BENEFICIARIO',		0,0,'L',1);
			$this->Cell(35,6,		     'CHEQUE REIMPRESO ',	0,0,'L',1);
			$this->Cell(30,6,		     'BANCO ',	0,0,'L',1);
			$this->Cell(35,6,		     'CUENTA ',	0,0,'L',1);
			$this->Cell(30,6,		     'CHEQUE EMITIDO',	0,0,'L',1);
			$this->Cell(25,6,		     'CUENTA ',	0,0,'L',1);
	...
	while (!$row->EOF) 
	{
		...
		$total_general=$total_general+$row->fields("monto_cheque");	
			...
			$pdf->Cell(30,6,		     $n_cheque,	0,0,'L',0);
			$pdf->Cell(25,6,		     $row->fields("cuenta_banco"),	0,0,'L',0);
			$pdf->Cell(30,6,		     number_format($monto,2,',','.'),	0,0,'C',0);
...
{	
	...
	class PDF extends FPDF
	{
		...
		function Header()
		{		
			$this->Image("../../../../imagenes/logos/logo_ochina_295x .jpg",10,10,29);
			$this->Cell(0,5,'República Bolivariana de Venezuela',0,0,'C');
			$this->Ln();
			$this->Cell(0,5,'Ministerio del Poder Popular para la Defensa',0,0,'C');
			$this->Ln();
