...
if(isset($_GET[busq_cuenta]))
{
	...
	//die($busq_cuenta);
	if($busq_cuenta!="")
	$where.="and substring(integracion_contable.numero_comprobante::varchar,11) like'%$busq_cuenta%'";
...
$ano=strtoupper($_GET["ano"]);
if(isset($_GET[ano]))
{
	$where.="and integracion_contable.ano_comprobante='$ano'";
}	
...
$Sql="
		select  
				count (distinct(cheques.numero_comprobante)) 
		from 
				integracion_contable  
		INNER JOIN
				cheques
		ON	
				integracion_contable.numero_comprobante=cheques.numero_comprobante_integracion	
		inner join
			tipo_comprobante
		on
			integracion_contable.id_tipo_comprobante=tipo_comprobante.id
		where
				cheques.numero_comprobante_integracion!=0	
		$where			
				";
//die($Sql);
