...
//bloquead por ahora en algun momento la fecha debe ser la de el dia
/*$fecha = date("Y-m-d H:i:s");
$ano=substr($fecha,0,4);
$mes=substr($fecha,5,2);
$dia=substr($fecha,8,2);
$ayos=$_POST[tesoreria_integracion_contable_ano];

*/
$fecha =$_POST[revint_comp_pr_fecha];
...
$ci=0;
///verifica si el módulo de tesoreria esta cargado de no estarlo como tal no permitirá reliaza la integracion tesoreria /contabilidad
$sql="SELECT * FROM modulo where nombre='Tesoreria' and id='4'";
$rs_modulo =& $conn->Execute($sql);
if(!$rs_modulo->EOF) 
{
	$modulo=$rs_modulo->fields('id');
}else
{
	die('no_modulo');
}
...
//	$comprobante=$ano.$mes.$dia.$comprobante;		
	...
	$numero_comprobantet=$numero_comprobante;
	$numero_comprobante=$ano.$mes.$dia.$numero_comprobante;	
	//echo($numero_comprobantet);
	//die("q");
