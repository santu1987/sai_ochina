...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
*** SE TUVO QUE MODIFICAR NUEVAMENTE ESTE SCRIPT FECHA 29/02/2012 CAMBIANDO LAS RELACIONES CON LAS TABLAS REQUISICION ENCABEZADO  A ORDEN ENCABEZADO, PARA AGOSTO DE 2011 ESTABA ASI PERO DEBIDO A INDICACIONES DADAS PARA CREA UN PROCESO QUE PERMITIERA CREAR ORDENES A CONTABILIDAD (PROGRAMA PRESUPUESTARIO) SE REALIZ” EL CAMBIO Y AHORA DEBE REVERTIRSE EN TODOS LOS PROGRAMAS DE LOS MODULOS DE CXP Y TESORERIA...
*/
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function valDia(oTxt){
 ...
 function v_fecha(oTxt){
  fech=new Date(); 
  oTxt=getObj('tesoreria_cheque_pr_fecha');
  var bOk = true;
  if (oTxt.value != ""){
   bOk = bOk && (valAno(oTxt));
   bOk = bOk && (valMes(oTxt));
...
<script type='text/javascript'>
function incluir_beneficiario()
{
	if(getObj('tesoreria_cheque_db_otro_beneficiario_oc').value=="0")
	{
		getObj('tr_empleado323').style.display='';
		getObj('tesoreria_cheque_db_otro_beneficiario_oc').value="1";
	}else
	if(getObj('tesoreria_cheque_db_otro_beneficiario_oc').value=="1")
	{
		getObj('tr_empleado323').style.display='none';
		getObj('tesoreria_cheque_db_otro_beneficiario_oc').value="0";
	}
}
...
if(getObj('tesoreria_cheques_db_ordenes_pago').value!="")
{
	...
	{getObj('tesoreria_cheques_db_monto_pagar').disabled="";
			...
			{//alert(html);
				if (html=="Actualizado")
				{
					...
					getObj('tesoreria_cheques_db_endosable_oculto').value=1;
						getObj('tesoreria_cheque_db_otro_beneficiario_oc').value='0';
						getObj('tr_empleado323').style.display='none';
						document.form_tesoreria_db_cheque.tesoreria_cheque_db_otro_beneficiario.checked="";
		...
		*/	}
				...
				if(html=="el cheque supera al monto total de la orden")
				{
					setBarraEstado("<div id='mensaje'><p><img align='absmiddle' src='imagenes/iconos/folder_important.png />EL MONTO DEL CHEQUE EXCEDE AL TOTAL DE LA ORDEN</p></div>",true,true);
					}
				else
				{
					alert(html);
...
*/						//setTimeout("limpiar()",2000);
					...
					{					
							...
							alert("LA CHEQUERA ACTUAL SE AGOT&Oacute; NO HAY MAS CHEQUERAS CARGADA PARA ESTE BANCO");
...
$("#tesoreria_cheques_pr_calcular_impuesto").click(function() {

cambio();

});
///////////////////// vista previa de precheqeu sin pasarlo a cheque
$("#tesoreria_cheque_db_btn_imprimir_precheque").click(function() {
b=0;
if($('#').jVal())
	{
		getObj('tesoreria_cheques_db_monto_pagar').disabled="";
		Boxy.ask("<div id='mensaje'><p><img align='absmiddle' src='imagenes/iconos/folder_important.png />	Desea realmente imprimir el precheque </p></div>", ["ACEPTAR","CANCELAR"], 
					function(val)
					 {
                		if(val=='ACEPTAR')
						{
...
//if (confirmar)
						$.ajax (
									{
										url: "modulos/tesoreria/cheques/pr/sql.impresion_precheques.php",
										data:dataForm('form_tesoreria_db_cheque'),
										type:'POST',
										cache: false,
										success: function(html)
										{//alert(html);
											...
											{	recordset=html;
													...
													if(getObj('tesoreria_cheque_db_otro_beneficiario_oc').value=="1")
												{
													...
													urls="pdf.php?p=modulos/tesoreria/cheques/rp/vista.lst.cheque_manual.phpøid_banco="+getObj('tesoreria_cheques_db_banco_id_banco').value+"@opcion="+getObj('tesoreria_cheque_db_otro_beneficiario_oc').value+"@ncheque="+recordset[0]+"@ncuenta="+getObj('tesoreria_cheques_db_n_cuenta').value+"@proveedor="+getObj('tesoreria_cheques_pr_proveedor_id').value+"@caducidad="+getObj('tesoreria_cheques_pr_caducidad').value+"@endosable="+getObj('tesoreria_cheques_db_endosable_oculto').value+"@secuencia="+recordset[1]+"@proyecto="+recordset[3]+"@ejecutora="+recordset[2]+"@partida="+recordset[4]; 
												//	alert(urls);
													openTab("cheques",urls);						
										...
										/*	
											url2="pdf.php?p=modulos/tesoreria/cheques/rp/vista.lst_partida_cheques.phpøordenes="+getObj('tesoreria_cheques_db_ordenes_pago').value+"@ncheque="+recordset[0]; 
...
function valor_punto_coma(cadena)
{	
...
Boxy.ask("<div id='mensaje'><p><img align='absmiddle' src='imagenes/iconos/folder_important.png /> &iquest;Desea elminar el registro seleccionado?</p></div>", ["ACEPTAR","CANCELAR"], 
function(val)
{
if(val=='ACEPTAR')
{	
...
setBarraEstado("");
	...
	getObj('tesoreria_cheques_pr_ret_islr').value="0,00";
	getObj('tesoreria_cheques_pr_sustraendo').checked='';
	getObj('tesoreria_cheques_pr_sustraendo').disabled='disabled';
	getObj('tesoreria_cheques_pr_sustraendo_oculto').value='0';
	getObj('tesoreria_cheque_db_otro_beneficiario_oc').value='0';
	getObj('tr_empleado323').style.display='none';
	getObj('tesoreria_cheque_db_btn_imprimir_precheque').style.display='none';
	document.form_tesoreria_db_cheque.tesoreria_cheque_db_otro_beneficiario.checked="";
	getObj('tesoreria_cheque_pr_fecha').value="<?=  date("d/m/Y"); ?>";
		...
		$.ajax ({
			...
			success: function(data)
			{
				...
				dialog_reload=function gridReload(){ 
					...
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/sql_grid_cuentas.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid"); 
				}	
				...
				{	ano_dosearch();
					...
					function ano_gridReload()
					{
							...
							var busq_cuenta= jQuery("#tesoreria_cheques_chequeras_pr_cuenta").val(); 
jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/sql_grid_cuentas.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid");
						url="modulos/tesoreria/cheques/pr/sql_grid_cuentas.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&busq_cuenta='+busq_cuenta;
					//	alert(url);
						}
	...
	//
						...
						//jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value,page:1}).trigger("reloadGrid");
									jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&id_cheques='+getObj('id_cheques').value,page:1}).trigger("reloadGrid");
						...
						function consulta_cheque_banc_dosearch()
					{
						...
						function crear_grid()
						{
							...
							({
								...
								dialog.hideAndUnload();
								jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&islr='+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&id_cheques='+getObj('id_cheques').value,page:1}).trigger("reloadGrid");
				 			},
								loadComplete:function (id){
									setBarraEstado("");
									dialog.center();
						...
						function crear_grid()
						{
							...
							({
								...
								var ret = jQuery("#list_grid_"+nd).getRowData(id);
									...
									rif=ret.rif;
									rif2 = rif.split("-");
									getObj('tesoreria_cheques_pr_proveedor_rif').value=rif2[0];
									if(getObj('tesoreria_cheques_pr_proveedor_rif').value=='V')
									{
										getObj('tesoreria_cheques_pr_sustraendo').disabled='';
									}
									dialog.hideAndUnload();
								//	url='modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value;
									//setBarraEstado(url);
									jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&id_cheques='+getObj('id_cheques').value,page:1}).trigger("reloadGrid");
					 			url='modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+nd+'&proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&id_cheques='+getObj('id_cheques').value;
							//	alert(url);
								},
								loadComplete:function (id){
									setBarraEstado("");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////filtrar de carga de cheques/////////////////////////////////////////////////////////
	var nd=new Date().getTime();
	setBarraEstado("<img align='absmiddle' src='imagenes/loading.gif' /> Esperando Respuesta del Servidor...");
		$.ajax ({
		    url:"modulos/tesoreria/cheques/pr/grid_cheques_banco2.php",
			data:"id_grid=list_grid_"+nd+"&id_pager=pager_grid_"+nd,
			type:'POST', 
			cache: false,
			success: function(data)
			{
				dialog=new Boxy(data,{title: 'Consulta Emergente De PreCheques', modal: true,center:false,x:0,y:0,show:false});
				dialog_reload=function gridReload(){ 
					 var busq_cheques= jQuery("#tesoreria_cheques_busqueda_cheques").val(); 
					 var proveedor= jQuery("#tesoreria_cheques_pr_proveedor_id").val(); 
					 jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/cmb.sql.precheque.php?busq_cheques="+busq_cheques+"&proveedor="+proveedor,page:1}).trigger("reloadGrid"); 
	 		}	
				crear_grid();
				var timeoutHnd; 
				var flAuto = true;
				$("#tesoreria_cheques_busqueda_cheques").keypress(function(key)
				{
					...
					function consulta_doc_dosearch()
					{
						...
						function consulta_doc_gridReload()
						{
								...
								var proveedor= jQuery("#tesoreria_cheques_pr_proveedor_id").val(); 
								 jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/cmb.sql.precheque.php?busq_cheques="+busq_cheques+"&proveedor="+proveedor,page:1}).trigger("reloadGrid"); 
								 url="modulos/tesoreria/cheques/pr/cmb.sql.precheque.php?busq_cheques="+busq_cheques+"&proveedor="+proveedor;
								//alert(url);
								// setBarraEstado(url);
						}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////						
						function crear_grid()
						{
							...
							({
								...
								url:'modulos/tesoreria/cheques/pr/cmb.sql.precheque.php?nd='+nd+"&proveedor="+getObj('tesoreria_cheques_pr_proveedor_id').value,
								datatype: "json",
								colNames:['Id','N precheque','Id Banco','Banco','N Cuenta','id_proveedor','codigo_proveedor','nombre_proveedor2','nombre_proveedor','Base Imp','Concepto','porcentaje','ordenes','islr','sustraendo','Benef','fecha'],
								colModel:[
									{name:'id',index:'id', width:50,sortable:false,resizable:false,hidden:true},
									...
									{name:'benef',index:'benef', width:100,sortable:false,resizable:false,hidden:true},
								...
								var ret = jQuery("#list_grid_"+nd).getRowData(id);
									...
									getObj('id_cheques').value=ret.id;
									...
									if(ret.benef!="")
									{
										...
										getObj('tr_empleado323').style.display="";
										getObj('tesoreria_cheque_db_otro_beneficiario_oc').value=1;
										getObj('tesoreria_cheque_db_otro_beneficiario').checked="checked";
										}
							...
							/*	getObj('tesoreria_cheques_db_nombre_banco').disabled="disabled";
								...
								getObj('tesoreria_cheque_db_btn_imprimir_precheque').style.display='';
								...
								getObj('tesoreria_cheques_pr_sustraendo_oculto').value=ret.sustraendo;
								if(getObj('tesoreria_cheques_pr_sustraendo_oculto').value=='1')
								{
									getObj('tesoreria_cheques_pr_sustraendo').checked='checked';
								}else
								if(getObj('tesoreria_cheques_pr_sustraendo_oculto').value=='0')
								{
									getObj('tesoreria_cheques_pr_sustraendo').checked='';
								}
					 ...
					 {//alert(html);
						...
						{		var recordset=html;
						 if(recordset)
								{
									recordset = recordset.split("*");
									getObj('id_cheques').value=recordset[0];
								...
								*/	
								...
								/*	getObj('tesoreria_cheques_db_nombre_banco').disabled="disabled";
									...
									getObj('tesoreria_cheques_pr_sustraendo_oculto').value=recordset[13];
									if(getObj('tesoreria_cheques_pr_sustraendo_oculto').value=='1')
									{
										getObj('tesoreria_cheques_pr_sustraendo').checked='checked';
									}else
									if(getObj('tesoreria_cheques_pr_sustraendo_oculto').value=='0')
									{
										...
										jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&precheque='+getObj('tesoreria_cheques_db_n_precheque').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&sustraendo='+getObj('tesoreria_cheques_pr_sustraendo_oculto').value+'&id_cheques='+getObj('id_cheques').value,page:1}).trigger("reloadGrid");
								 ...
								 else
								 {
									...
									jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&precheque='+getObj('tesoreria_cheques_db_n_precheque').value+'&id_cheques='+getObj('id_cheques').value,page:1}).trigger("reloadGrid");
									document.form_tesoreria_db_cheque.tesoreria_cheques_db_itf.checked="";
...
$("#list_orden_pago").jqGrid({
	...
	url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?nd='+new Date().getTime()+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value,
//	+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value
	datatype: "json",
		colNames:['&ordm;Id','Orden','Fecha','SubTotal','Base Imp.','%IVA','%Ret.IVA','Total.IVA','%ISLR','Total.ISLR','RetExtras','Total.Pagar','Saldo Orden'],
   	colModel:[
			...
			{name:'ret',index:'ret1',width:60},
			{name:'monto',index:'monto',width:55,hidden:true},
			{name:'saldo',index:'saldo',width:55}
   	],
   	rowNum:10,
   	rowList:[10,20,30],
   	imgpath: 'utilidades/jqgrid_demo/themes/basic/images',
	...
	if(getObj('tesoreria_cheques_db_monto_pro').value!=1)
	{
	...
	//	alert(s);
		...
		$.ajax({
			url:"modulos/tesoreria/cheques/pr/sql.consulta_selec.php?id="+idd+"&vector="+s+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&sustraendo='+getObj('tesoreria_cheques_pr_sustraendo_oculto').value+'&id_cheques='+getObj('id_cheques').value,
		    data:dataForm('form_tesoreria_db_cheque'), 
			type:'GET',
			cache: false,
			 success:function(html)
			...
			//	valor=recordset;
				...
				monto_total=recordset[0];
				monto_p=recordset[1];
				saldo=recordset[2];
				monto_p2=recordset[3];
				counter=recordset[4];
				prorateo=recordset[5];
		...
		/*		if(monto_p=='0,00')
				{
					alert('no se pueden relacionar las  ordenes');						
				}*/
				/*monto_total = monto_total.currency(2,',','.');
				monto_p = monto_p.currency(2,',','.');
				saldo = saldo.currency(2,',','.');
				monto_p2 = monto_p2.currency(2,',','.');*/	
			    getObj('tesoreria_cheques_db_monto_pagar').value=monto_p;
				getObj('tesoreria_cheques_db_monto_total').value=monto_total;
				getObj('tesoreria_cheques_db_monto_saldo').value=saldo;
				getObj('tesoreria_cheques_db_monto_cheque').value=monto_p2;
				getObj('tesoreria_cheques_db_monto_pro').value=prorateo;
				if(getObj('tesoreria_cheques_db_monto_pagar').value=='0,00')
				{
						getObj('tesoreria_cheques_pr_sustraendo').checked='';
	        			getObj('tesoreria_cheques_pr_sustraendo_oculto').value='0';	
				}
	...
	}//fin de if prorateo!=1
	else
	setBarraEstado("<div id='mensaje'><p><img align='absmiddle' src='imagenes/iconos/folder_important.png />NO SE DEBEN MEZCLAR ORDENES PRORATEADAS CON ORDENES COMPLETAS</p></div>",true,true);
	},
...
if(getObj('tesoreria_cheques_db_monto_pro').value!=1)
	{
				...
				idd = ret.id;
		//	alert(idd);
			url="modulos/tesoreria/cheques/pr/sql.consulta_selec.php?id="+idd+"&vector="+s+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&sustraendo='+getObj('tesoreria_cheques_pr_sustraendo_oculto').value+'&id_cheques='+getObj('id_cheques').value;
			//alert(url);		
				if(id && id!==lastsel)
				{
	...
	setBarraEstado("<div id='mensaje'><p><img align='absmiddle' src='imagenes/iconos/folder_important.png />NO SE DEBEN MEZCLAR ORDENES PRORATEADAS CON ORDENES COMPLETAS</p></div>",true,true);
	//alert("no puede mezclar ordenes prorateadas con completas");
		}
	...
	$.ajax({
			...
			cache: false,
			 success:function(html)
			 {
	            ...
	            jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+recordset[0]+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&id_cheques='+getObj('id_cheques').value,page:1}).trigger("reloadGrid");
...
{
//alert("el pasante");
//url='modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&precheque='+getObj('tesoreria_cheques_db_n_precheque').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&ordenes='+getObj('tesoreria_cheques_db_ordenes_pago').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value
	jQuery("#list_orden_pago").setGridParam({url:'modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&precheque='+getObj('tesoreria_cheques_db_n_precheque').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&ordenes='+getObj('tesoreria_cheques_db_ordenes_pago').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&sustraendo='+getObj('tesoreria_cheques_pr_sustraendo_oculto').value+'&id_cheques='+getObj('id_cheques').value,page:1}).trigger("reloadGrid");	
	url='modulos/tesoreria/cheques/pr/cmb.sql.orden_pago.php?proveedor='+getObj('tesoreria_cheques_pr_proveedor_id').value+'&ncuenta='+getObj('tesoreria_cheques_db_n_cuenta').value+'&banco='+getObj('tesoreria_cheques_db_banco_id_banco').value+'&precheque='+getObj('tesoreria_cheques_db_n_precheque').value+'&islr='+getObj('tesoreria_cheques_pr_ret_islr').value+'&ordenes='+getObj('tesoreria_cheques_db_ordenes_pago').value+'&rif='+getObj('tesoreria_cheques_pr_proveedor_rif').value+'&sustraendo='+getObj('tesoreria_cheques_pr_sustraendo_oculto').value+'&id_cheques='+getObj('id_cheques').value;
	//alert(url);
}	
...
$('#tesoreria_cheques_pr_ret_islr').numeric({});
...
$("#tesoreria_cheques_pr_sustraendo").click(function(){
		if(getObj('tesoreria_cheques_pr_sustraendo_oculto').value=="0")
			getObj('tesoreria_cheques_pr_sustraendo_oculto').value="1"
		else
		if(getObj('tesoreria_cheques_pr_sustraendo_oculto').value=="1")
			getObj('tesoreria_cheques_pr_sustraendo_oculto').value="0"
...
$('#tesoreria_cheques_pr_proveedor_codigo').change(consulta_automatica_proveedor);
   ...
   <div id="botonera"><img id="tesoreria_cheque_db_btn_cancelar" class="btn_cancelar"src="imagenes/null.gif"  /><img id="tesoreria_cheque_db_btn_eliminar" class="btn_eliminar"src="imagenes/null.gif" style="display:none"/>
	...
	<img id="tesoreria_cheque_db_btn_imprimir"  class="btn_imprimir" src="imagenes/null.gif"  style="display:none" />
	<img id="tesoreria_cheque_db_btn_imprimir_precheque"  src="imagenes/iconos/vistapreviaprecheque.png" style="display:none" /></div>
...
<input type="hidden"  id="id_cheques" name="id_cheques"/>
...
<input type="hidden" name="orden_pago_pr_cot_select" id="orden_pago_pr_cot_select"  />
  <table   class="cuerpo_formulario">
   ...
   <tr>
		...
		<ul class="input_con_emergente">
		<li>
				<input name="tesoreria_cheques_db_n_cuenta" type="text" id="tesoreria_cheques_db_n_cuenta"   value="" size="50" maxlength="20" message="Introduzca el N&uacute;mero de cuenta. " readonly=""
		...
		<li id="tesoreria_cheques_db_btn_consultar_cuentas_chequeras" class="btn_consulta_emergente">
	...
	<tr>
		<th>Proveedor:</th>
		  <td>
				...
				<input type="hidden" name="tesoreria_cheques_pr_proveedor_rif" id="tesoreria_cheques_pr_proveedor_rif" readonly />
				</li> 
	...
	<tr>
    		<th>Beneficiario:</th>
	<td><label>
	    <input type="checkbox" name="tesoreria_cheque_db_otro_beneficiario" id="tesoreria_cheque_db_otro_beneficiario" value="checkbox" onclick="incluir_beneficiario();" />
		<input type="hidden" id="tesoreria_cheque_db_otro_beneficiario_oc" name="tesoreria_cheque_db_otro_beneficiario_oc"  value="0" />
	 <b> Incluir Beneficiario</b> </label></td>
    </tr>
    <tr id="tr_empleado323" style="display:none">
    	<th>Nom Benef.</th>
        <td><input type="text" name="tesoreria_cheque_db_nombre_benef" id="tesoreria_cheque_db_nombre_benef" maxlength="60" size="60"></td>
    </tr>
    <tr>
    	<th>
			Fecha:
		...
		<td width="124">
		            <input alt="date" type="text" name="tesoreria_cheque_pr_fecha" id="tesoreria_cheque_pr_fecha" size="7" value="<?  $year=date("Y"); echo date("d/m")."/".$year;?>" jval="{valid:/^[0-9/-]{1,60}$/, message:'Fecha Invalida', styleType:'cover'}" onchange="v_fecha();"
					jvalkey="{valid:/[0-9/-]/, cFunc:'alert', cArgs:['Fecha Impuesto: '+$(this).val()]}"/>
                    <input type="hidden"  name="tesoreria_cheque_pr_fecha_oculto" id="tesoreria_cheque_pr_fecha_oculto" value="<? $year=date("Y"); echo date("d/m")."/".$year;?>"/>
				  <button type="reset" id="tesoreria_cheque_pr_fecha_boton_d">...</button>
				  <script type="text/javascript">
							Calendar.setup({
								inputField     :    "tesoreria_cheque_pr_fecha",      // id of the input field
								ifFormat       :    "%d/%m/%Y",       // format of the input field
								showsTime      :    false,            // will display a time selector
								button         :    "tesoreria_cheque_pr_fecha_boton_d",   // trigger for the calendar (button ID)
								singleClick    :    true,          // double-click mode
								onUpdate :function(date){
										f1=new Date( getObj("tesoreria_cheque_pr_fecha").value.MMDDAAAA() );
   ...
   <tr>
	...
	<tr>
		<th>Concepto:</th>
	    <td>
	<textarea name="tesoreria_cheques_db_concepto" cols="90" rows="2"  id="tesoreria_cheques_db_concepto" 
	...
	<tr>
		<th>
			Monto a Pagar		</th>
		<td>
		<input align="right"  name="tesoreria_cheques_db_monto_pagar" type="text" id="tesoreria_cheques_db_monto_pagar"   value="0,00" size="16" maxlength="16"  style="text-align:right"  onKeyPress="reais(this,event)" onKeyDown="backspace(this,event)" />
		<input  name="tesoreria_cheques_db_ordenes_pago" type="hidden" id="tesoreria_cheques_db_ordenes_pago" size="16" maxlength="16" />		
		<input align="right"  name="tesoreria_cheques_db_monto_total" type="hidden" id="tesoreria_cheques_db_monto_total"  readonly value="0,00" size="16" maxlength="16"  style="text-align:right" />
		<input align="right"  name="tesoreria_cheques_db_monto_saldo" type="hidden" id="tesoreria_cheques_db_monto_saldo"  readonly value="0,00" size="16" maxlength="16"  style="text-align:right" />
		<input align="right"  name="tesoreria_cheques_db_monto_cheque" type="hidden" id="tesoreria_cheques_db_monto_cheque"  readonly value="0,00" size="16" maxlength="16"  style="text-align:right" />
		<input align="right"  name="tesoreria_cheques_db_monto_pro" type="hidden" id="tesoreria_cheques_db_monto_pro"  readonly value="0" size="16" maxlength="16"  style="text-align:right" />
		</td>
	</tr>
	<tr>
	...
	<th>% Ret Islr</th>
	  <td>
			<input name="tesoreria_cheques_pr_ret_islr" type="text"  id="tesoreria_cheques_pr_ret_islr"  onblur=""   size="16" maxlength="10" value="0,00"  style="text-align:right" onKeyPress="reais(this,event)" onKeyDown="backspace(this,event)"
			jVal="{valid:/^[0123456789 .,]{1,10}$/, message:'N&uacute;mero  Invalido', styleType:'cover'}"
			jValKey="{valid:/[0123456789 .,]/, cFunc:'alert', cArgs:['N&uacute;mero: '+$(this).val()]}"/>
				
			%	
<img class="btn_impuesto" name="tesoreria_cheques_pr_calcular_impuesto" id="tesoreria_cheques_pr_calcular_impuesto"  src="imagenes/null.gif"/>


	 Aplicar Impuesto
	<input name="tesoreria_cheques_pr_sustraendo" id="tesoreria_cheques_pr_sustraendo" type="checkbox" value=""  disabled="disabled"/>
	Aplicar Sustraendo 
	<input type="hidden" name="tesoreria_cheques_pr_sustraendo_oculto" id="tesoreria_cheques_pr_sustraendo_oculto"  value="0"/>
