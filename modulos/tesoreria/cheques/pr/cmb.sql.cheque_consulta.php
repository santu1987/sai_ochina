...
if(isset($_GET['busq_cheques']))
{
	...
	if($busq_cheques!="")
	{
		$where.="and numero_cheque::VARCHAR like '%$busq_cheques%'";
	}
	//die($busq_cheques);
}
...
if(isset($_GET['busq_benef']))
{
	...
	if($busq_benef!='')
	$where.= " AND  (upper(cheques.benef_nom) like '%$busq_benef%')";
}
$Sql="
				SELECT 
					count(numero_cheque)
...
if(($row->fields("id_proveedor")!="")and($row->fields("id_proveedor")!="0"))
{
