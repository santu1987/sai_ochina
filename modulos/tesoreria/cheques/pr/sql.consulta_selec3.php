			...
			$orden=$id_detalles[$i];
				if($orden!='')
				{
					...
					if($contador==1)
					{
						...
						//die($saldo);
						////////////////////////////
						/*$sql_cheque="SELECT SUM(monto_cheque) as monto_suma from cheques where ordenes[1]='$orden'";
						$row_cheque=& $conn->Execute($sql_cheque);
						if(!$row_cheque->EOF) 
						{
								$monto_sumados=$row_cheque->fields("monto_suma");
						}*/
...
echo number_format($pagar,2,',','.')."*".number_format($monto_pagado_cheque,2,',','.')."*".number_format($saldo,2,',','.')."*".number_format($monto_pagado_cheque2,2,',','.')."*".$contador;
  ?>