...
{*/
						...
						//-------------------- verificando si hay firmads activas
						$Sql_firmas="
									SELECT 
						...
						//------ verificando si la cuenta y el banco tienen chequera creada
								$opcion='$_POST[tesoreria_cheque_manual_pr_op_oculto_orden]';		
								if($opcion=='1')
								{
								
									$where="AND 
											id_proveedor='$_POST[tesoreria_cheques_manual_orden_pr_proveedor_id]'";
								}
								else
								if($opcion=='2')
								{
									...
									$where="AND 
											cheques.cedula_rif_beneficiario='$_POST[tesoreria_cheque_manual_orden_pr_empleado_codigo]'";
								}
								$sql_cheque = "SELECT 
											id_cheques
									   FROM 
						...
						$sql_saldo_actual = "SELECT 
							...
							{		
										...
										$monto_cheque=$_POST[tesoreria_cheques_manual_orden_db_monto_pagar];
										...
										$saldo_total=($saldo_actual)-($monto_cheque);
											...
											else
											{
												...
												WHERE cuenta_banco='$_POST[tesoreria_cheques_manual_orden_db_n_cuenta]'
												AND
						...
						//------------------- VERIFICANDO SI LAS ORDENES DE PAGO NO FUERON CANCELADAS POR OTROS CHEQUES//----------------------------//
										...
										while($i < $contador)
										{
												$sql_orden="SELECT * 
															...
															AND
																(orden_pago.cheque='$_POST[tesoreria_cheques_manual_orden_db_n_precheque]')
															";
			...
			/****************************/
												...
												if($tipo=='1')
												{
												$where2="AND id_proyecto = '$accion_central'"; 
												}else
												$where2="AND id_accion_centralizada ='$accion_central'"; 
...
////																	
							...
							$i++;							
									 ...
									 $fact_ord1=str_replace(".","",$_POST[tesoreria_cheques_manual_orden_db_monto_pagar]);
						...
						/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
							...
							if($n_cheque!="")
							{
								...
								if (!$conn->Execute($sql_cheques)) 
										die ('Error_impresion'.$sql_cheques );
								...
								$sql_orden=" UPDATE orden_pago
										...
										AND	
											cuenta_banco='$_POST[tesoreria_cheques_manual_orden_db_n_cuenta]'
										AND
											cheque='$_POST[tesoreria_cheques_manual_orden_db_n_precheque]'
										";	
								//die($sql_orden);					
								if (!$conn->Execute($sql_orden)) 
									die ('Error_impresion'.$sql_orden );
								...
								if (!$conn->Execute($sql_ultimo_emitido2))
									die ('Error_impresion'.$sql_ultimo_emitido2 );
