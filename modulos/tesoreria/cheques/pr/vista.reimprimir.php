...
monto=getObj('tesoreria_cheque_reimpresion_pr_monto_pagar').value;
	...
	/*if(confirm("¿Desea reimprimir el cheque nº "+ncheque+" del beneficiario "+beneficiario+" por el siguiente monto: "+monto+"?")) 
	{	*/
...
function(val)
 {
	if(val=="ACEPTAR")
	{
					...
					//alert(getObj('tesoreria_cheques_reimpresion_pr_mismo_numero_oculto').value);
					//limpiar_reimprimir();		
	}
})
});
...
//alert(html);
							if((html!='Error-REIMPRIMIR' )&&(html!='chequera_agotada')&&(html!='ERROR-EN ORDEN')&&(html!='ERROR-P:MODIFICAR-CHEQUERA')&&(html!='Error-act cheque'))
							{
								...
								if(recordset[2]==1)
								{
									url="pdf.php?p=modulos/tesoreria/cheques/rp/vista.lst.cheque.php¿id_banco="+getObj('tesoreria_cheque_reimpresion_pr_banco_id_nuevo').value+"@ncheque="+recordset[0]+"@ncuenta="+getObj('tesoreria_cheque_reimpresion_pr_n_cuenta_nuevo').value+"@ordenes="+getObj('tesoreria_cheque_reimpresion_pr_ordenes').value+"@secuencia="+recordset[1]+"@caducidad="+getObj('tesoreria_cheques_reimpresion_pr_caducidad').value+"@endosable="+getObj('tesoreria_cheques_reimpresion_pr_endosable_oculto').value+"@proyecto="+recordset[5]+"@partida="+recordset[6]+"@ejecutora="+recordset[4];
									//alert(url);
								}else
								if(recordset[2]==2)
								{
									url="pdf.php?p=modulos/tesoreria/cheques/rp/vista.lst.cheque_manual.php¿id_banco="+getObj('tesoreria_cheque_reimpresion_pr_banco_id_nuevo').value+"@ncheque="+recordset[0]+"@ncuenta="+getObj('tesoreria_cheque_reimpresion_pr_n_cuenta_nuevo').value+"@ordenes="+getObj('tesoreria_cheque_reimpresion_pr_ordenes').value+"@secuencia="+recordset[1]+"@caducidad="+getObj('tesoreria_cheques_reimpresion_pr_caducidad').value+"@endosable="+getObj('tesoreria_cheques_reimpresion_pr_endosable_oculto').value+"@opcion="+recordset[3]+"@proyecto="+recordset[5]+"@partida="+recordset[6]+"@ejecutora="+recordset[4];
								//	alert(url);
								}
		...
		$.ajax (
					{
						...
						{//alert(html);
							if((html!='Error-REIMPRIMIR' )&&(html!='chequera_agotada')&&(html!='ERROR-EN ORDEN')&&(html!='ERROR-P:MODIFICAR-CHEQUERA')&&(html!='Error-act cheque')&&(html!='no_disponible_saldo')&&(html!='integrado'))
							{
								...
								modificar_inactivo_reimpreso();
								Boxy.ask("<iframe style='width:0px; height:0px; border:0px' src="+url+" ></iframe><div id='mensaje'><p><img align='absmiddle' src='imagenes/iconos/ajax-loader2.gif />REALIZANDO IMPRESI&Oacute;N:porfavor presione el boton cerrar de esta ventana luego que haya culminado la impresión</p></div>", ["CERRAR"], 
								function(val)
								 {
								 if(val=="CERRAR")
									{
		...
		$.ajax (
					{
						...
						success: function(html)
						{
							...
							if((html!='Error-REIMPRIMIR' )&&(html!='chequera_agotada')&&(html!='ERROR-EN ORDEN')&&(html!='ERROR-P:MODIFICAR-CHEQUERA')&&(html!='Error-act cheque'))
							{
								...
								//setBarraEstado(url);
								Boxy.ask("<iframe style='width:0px; height:0px; border:0px' src="+url+" ></iframe><div id='mensaje'><p><img align='absmiddle' src='imagenes/iconos/ajax-loader2.gif />REALIZANDO IMPRESIÓN:porfavor presione el boton cerrar de esta ventana luego que haya culminado la impresión</p></div>", ["CERRAR"], 
...
setBarraEstado("");
	...
	getObj('tesoreria_cheque_reimpresion_pr_concepto').value="";
		...
		$.ajax ({
			...
			success: function(data)
			{
				...
				dialog_reload=function gridReload(){ 
					...
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/sql_grid_cuentas_anular.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheque_reimpresion_pr_banco_id_banco').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid"); 
				}	
				...
				{	ano_dosearch();
					...
					function ano_gridReload()
					{
							...
							var busq_cuenta= jQuery("#tesoreria_cheques_chequeras_pr_cuenta").val(); 
jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/sql_grid_cuentas_anular.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheque_reimpresion_pr_banco_id_banco').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid");
						url="modulos/tesoreria/cheques/pr/sql_grid_cuentas_anular.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheque_reimpresion_pr_banco_id_banco').value+'&busq_cuenta='+busq_cuenta;
						//alert(url);
						}
		...
		$.ajax ({
			...
			success: function(data)
			{
				...
				dialog_reload=function gridReload(){ 
					...
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/sql_grid_cuentas.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheque_reimpresion_pr_banco_id_nuevo').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid"); 
				}	
				...
				{	ano_dosearch();
					...
					function ano_gridReload()
					{
							...
							var busq_cuenta= jQuery("#tesoreria_cheques_chequeras_pr_cuenta").val(); 
jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/sql_grid_cuentas.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheque_reimpresion_pr_banco_id_nuevo').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid");
						url="modulos/tesoreria/cheques/pr/sql_grid_cuentas.php?busq_ano="+busq_ano+'&banco='+getObj('tesoreria_cheque_reimpresion_pr_banco_id_nuevo').value+'&busq_cuenta='+busq_cuenta;
						//alert(url);
						}
...
{name:'concepto',index:'concepto', width:100,sortable:false,resizable:false,hidden:true}
								...
								var ret = jQuery("#list_grid_"+nd).getRowData(id);
									...
									getObj('tesoreria_cheque_reimpresion_pr_concepto').value=ret.concepto;
									dialog.hideAndUnload();
...
$('#tesoreria_cheque_reimpresion_pr_n_cheque').blur(consulta_automatica_cheque_codigo);
   ...
   <img id="tesoreria_cheque_db_btn_imprimir_vp_reimpresion"  class="btn_imprimir_vista_previa" src="imagenes/null.gif"  style="display:none" /></div>
<form method="post" id="form_tesoreria_pr_cheque_reimprimir" name="form_tesoreria_pr_cheque_reimprimir">
  <table   class="cuerpo_formulario">
   ...
   <tr>
		...
		<ul class="input_con_emergente">
		<li>
				<input name="tesoreria_cheque_reimpresion_pr_n_cuenta" type="text" id="tesoreria_cheque_reimpresion_pr_n_cuenta"   value="" size="50" maxlength="20" message="Introduzca el Número de cuenta. " readonly=""/>
		</li>
	...
	<tr>
		 <th>N&ordm; Cheque:</th> 
		  ...
		  <td>
		  		<ul class="input_con_emergente">
				<li>
				  <input name="tesoreria_cheque_reimpresion_pr_n_cheque" type="text" id="tesoreria_cheque_reimpresion_pr_n_cheque"  value="000000"   size="6"    maxlength="6"  message="Introduzca el Número de cheque " alt="signed-dec"  />
 ...
 <th>Banco:</th>
   ...
   <tr>
		...
		<ul class="input_con_emergente">
		<li>
				<input name="tesoreria_cheque_reimpresion_pr_n_cuenta_nuevo" type="text" id="tesoreria_cheque_reimpresion_pr_n_cuenta_nuevo"   value="" size="50" maxlength="20" message="Introduzca el Número de cuenta. " readonly=""/>
		</li>
   ...
   <tr>
    ...
    <tr>
		<th>Concepto:</th>
	    <td>
	<textarea name="tesoreria_cheque_reimpresion_pr_concepto" cols="90" rows="2"  id="tesoreria_cheque_reimpresion_pr_concepto" 
