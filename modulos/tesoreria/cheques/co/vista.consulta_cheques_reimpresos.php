...
({ 
	...
	url:'modulos/tesoreria/cheques/co/sql.consulta_cheques_reimpresos.php?nd='+new Date().getTime()+"&tesoreria_busqueda_ano_reimp="+getObj('tesoreria_busqueda_ano_reimp').value,
	datatype: "json",
   	colNames:['Usuario','Beneficiario','Ch Emitidos','Id_Banco','Banco','Cuenta','Ch Reimp','Id_Banco_reimpreso','Banco Reimpreso','Cuenta Reimpreso','Chequera','Ordenes','Monto','tipo'],
   	colModel:[
	...
	onSelectRow:function(id){			
					var ret=jQuery("#list_reimprimir_cheques").getRowData(id);
...
{ 
	  ...
	  var tesoreria_busqueda_ano_reimp=jQuery("#tesoreria_busqueda_ano_reimp").val(); 



 jQuery("#list_reimprimir_cheques").setGridParam({url:"modulos/tesoreria/cheques/co/sql.consulta_cheques_reimpresos.php?usu_chereimp="+usu_chereimp+"&banco_chereimp="+banco_chereimp+"&cuenta_chereimp="+cuenta_chereimp+"&prov_chereimp="+prov_chereimp+"&tesoreria_busqueda_ano_reimp="+tesoreria_busqueda_ano_reimp,page:1}).trigger("reloadGrid"); 
...
$('#usu_chereimp').numeric({});
$('#banco_chereimp').alpha({allow:' ·ÈÌÛ˙ƒ…Õ”⁄-'});
$('#prov_chereimp').alpha({allow:' ·ÈÌÛ˙ƒ…Õ”⁄'});
$('#cuenta_chereimp').numeric({allow:'-'});
$('#tesoreria_busqueda_proveedor_reim').alpha({allow:' ·ÈÌÛ˙ƒ…Õ”⁄.- '});
$('#tesoreria_busqueda_beneficiario_reim').alpha({allow:' ·ÈÌÛ˙ƒ…Õ”⁄.- '});
...
<table class="cuerpo_formulario">
	...
	<tr>	
		<td class="celda_consulta">
		 ...
		 <input type="text"  id="tesoreria_busqueda_ano_reimp" size="10" name="tesoreria_busqueda_ano_reimp" value="<? echo(date("Y")); ?>" onkeydown="reimprimir_cheque_doSearch(arguments[0]||event)"/>
		 </div>
