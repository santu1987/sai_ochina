...
if(isset($_GET['n_cheques']))
{
	$n_cheque=strtoupper($_GET['n_cheques']);
	if($n_cheque!="")
	$sql_where.= " AND  (numero_cheque = '$n_cheque')";
}
...
if(isset($_GET['tesoreria_busqueda_ano']))
{
	$busq_ano=$_GET['tesoreria_busqueda_ano'];
	if($busq_ano!="")
	{
		$sql_where.="AND substr(cheques.fecha_cheque::varchar,1,4)='$busq_ano'";
	}else
	$sql_where.="AND substr(cheques.fecha_cheque::varchar,1,4)='$busq_ayo'";
}else
$sql_where.="AND substr(cheques.fecha_cheque::varchar,1,4)='$busq_ayo'";
//
...
while (!$row->EOF) 
{
	...
	$len_estado=strlen($estado);
	$estado3=substr($estado,1,($len_estado-2));	
	$estado2 = split(",",$estado3);															
	if ($estado2[3]!="0")
		$edo="Archivo";
	else
	if ($estado2[2]!="0")
