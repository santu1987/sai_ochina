...
$Sql="
		SELECT  DISTINCT
			...
			$where
			ORDER BY
				cheques.cuenta_banco,cheques.numero_cheque desc
";
...
{ 
	...
	class PDF extends FPDF
	{
		...
		{	global $banco;	
			...
			$this->Cell(0,10,'RELACIÓN BANCOS PRECHEQUES EMITIDOS',0,0,'C');
            $this->ln();
			...
			$this->Cell(50,6,		     'USUARIO',			0,0,'C',1);
			$this->Ln(6);
	...
	//-------------------------------------------------------------------------------------------------------------------------
			...
			if($row->fields("tipo_cheque")=='1')
			{
					...
					$pdf->Cell(50,6,strtoupper($nombre_usuario),	0,0,'C',1);
					$pdf->Ln();
