...
{ 
	...
	while (!$row->EOF) 
	{
		...
		$total_general=$total_general+$row->fields("monto_cheque");	
			...
			$pdf->Cell(30,6,	         strtoupper(substr($banco_reimpreso,0,18)),0,0,'L',0);
			...
			$pdf->Cell(20,6,		     number_format($monto,2,',','.'),	0,0,'R',0);
		...
		//	$proveedor_ant=strtoupper($row->fields("proveedor"));
			...
			else
			{
				...
				$total_cuenta=$total_cuenta+$monto;
				$pdf->Cell(280,6,"Total Cuenta:".number_format($total_cuenta,2,',','.'),0,1,'R',1);
				$total_cuenta=0;		
	...
	if($banco_ant==$banco_sig)
	...
	else
		{
			...
			$total_banco=$total_banco+$monto_cheques2;
			$pdf->Cell(280,6,"Total Banco:".number_format($total_banco,2,',','.'),0,1,'R',1);
			//$pdf->Ln();
			$coordenada++;
			$total_cuenta=0;
			$total_banco=0;
