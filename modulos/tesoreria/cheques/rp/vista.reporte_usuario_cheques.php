...
if(($('#form_rp_cheques_usuarios').jVal()))
	...
	//usuarios solos												
				...
				/// ----------------------------consultas de reportes combinados:
						...
						// beneficiario
							if((getObj('tesoreria_cheque_manual_rp_empleado_nombre').value!="")&&(getObj('tesoreria_cheques_usuarios_rp_id_usuario').value=="")&&(getObj('tesoreria_cheques_banco_rp_id_banco').value=="")&&(getObj('tesoreria_cheques_banco_rp_n_cuenta').value==""))
									{
										url="pdf.php?p=modulos/tesoreria/cheques/rp/vista.lst.beneficiario_cheques.php¿rif="+getObj('tesoreria_cheque_manual_rp_empleado_nombre').value+"@desde="+getObj('tesoreria_cheques_usuarios_rp_fecha_desde').value+"@hasta="+getObj('tesoreria_cheques_usuarios_rp_fecha_hasta').value+"@tipo="+getObj('tesoreria_cheques_proveedor_db_btn_consultar_tipo_cheque_cmb').value+"@eva_opcion="+opciones;
		...
		$.ajax ({
			...
			success: function(data)
			{
				...
				dialog_reload=function gridReload(){ 
					var busq_usuario= jQuery("#tesoreria-reportes-busq_nombre_usuario_banco").val(); 
					var busq_nombre= jQuery("#tesoreria-reportes-busq_nombre_usuario_banco2").val();
		...
		$.ajax ({
			...
			success: function(data)
			{
				...
				dialog_reload=function gridReload(){ 
					...
					var busq_codigo=jQuery("#tesoreria_rp_proveedor_consulta_cod ").val();
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/rp/cmb.sql.proveedor.php?busq_proveedor="+busq_proveedor+"&busq_codigo="+busq_codigo,page:1}).trigger("reloadGrid"); 
			}	
				crear_grid();
				var timeoutHnd; 
				var flAuto = true;
				
				$("#tesoreria_rp_proveedor_consulta_cod").keypress(function(key)
				{
				...
				function consulta_doc_dosearch()
					{
						...
						function consulta_doc_gridReload()
						{
							...
							var busq_codigo=jQuery("#tesoreria_rp_proveedor_consulta_cod ").val();
							jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/rp/cmb.sql.proveedor.php?busq_proveedor="+busq_proveedor+"&busq_codigo="+busq_codigo,page:1}).trigger("reloadGrid"); 
							url="modulos/tesoreria/cheques/rp/cmb.sql.proveedor.php?busq_proveedor="+busq_proveedor+"&busq_codigo="+busq_codigo;
						//	setBarraEstado(url);
						}
...
var nd=new Date().getTime();
		setBarraEstado("<img align='absmiddle' src='imagenes/loading.gif' /> Esperando Respuesta del Servidor...");
		$.ajax ({
		    url:"modulos/tesoreria/banco_cuentas/rp/sql_grid_cuentas_chequeras.php",
			data:"id_grid=list_grid_"+nd+"&id_pager=pager_grid_"+nd,
			type:'POST', 
			cache: false,
			success: function(data)
			{
				...
				dialog_reload=function gridReload(){ 
					var busq_cuenta= jQuery("#tesoreria_banco_cuentas_rp_cuenta").val(); 
 
					jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/sql_grid_cuenta_cheque.php?banco="+getObj('tesoreria_cheques_banco_rp_id_banco').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid"); 
				}	
				...
				$("#tesoreria_banco_cuentas_rp_cuenta").keypress(function(key)
				{	ano_dosearch();
				});
					...
					function ano_gridReload()
					{
							var busq_cuenta= jQuery("#tesoreria_banco_cuentas_rp_cuenta").val(); 
jQuery("#list_grid_"+nd).setGridParam({url:"modulos/tesoreria/cheques/pr/sql_grid_cuenta_cheque.php?banco="+getObj('tesoreria_cheques_banco_rp_id_banco').value+'&busq_cuenta='+busq_cuenta,page:1}).trigger("reloadGrid");
						    url="modulos/tesoreria/cheques/pr/sql_grid_cuenta_cheque.php?banco="+getObj('tesoreria_cheques_banco_rp_id_banco').value+'&busq_cuenta='+busq_cuenta;
						  //alert(url);
						}
				}
		});			
...
<form method="post" name="form_rp_cheques_usuarios" id="form_rp_cheques_usuarios">
  <table  class="cuerpo_formulario"  style="width:700">
		<tr>
		<th  class="titulo_frame" colspan="3"> <img src="imagenes/iconos/desktop24x24.png" style="padding-right:5px;" align="absmiddle" />Relaci&oacute;n Cheques Emitidos s </th>
	   </tr>
	   <tr>
...
<tr>
	 ...
	 <table class="clear" width="100%" border="0" >
			...
			<tr id="tr_proveedor_reporte_cheque">
	 <tr id="tr_empleado_reporte_cheque" >
      <th>Beneficiario :</th>
      <td >		<!--<ul class="input_con_emergente">
	  <li>-->
      <input type="hidden"  name="tesoreria_cheques_manual_rp_empleado_codigo"  id="tesoreria_cheques_manual_rp_empleado_codigo" />
	    <input name="tesoreria_cheque_manual_rp_empleado_nombre" type="text" id="tesoreria_cheque_manual_rp_empleado_nombre" size="45" maxlength="60" 
				message="Introduzca el nombre del Beneficiario." />
	  <!--</li> 
