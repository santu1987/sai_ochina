...
$sord = $_GET['sord'];
$where="WHERE 1=1 ";
 if(isset($_GET['busq_nombre_usu']))
 {
	 $usuario=strtoupper($_GET['busq_nombre_usu']);
	 $where.=" AND upper(usuario.nombre) like '%$usuario%'
	 			OR upper(usuario.apellido) like '%$usuario%'";	
}
//************************************************************************
$limit = 15;
if(!$sidx) $sidx =1;
...
while (!$row->EOF) 
{
	...
	$responce->rows[$i]['cell']=array(	
															...
															$row->fields("usuario"),
															$row->fields("clave"),
															$row->fields("fecha_desde"),
															$row->fields("fecha_hasta"),
