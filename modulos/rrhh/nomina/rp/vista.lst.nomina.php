...
//{ 
	...
	while (!$row->EOF) 
	{
		...
		$pdf->Cell(50,	6,			'TIPO DE NÓMINA',			0,0,'C',1);
		$pdf->Cell(29,	6,			'SUELDO BASE',			0,0,'C',1);
		$pdf->Cell(30,	6,			'AÑOS DE SERVICIO',			0,1,'C',1);
		$pdf->SetFillColor(255) ;
		$pdf->SetTextColor(0);
		...
		/*$sql_cf = "SELECT 
					...
					INNER JOIN
						conceptos_fijos
					ON
						conceptos.id_concepto = conceptos_fijos.id_concepto
					INNER JOIN
						confij_tipo_nomina
					ON
						conceptos_fijos.id_concepto_fijos = confij_tipo_nomina.id_concepto_fijos
					WHERE confij_tipo_nomina.id_trabajador = ".$row->fields("id_trabajador")." AND confij_tipo_nomina.id_tipo_nomina= ".$_GET['id_tipo_nomina'];*/
		...
		$pdf->Cell(45,		6,			"ASIGNACIÓN ",		0,0,'L',1);
		$pdf->Cell(45,		6,			"DEDUCCIÓN ",		0,0,'L',1);
		...
		if($row_nomina->EOF){
			...
			//$pdf->SetTextColor(255);
			//$pdf->Cell(54,		6,			'TOTAL DEDUCCION',		1,0,'C',1,1);
			//$pdf->SetFillColor(255);
			//$pdf->SetTextColor(0);
			//$pdf->Cell(40,		6,			'',		1,0,'C',1);
	...
	$pdf->Line(144,$y1,144,$y2);
	$pdf->Line(144,$y2,184,$y2);
	//
	$pdf->Line(104,$y1,104,$y2);
	$pdf->Line(64,$y1,64,$y2);
	$pdf->Line(10,$y1,10,$y2-6);
	//
	$pdf->Line($x1,$y1-6,184,$y1-6);
	$pdf->Line($x1,$y1-18,184,$y1-18);
	$pdf->Line($x1,$y1,184,$y1);
