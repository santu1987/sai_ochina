...
$sql = "SELECT * FROM impuestos $where $sort $limit";
$result = runSQL($sql);

$total = countRec('id_impuesto','impuestos');
...
while ($row = pg_fetch_array($result)) {
	if ($rc) $json .= ",";
	$json .= "{";
		$json .= "id:$row[id_impuesto],";
		$json .= "cell:[";
			$json .= $row['id_impuesto'];
			$json .= ",'".addslashes(trim($row['nombre']))."'";			
			$json .= ",'".addslashes(trim($row['comentarios']))."'";
			$json .= ",'".addslashes(trim($row['fecha']))."'";
		$json .= "]";		
	$json .= "}";
	$rc = true;		
}
