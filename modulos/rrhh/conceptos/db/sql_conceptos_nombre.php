<?php
session_start();
require_once('../../../../controladores/db.inc.php');
require_once('../../../../utilidades/adodb/adodb.inc.php');
require_once('../../../../utilidades/jqgrid_demo/JSON.php');
$json=new Services_JSON();
$conn = &ADONewConnection('postgres');

$db=dbconn("pgsql");

$conn->PConnect($db["host"],$db["user"],$db["password"],$db["dbname"]);

//************************************************************************
//									HACIENDO LLAMADO A CONTROLES
//************************************************************************
//************************************************************************
//VARIABLES DE PAGINACION
$page = $_GET['page'];
$limit = $_GET['rows'];
$sidx = $_GET['sidx'];
$sord = $_GET['sord'];
//************************************************************************
$limit = 15;

foreach($_REQUEST as $variable => $valor){
	$_REQUEST[$variable] = htmlentities($_REQUEST[$variable]);
	//echo $_POST[$variable]." ";
}

$busq_nombre ="";
if (isset($_GET['busq_nombre']))
$busq_nombre = strtolower($_GET['busq_nombre']);
$busq_numero ="";
if ($_GET['busq_numero'])
$busq_numero = $_GET['busq_numero'];
//echo($busq_nombre);
////////////////////////////////
$where = " WHERE 1 = 1 ";
if($busq_nombre!='')
	$where.= " AND (lower(conceptos.descripcion) LIKE '%$busq_nombre%')";
if($busq_numero!='')
	$where.= " AND conceptos.id_concepto = $busq_numero ";
////////////////////

if(!$sidx) $sidx =1;

$Sql="
			SELECT 
				count(id_concepto) 
			FROM 
				conceptos 
				".$where."
			AND
				conceptos.id_organismo = $_SESSION[id_organismo]
				
";

$row=& $conn->Execute($Sql);
if (!$row->EOF)
{
	$count = $row->fields("count");
}

// calculation of total pages for the query
if( $count >0 ) {
	$total_pages = ceil($count/$limit);
} 
else {
	$total_pages = 0;
}

// if for some reasons the requested page is greater than the total
// set the requested page to total page
if ($page > $total_pages) $page=$total_pages;

// calculate the starting position of the rows
$start = $limit*$page - $limit; // do not put $limit*($page - 1)
// if for some reasons start position is negative set it to 0
// typical case is that the user type 0 for the requested page
if($start <0) $start = 0;

// the actual query for the grid data
$Sql="
			SELECT 
				conceptos.id_concepto,
				conceptos.descripcion, 
				conceptos.limite_inf,
				conceptos.limite_sup,
				conceptos.asignacion_deduccion,
				conceptos.observacion,
				conceptos.estatus,
				conceptos.num_orden
			FROM 
				conceptos 
			".$where."
			AND
				conceptos.id_organismo = $_SESSION[id_organismo]
			ORDER BY 
				$sidx $sord 
			LIMIT 
				$limit 
			OFFSET 
				$start;
				";
$row=& $conn->Execute($Sql);
// constructing a JSON
/*$sidx $sord 
			LIMIT 
				$limit 
			OFFSET 
				$start ;*/
$responce->page = $page;
$responce->total = $total_pages;
$responce->records = $count;
$i=0;

while (!$row->EOF) 
{
	//
	$limite_inf = $row->fields("limite_inf");
	if(strpos($limite_inf,'.')==0)
		$limite_inf = $limite_inf.",00";
	else
		$limite_inf = str_replace('.',',',$limite_inf);
	$limite_sup = $row->fields("limite_sup");
	if(strpos($limite_sup,'.')==0)
		$limite_sup = $limite_sup.",00";
	else
		$limite_sup = str_replace('.',',',$limite_sup);
	//	
	$responce->rows[$i]['id']=$row->fields("id_concepto");
	
	$responce->rows[$i]['cell']=array(	
															$row->fields("id_concepto"),
															$row->fields("descripcion"),
															$limite_inf,
															$limite_sup,
															$row->fields("asignacion_deduccion"),
															$row->fields("tipo_concepto"),
															$row->fields("observacion"),
															$row->fields("estatus"),
															$row->fields("num_orden")
														);
	$i++;
	$row->MoveNext();
}
// return the formated data
echo $json->encode($responce);

?>