...
$where.= " WHERE tipo_nomina.id_tipo_nomina = $id_tipo_nomina ";
$Sql="
			SELECT 
...
if (!$row->EOF) 
{
	...
	$responce =$row->fields("id_concepto_variable")."*". $row->fields("porcentaje") ."*". $row->fields("monto")."*". $row->fields("observacion")."*". $row->fields("id_concepto")."*". $row->fields("descripcion");
