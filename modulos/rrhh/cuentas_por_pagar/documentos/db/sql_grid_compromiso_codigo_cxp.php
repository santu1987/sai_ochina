...
$where="WHERE 
		...
		AND
					(\"orden_compra_servicioE\".numero_compromiso='$numero_compromiso')			
";
	$proveedor=$_POST['cuentas_por_pagar_db_proveedor_id'];
	$where.="AND \"orden_compra_servicioE\".id_proveedor='$proveedor'";
//************************************************************************
$limit = 15;
if(!$sidx) $sidx =1;
...
while (!$row_partidas->EOF) 
{
	...
	$responce =$row->fields("nombre")."*". $row->fields("numero_compromiso")."*". $row->fields("numero_orden_compra_servicio")."*".$fecha."*".number_format($total_renglon,2,',','.')."*".number_format($fact_ord,2,',','.')."*".$tipo."*".number_format($monto_ant,2,',','.')."*".number_format($porcentaje_ant,2,',','.')."*".$iva_anticipo."*".number_format($total_facturas,2,',','.')."*".$row->fields("id_proveedor")."*".$row->fields("codigo_proveedor")."*".$contar;	
