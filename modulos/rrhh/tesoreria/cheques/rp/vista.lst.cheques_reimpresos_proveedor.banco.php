...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{	global $nombre_banco;
			...
			$this->ln();
			$this->Cell(0,10,strtoupper($proveedor).":".$nombre_banco,0,0,'C');
            $this->ln();
			...
			$this->Cell(50,6,			 'BANCO EMITIDO',		0,0,'L',1);
			$this->Cell(30,6,		     'CUENTA ',	0,0,'L',1);
	...
	while (!$row->EOF) 
	{
		...
		$total_general=$total_general+$row->fields("monto_cheque");	
			...
			else
			{
				...
				$total_cuenta=$total_cuenta+$monto;
				$pdf->Cell(230,6,"Total Cuenta:".number_format($total_cuenta,2,',','.'),0,1,'R',1);
				//$pdf->Ln();	
	...
	if($banco_ant==$banco_sig)
		...
		$pdf->Cell(240,6,"TOTAL GENERAL:".number_format($total_general,2,',','.'),1,1,'R',1);
		$pdf->Ln();
