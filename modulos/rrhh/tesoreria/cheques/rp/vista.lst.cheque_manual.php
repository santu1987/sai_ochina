...
{ 
	...
	//-------------------
		...
		if(($id_proveedor=='0')or($id_proveedor==''))
		{		
	...
	*/	}
	...
	//$pdf->SetAutoPageBreak(auto,15);	
		...
		*/
		...
		$xe=0;
		$pdf->ln(1);
		$pdf->Cell(144,4,'***'.number_format($row->fields("monto_cheque"),2,',','.').'***',0,0,'R');
		$pdf->Ln(10);
		$pdf->Cell(100,4,"********".strtoupper($beneficiario)."********",0,0,'R');
		$pdf->Ln(22);
		$pdf->Cell(60,4,"La Guaira $dia2 de $mes   $ano",0,0,'R');
		$pdf->Ln(14);
		$pdf->Cell(128,4,$endosable,0,0,'R');
		$pdf->Ln(5);
		$pdf->Cell(133,4,$caducidad,0,0,'R');
		$pdf->Ln(22);
		$pdf->Cell(30,4,"".date("d-m-Y")."",0,0,'R');
		$pdf->Cell(80,4,$n_cheque,0,0,'C');
		$pdf->Ln(20);
		$pdf->SetFont('arial','B',9);
	    $pdf->Cell(45,4,substr($monto_escrito,0,65),0,0,'L');
		$pdf->SetFont('arial','B',9);
		$pdf->Cell(100,4,number_format($row->fields("monto_cheque"),2,',','.'),0,0,'R');
		$pdf->SetFont('arial','B',9);
		$pdf->Ln();
		...
		if(($con>200)&&($con<300))
			$pdf->MultiCell(180,9,$row->fields("concepto"),0,'LBR','L');
		if(($con>300)&&($con<400))
			$pdf->MultiCell(180,7,$row->fields("concepto"),0,'LBR','L');
		if(($con>400)&&($con<500))
	...
	//	$pdf->Ln(15);
    	...
    	$pdf->Cell(70,4,$nombre_director,0,0,'C');
		$pdf->Ln(18);
