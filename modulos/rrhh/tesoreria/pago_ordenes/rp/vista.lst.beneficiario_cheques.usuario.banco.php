...
class PDF extends PDF_Code128
	{
		...
		$this->Ln();	
			...
			$this->Cell(0,10,'RELACIÓN USUARIOS/BENEFICIARIOS/BANCO CHEQUES REGISTRADOS',0,0,'C');
            $this->ln(5);
			$this->Cell(0,10,"USUARIO:".strtoupper($nombre_usuario),0,0,'C');
			$this->Ln(5);
			$this->Cell(0,10,"BENEFICIARIO:".strtoupper($proveedor),0,0,'C');
			$this->Ln(5);
			$this->Cell(0,10,"BANCO:"." ".$nom_banco,0,0,'C');
			$this->Ln(5);
	...
	{		
			...
			//$n_cheque=$row->fields("numero_cheque");
				...
				$cuenta=$row->fields("cuenta_banco");
					...
					$pdf->Cell(40,6,$fecha_emitido,0,0,'L',1);
					$pdf->Cell(35,6,$monto_cheques,0,0,'L',1);
					$cheque=$row->fields("numero_cheque");
