...
class PDF extends PDF_Code128
	{
		...
		{	
			...
			$this->Cell(50,6,		     'MOVIMIENTOS',			0,0,'L',1);
...
while (!$row->EOF) 
	...
	$fecha_proceso = substr($fecha_proceso,8,2)."".substr($fecha_proceso,4,4)."".substr($fecha_proceso,0,4);
				...
				$pdf->SetFont('Arial','B',10);
						...
						$pdf->Cell(50,6,strtoupper($row->fields("referencia")),0,0,'L',1);
						$pdf->Cell(50,6,$fecha_proceso,	0,0,'L',1);
						$pdf->Cell(40,6,number_format($row->fields("monto"),2,',','.'),	0,0,'L',1);
	$pdf->Ln(6);					
	$row->MoveNext();
