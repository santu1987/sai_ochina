				...
				{	
						...
						$codigo_tipo_comprobante=$row->fields("codigo_tipo_comprobante");
								...
								if($contar==0)
								{
									...
									$pdf->ln(4);
									$pdf->Cell(0,10,'COMPROBANTE '." ".$numero_comprobante." "."del"." ".$fecha_comprobante,0,0,'C');
									$pdf->ln(4);
									...
									$pdf->SetTextColor(0);
									/*/*$pdf->Cell(20,6,		     'CUENTA',			0,0,'L',1);
									$pdf->Cell(40,6,			 'DESCRIPCION DE LA CUENTA',		0,0,'L',1);
									$pdf->Cell(50,6,			 'DESCRIPCION DEL ASIENTO',		0,0,'L',1);
									$pdf->Cell(10,6,			 'REF',		0,0,'L',1);
									$pdf->Cell(20,6,		     'DEBE',	0,0,'C',1);
									$pdf->Cell(20,6,		     'HABER',	0,0,'C',1);
									$pdf->Cell(20,6,		     'SECUENCIA',	0,0,'C',1);
									$pdf->Ln(6);
								}
								$pdf->SetFont('arial','',6);
								//$pdf->SetFillColor(255);
							...
							if($switch!=0)
							{
								...
								$pdf->Cell(40,6,strtoupper($row->fields("descripcion_cuenta")),1,0,'L',1);
							}
							else
							{
							$valor_concat="";
							$valor_concat2="   ";
							$desc=strtoupper($row->fields("descripcion_cuenta"));
								...
								/////////////////////////	
											...
											$x = $pdf->GetX();
											$y = $pdf->GetY();
											$pdf->MultiCell(20,3,$row->fields("cuenta_contable").$valor_concat2." ".auxiliar,1,'LBR','L');
											$pdf->SetXY($x+20,$y);
											
											//2da
											$x = $pdf->GetX();
											$y = $pdf->GetY();
											$pdf->MultiCell(40,3,substr(strtoupper($desc),0,25).$valor_concat." ".$cuenta_auxiliar." ".strtoupper($auxiliar),1,'LBR','L');
											$pdf->SetXY($x+40,$y);
											
							}
								$pdf->Cell(50,6,strtoupper($row->fields("descripcion")),1,0,'L',1);
								if(($row->fields("id_utilizacion_fondos")!="")&&($row->fields("id_utilizacion_fondos")!=0))	
								{
										...
										//1era//
											$x = $pdf->GetX();
											$y = $pdf->GetY();
											$pdf->MultiCell(10,3,$row->fields("referencia")."          "."UF".$cuenta_utf,1,'LBR','L');
											$pdf->SetXY($x+10,$y);
...
//$pdf->Cell(40,6,"aux"." ".$row->fields("codigo_auxiliar")."  ".$auxiliar,1,0,'L',1);
							...
							if($switch!=0)
							{
								...
								$pdf->Cell(40,6,substr(strtoupper($row->fields("descripcion_cuenta")),0,23),1,0,'L',1);
							}
							else
							{
							$valor_concat="";
							$valor_concat2="   ";
							$desc=substr(strtoupper($row->fields("descripcion_cuenta")),0,24);
							/*$desc2=strlen($desc);
							if($desc2<150)
							{
								while($desc2<100)
