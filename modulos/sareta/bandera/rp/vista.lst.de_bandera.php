...
$Sql="SELECT 
				...
				sareta.bandera.abreviatura, 
				sareta.bandera.obs 
			FROM 
				sareta.bandera
				WHERE 
				upper(sareta.bandera.nombre) like '".strtoupper($_GET['busq_nombre_bandera'])."%'
				or
				sareta.bandera.nombre like '".$_GET['busq_nombre_bandera']."%'
				ORDER BY 
				sareta.bandera.nombre 
		";
...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{		
			...
			$this->Ln();
			$this->Cell(0,10,'LISTADO DE BANDERAS',0,0,'C');
			$this->Ln(10);
	...
	while (!$row->EOF) 
	{
		...
		$pdf->Cell(50,		$esp,		utf8_decode($row->fields("nombre")),		1,'LBR','L');
		$pdf->Cell(10,		$esp,		utf8_decode($row->fields("abreviatura")),				1,'LBR','L');
		...
		$pdf->Cell(10,		6,		$row->fields("abreviatura"),				1,0,'L',1);
		$pdf->Cell(120,	6,		utf8_decode(str_replace("\n"," ",$row->fields("obs"))),						1,0,'L',1);
		$pdf->Ln(6);
		}
		$row->MoveNext();
	}
