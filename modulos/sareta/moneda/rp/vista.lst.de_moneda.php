...
$Sql="SELECT 
			...
			FROM 
				sareta.moneda
				WHERE 
				upper(nombre) like '".strtoupper($_GET['busq_nombre_moneda'])."%'
				or
				nombre like '".$_GET['busq_nombre_moneda']."%'
				ORDER BY 
				nombre 
		";
...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{		
			...
			$this->Cell(0,10,'LISTADO DE MONEDAS',0,0,'C');
			$this->Ln(10);
			...
			$this->Cell(25,	6,			utf8_decode('OPERACIÓN'),			0,0,'L',1);
			$this->Cell(90,	6,			utf8_decode('OBSERVACIÓN'),			0,0,'L',1);
			$this->Ln(6);
		}
		//Pie de página
		function Footer()
	...
	while (!$row->EOF) 
	{
		...
		$esp=6*$Cantidad_lineas_total;
											...
											$x1=$x;
											$y1=$y;
											$pdf->MultiCell(55,$esp,utf8_decode($row->fields("nombre")),1,'C','L');
											$pdf->SetXY($x+55,$y);
											...
											$x = $pdf->GetX();
											$y = $pdf->GetY();
											$pdf->MultiCell(25,$esp,utf8_decode($row->fields("multiplica_divide")),1,'LBR','L');
											$pdf->SetXY($x+25,$y);
											
											$x = $pdf->GetX();
											$y = $pdf->GetY();
											$pdf->MultiCell(90,6,utf8_decode(str_replace("\n"," ",$row->fields("obs"))),0,'LBR','L');
											$pdf->SetXY($x+90,$y);
											
											if($Cantidad_lineas_total==1){
											$pdf->line($x1,$y+$esp,$x+6,$y+$esp);
												$pdf->line($x+90,$y+$esp,$x+90,$y1);
											}else{													
											$pdf->line($x1,$y+$esp,$x+90,$y+$esp);
												$pdf->line($x+90,$y+$esp,$x+90,$y1);
												$pdf->line($x1,$y,$x+90,$y);
											}
