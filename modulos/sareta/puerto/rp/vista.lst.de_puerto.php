...
SELECT 
				id_puerto ,
  				sareta.puerto.id_bandera,
  				sareta.puerto.nombre,
				sareta.bandera.nombre as nombre_bandera,
  				sareta.puerto.obs
			FROM 
				sareta.puerto, sareta.bandera 
				WHERE id_puerto=id_puerto and id=id_bandera
				AND sareta.puerto.nombre like '%".$_GET['busq_nombre_puerto']."%'
			ORDER BY 
...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{		
			...
			$this->Ln();
			$this->Cell(0,10,'LISTADO DE PUERTOS',0,0,'C');
			$this->Ln(10);
			...
			$this->Cell(50,	6,			'BANDERA',			0,0,'L',1);
			$this->Cell(80,	6,			utf8_decode('COMENTARIO'),			0,0,'L',1);
			$this->Ln(6);
		}
		//Pie de página
		function Footer()
...
if($Cantidad_lineas_total_obs>=$Cantidad_lineas_total_nombre){
		...
		$esp=6*$Cantidad_lineas_total;
											...
											$pdf->SetXY($x+50,$y);
												$pdf->line($x+50,$y+$esp,$x+50,$y);
											$x = $pdf->GetX();
											$y = $pdf->GetY();
											$x1=$x;
											$y1=$y;
										
											$pdf->MultiCell(50,$esp1/$div,utf8_decode($row->fields("nombre_bandera")),0,1,'C',0);
											$pdf->SetXY($x+50,$y);
												$pdf->line($x+50,$y+$esp,$x+50,$y1);
											$x = $pdf->GetX();
											$y = $pdf->GetY();
											$pdf->MultiCell(80,$esp2/$div,utf8_decode(str_replace("\n"," ",$row->fields("obs"))),0,1,'C',0);
											$pdf->SetXY($x+80,$y);
											
											if($Cantidad_lineas_total==1){
											$pdf->line($x1-50,$y+$esp,$x+80,$y+$esp);
												$pdf->line($x+80,$y+$esp,$x+80,$y1);
												$pdf->line($x1-50,$y,$x+80,$y);
											}else{													
											$pdf->line($x1-50,$y+$esp,$x+80,$y+$esp);
												$pdf->line($x+80,$y+$esp,$x+80,$y1);
												$pdf->line($x1-50,$y,$x+80,$y);
