...
$Sql="SELECT 
			...
			FROM 
				...
				WHERE 
				upper(nombre) like '".strtoupper($_GET['busq_nombre_forma_de_pago'])."%'
				or
				nombre like '".$_GET['busq_nombre_forma_de_pago']."%'
				ORDER BY 
				nombre 
		";
...
{ 
	...
	class PDF extends PDF_Code128
	{
		...
		{		
			...
			$this->Cell(0,10,'LISTADO DE FORMAS DE PAGO',0,0,'C');
			$this->Ln(10);
			...
			$this->Cell(130,	6,			utf8_decode('COMENTARIO'),			0,0,'L',1);
			$this->Ln(6);
		}
		//Pie de página
		function Footer()
	...
	while (!$row->EOF) 
	{
		...
		$Logitud_Max_Texto = strlen($row->fields("comentario"));
