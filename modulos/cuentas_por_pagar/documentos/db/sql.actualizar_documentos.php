...
$partida_comp=$_POST[partida_comp];
								if(($monto_causar_comp!="")&&($partida_comp!=""))
								{
									...
									$sql_doc_det="select sum(monto) as monto from doc_cxp_detalle where id_doc='$id2'
...
if(($cerrado!="ano")||($cerrado!="mes"))
{
				...
				if(!$row->EOF)
				{
					...
					{
						echo($row->fields("numero_compromiso")."!=".$compromiso);
						die("documento_comp");	
					}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			if($partida_comp!="")
			{
				...
				$sql_ext="select id_documentos from documentos_cxp where numero_documento='$_POST[cuentas_por_pagar_db_numero_documento]' and numero_control='$_POST[cuentas_por_pagar_db_numero_control]' and id_proveedor='$_POST[cuentas_por_pagar_db_proveedor_id]'";
					...
					//consultando a ver cual es el monto total de esa partida//02902/2012 se quito el campo requisicion de las relaciones
$sql_orden="SELECT 
SUM((\"orden_compra_servicioD\".cantidad*\"orden_compra_servicioD\".monto)+(((\"orden_compra_servicioD\".cantidad*\"orden_compra_servicioD\".monto)*\"orden_compra_servicioD\".impuesto)/100)) as total_renglon,
									...
									if(!$row_orden_compra->EOF)
									{
										...
										$base_imponible=round($row_orden_compra->fields("base_imponible"),2);
									}
								...
								//	echo("entro2");
									...
									//*****************************************************************************************************************
															$sql_doc_det2="select SUM(monto) AS monto from doc_cxp_detalle where compromiso='$compromiso'
																				and partida='$t_part' and id_doc!='$id2'
																				";
																			//	die($sql_doc_det);
																				$row_doc_det2=& $conn->Execute($sql_doc_det2);
																				$total_det="";
																						if(!$row_doc_det2->EOF)
																						{
																							...
																							$validare=$total_det+$monto2;
																							//die($total_det."+".$monto2);
																						}
									$total_det=round($total_det,2);
									//$validare=round($validare,2);
									$total_renglon2=round($total_renglon2,2);
									//si la suma de la factura mas otras facturas supera a la bi q de un alerta
									if($validare>$total_renglon2)
									{
									//echo($validare."<=".$total_renglon2);
									/*die(".");*/
										die("monto_superior");
									}
									//
									...
									//******************************************************************************************************************						
															...
															if(($monto_causar_comp!="")&&($partida_comp!=""))
															{
																	...
																	//para el caso de que los montos cargados sean iguales al monto del renglon y se deseen modificar
																if($total_renglon2==$total_det)
																{
																	$monto_causar=$total_renglon2;
																}
																//
