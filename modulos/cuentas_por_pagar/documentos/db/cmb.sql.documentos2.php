...
//*********************************************************************************************************************//
function cargar_partidas($numero_compromiso)
{
require_once('../../../../controladores/db.inc.php');
require_once('../../../../utilidades/adodb/adodb.inc.php');
require_once('../../../../utilidades/jqgrid_demo/JSON.php');
...
if($_GET['anos'])
{
	...
	if($ano_s!="")
	{
		$where.="and (documentos_cxp.ano)='$ano_s'";
	}
}
...
$sql_orden="SELECT 
				...
				//	 die($sql_orden);
									...
									while(!$row_orden_compra->EOF)
									{
										...
										$cta_orden=$row_orden_compra->fields("cuenta_orden");
...
// $porcentaje_ant="0";
	...
	if(($row->fields("numero_comprobante")!='0')&&($row->fields("numero_comprobante")!=''))
	{
				...
				$sql_integrado="SELECT distinct movimientos_contables.id_movimientos_contables,
									movimientos_contables.cuenta_contable,
									id_tipo_comprobante,
									movimientos_contables.descripcion as nombre_asiento, 
									referencia,
									monto_debito, 
							...
							where
								movimientos_contables.debito_credito=1	
							AND
								movimientos_contables.numero_comprobante='$number'		
							order by
									movimientos_contables.secuencia
	...
	if(($row->fields("numero_compromiso")!="")&&($row->fields("numero_compromiso")!=0)&&($row->fields("numero_compromiso")!="NULL"))
	{
		...
		$partidas=cargar_partidas($row->fields("numero_compromiso"));
	}	
