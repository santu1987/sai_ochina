...
if($busq_nom!="")
{
	$where.="AND upper(proveedor.nombre) like '%$busq_nom%'";
}
if($busq_com!="")
{
	$where.="AND \"orden_compra_servicioE\".numero_compromiso like '%$busq_com%' ";
}
...
$Sql = "	
				SELECT  DISTINCT
				   ...
				   \"orden_compra_servicioE\".id_orden_compra_servicioe,
					...
					\"orden_compra_servicioE\".cuenta_orden,
					proveedor.nombre,
...
////////////////////////////////////////////////////////////////////////////////////////////////////
								...
								/*if($compromiso=='110288')
								{
									echo($row_factura->fields("monto_base_imponible")."*".$row_factura->fields("porcentaje_iva"));
									die("-");
								}*/
								//die($monto_factura);
										...
										if(($row_factura->fields("tipo_documentocxp")==$tipos_fact)&&($row_factura->fields("amortizacion")!='0'))
											...
											$fact_ord=round($total_renglon,2)-round($total_facturas,2);
							$monto_anticipo=$total_facturas;
							if($total_renglon!=0)
							$porcentaje_ant=($monto_ant*100)/$total_renglon;	
					//		if($compromiso=='110288')die($total_renglon."-".$total_facturas);
...
while (!$row_partidas->EOF) 
{
	...
	//if($compromiso=='110256')echo(($total_renglon)."-".($total_facturas));	
		//echo(($total_renglon)."-".($total_facturas));die("-");				
