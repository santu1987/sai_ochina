...
if(isset($_GET["causado_pagado_compomiso"] ))
{
	$compromiso=$_GET["causado_pagado_compromiso"];
	if($compromiso!="")
	{
		$where.="AND doc_cxp_detalle.compromiso='$compromiso'";
	}
}
	...
	if(isset($_GET["desde_fecha"]))
	{
		...
		$where.=" and documentos_cxp.fecha_documento >= '$desde_fecha'";
	}
	if(isset($_GET["hasta_fecha"]))
		{	
			$hasta_fecha=$_GET["hasta_fecha"];
			$where.="AND documentos_cxp.fecha_documento <='$hasta_fecha'";
		}
/*if($desde_fecha!="")
	{	
...
//************************************************************************
			class PDF extends PDF_Code128
			{
				...
				$this->SetFont('times','B',8);
					...
					$this->SetTextColor(0);
					$this->Cell(20,6,		     'CUENTA',			1,0,'L',1);
					$this->Cell(25,6,			 'N COMPROMISO',		1,0,'L',1);
					$this->Cell(30,6,			 'N DOCUMENTO',		1,0,'C',1);
					$this->Cell(45,6,			 'MONTO CAUSADO',		1,0,'C',1);
					$this->Cell(25,6,			 'N CHEQUE',		1,0,'C',1);
					$this->Cell(40,6,		     'MONTO PAGADO',	1,0,'C',1);
					$this->Ln();
				}
				//Pie de página
				function Footer()
				...
				while(!$row->EOF)
				{
						...
						/////verificar si tiene cheque y cuanto ha sido pagado
							...
							while(!$row_cheque->EOF)
							{
								...
								$row_cheque->MoveNext();
								$pagados_cheques=$row->fields("monto_causado");
							}				
						/////
						$id_doc=$row->fields("id_doc");
						//$monto= $row->fields("monto_causado");
						$monto=0;
						$pdf->Cell(20,6,		     $row->fields("partida"),			1,0,'L',1);
						$pdf->Cell(25,6,			 $row->fields("compromiso"),		1,0,'L',1);
						$pdf->Cell(30,6,			 $row->fields("numero_documento"),		1,0,'C',1);
						$pdf->Cell(45,6,			 number_format($row->fields("monto_causado"),2,',','.'),		1,0,'R',1);
						$pdf->Cell(25,6,			 $cheque,		1,0,'C',1);
						$pdf->Cell(40,6,		     number_format(0,2,',','.'),	1,0,'R',1);
						$pdf->Ln(6);
						...
						///
							...
							$pagados=$pagados+$pagados_cheque;
							$total_causados_partida=$total_causados_partida+$row->fields("monto_causado");
							$total_pagados_partida=$total_pagados_partida+$pagados;
						...
						if($partida_ant!=$partida_sig)
						{
							...
							$pdf->SetTextColor(0);
							$pdf->Cell(50,6,"TOTAL "."  ".$partida_ant.":",'LTB',0,'C',1);
							$pdf->Cell(70,6,number_format($causados,2,',','.'),'TB',0,'R',1);
							$pdf->cell(65,6,number_format($pagados,2,',','.'),'TBR',0,'R',1);
							$pdf->Ln();
							$causados=0;
							$pagados=0;
							$pdf->SetFont('times','',6);
						...
						if(($generica_ant!=$generica2_sig)&&($len!=9))
						{
							...
							$pdf->SetFillColor(175) ;
							$pdf->SetTextColor(0);
							$pdf->Cell(50,6,"TOTAL "."  ".$generica_ant.":",'LTB',0,'C',1);
							$pdf->Cell(70,6,number_format($total_causados_partida,2,',','.'),'TB',0,'R',1);
							$pdf->cell(65,6,number_format($total_pagados_partida,2,',','.'),'TBR',0,'R',1);
							$pdf->Ln();
