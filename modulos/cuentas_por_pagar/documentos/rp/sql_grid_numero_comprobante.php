...
if(isset($_GET[busq_cuenta]))
{
	...
	//die($busq_cuenta);
	if($busq_cuenta!="")
	$where.="and substring(movimientos_contables.numero_comprobante::varchar,11) like'%$busq_cuenta%'";

}
//if($busq!="") $where.= " AND ano = $busq";
$anos=strtoupper($_GET["anos"]);
if(isset($_GET[anos]))
{
	$where.="and ano_comprobante='$anos'";
}	
$tipo=strtoupper($_GET["tipo"]);
if(isset($_GET[tipo]))
{
	if($tipo!="")
	$where.="and substring(movimientos_contables.numero_comprobante::varchar,9,2) like'%$tipo%'";
}			
//************************************************************************
$limit = 15;
if(!$sidx) $sidx =1;
...
$Sql="
		select 
				distinct movimientos_contables.numero_comprobante,fecha_comprobante
			from 
				movimientos_contables
			INNER JOIN
				documentos_cxp
