...
$busq_ano=$_POST[cuentas_por_pagar_db_ayo_orden_pago];
...
if($opcion=='1')
		...
		{		
					...
					$benef2=$row->fields("beneficiario");
	...
	$responce =$row->fields("id_orden_pago")."*".  $row->fields("orden")."*".$row->fields("documentos")."*".$id_proveedor."*".$codigo_proveedor."*".$beneficiario."*".  $row->fields("rif")."*".  $row->fields("ano")."*".  $row->fields("fecha_orden_pago")."*".$row->fields("comentarios")."*".$fecha."*".  $row->fields("estatus")."*".$opcion."*".$tipo."*".$benef2;
	echo ($responce);
