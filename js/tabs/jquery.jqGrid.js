...
$.fn.jqGrid = function( p ) {
	...
	return this.each( function() {
		...
		var addSubGrid = function(t,row,pos) {
			var td;
			td = document.createElement("td");
			$(td,t).html("<img src='../../../recursos/jqgrid_demo/js/"+ts.p.imgpath+"plus.gif'/>").toggle( function() { 
				$(this).html("<img src='../../../recursos/jqgrid_demo/js/"+ts.p.imgpath+"minus.gif'/>");
				var req = populatesubgrid($(this).parent()); var atd= pos==1?'<td></td>':'';
				var subdata = "<tr class='subgrid'>"+atd+"<td><img src='../../../recursos/jqgrid_demo/js/"+ts.p.imgpath+"line3.gif'/></td><td colspan='"+parseInt(ts.p.colNames.length-1)+"'><div class='tablediv'>"; 
				$(this).parent().after( subdata+ req +"</div></td></tr>" );$(".tablediv",ts).css("width", ts.grid.width-20+"px");	}, 
				function() { $(this).parent().next().remove(".subgrid"); $(this).html("<img src='../../../recursos/jqgrid_demo/js/"+ts.p.imgpath+"plus.gif'/>");
			});
			formatCol($(td,t), pos);
			row.appendChild(td);
		}
		...
		var setPager = function (){
			$(ts.p.pager).append('<span></span> <img id="first" src="../../../recursos/jqgrid_demo/js/'+ts.p.firstimg+'">  <img id="prev" src="../../../recursos/jqgrid_demo/js/'+ts.p.previmg+'"> <input class="selbox" type="text" size="3" maxlength="5" value="0"/><span></span> <img id="next" src="../../../recursos/jqgrid_demo/js/'+ts.p.nextimg+'">  <img id="last" src="../../../recursos/jqgrid_demo/js/'+ts.p.lastimg+'">');
		...
		var sortData = function (index, idxcol){
			...
			imgs = "<img src='../../../recursos/jqgrid_demo/js/"+imgs+"'>";
