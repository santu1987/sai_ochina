	...
	return this.each( function() {
		...
		var populate = function () {
			if(!grid.hDiv.loading) {
				...
				{
				case "json":
					$.ajax({url:ts.p.url,type:ts.p.mtype,dataType:"json",data: gdata, complete:function(JSON,st) { if(st=="success") {addJSONData(eval("("+JSON.responseText+")"),ts.grid.bDiv); if(loadComplete) {loadComplete();}}}, error:function(xhr,st,err){
		...
		if(ts.p.caption) {
			...
			if(ts.p.hidegrid===true) {
				$(".HeaderButton",grid.cDiv).toggle( function(){
					...
					if(onHdCl) {
    if(!hg) {ts.p.onHeaderClick(ts.p.gridstate);}}
					},
					function() {
